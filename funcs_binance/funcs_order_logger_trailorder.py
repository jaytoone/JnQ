...
def limit_order(self, order_type, config, limit_side, limit_price, limit_quantity, order_info=None, reduceonly=False):
    open_retry_cnt = 0
    res_code = 0
    print("self.over_balance :", self.over_balance)
    while 1:  # <-- loop for complete open order
        try:
            # ------------ post_order - limit & market ------------ #
            if order_type == OrderType.MARKET:
                # ------ market order ------ #
                post_order_res = request_client.post_order(symbol=config.trader_set.symbol, side=limit_side, ordertype=OrderType.MARKET,
                                                           quantity=str(limit_quantity))
            else:
                # ------ limit order - needs execution waiting time & check remaining order ------ #
                post_order_res = request_client.post_order(timeInForce=TimeInForce.GTC, symbol=config.trader_set.symbol,
                                                           side=limit_side,
            ...
            open_retry_cnt += 1     # Todo - for tp_executedQty miscalc.
            if '-2019' in str(e):
                try:
...
def partial_limit_order_v2(self, config, tp_list, close_side, open_executedQty, quantity_precision, partial_qty_divider):
    tp_count = 0
    # retry_cnt = 0
    post_order_res_list = []
    ...
    #        2. 완결성 보장된 list comprehensin post_ordering
    while 1:  # loop for partial tp
        ...
        # ------------ partial tp ------------ #
        # try:
        post_order_res, _, res_code = limit_order(self, config.tp_set.tp_type, config, close_side, tp_level, quantity, reduceonly=True)
        ...
        if post_order_res is not None:
            ...
            if order_type == OrderType.LIMIT:
                while 1:
                    try:
                        ...
                        sys_log.error('error in cancel_order (market_close_order phase) : {}'.format(e))
            ...
            while 1:
                try:
                    ...
                    tp_executedQty_list.append(float(order_info_res.executedQty))
                except Exception as e:
                    sys_log.error('error in get_order_info (sum(tp_executedQty) phase) : {}'.format(e))
                else:
                    break

    return np.sum(tp_executedQty_list)


def market_close_order(self, config, remain_tp_canceled, post_order_res_list, close_side, open_executedQty, out, log_tp):
    while 1:  # <--- This loop for complete close
        if not remain_tp_canceled:
            tp_executedQty = cancel_order_list(config.trader_set.symbol, post_order_res_list)
        ...
        while 1:  # <-- loop for complete close order
            try:
                #        1. limit_out 은 한동안 없을 것 (market_close 에 limit_out 이라..)
                if config.out_set.out_type != OrderType.MARKET:
                    if pd.isna(out):
                        try:
                            #   Todo <-- for what ?   #
                            exit_price = get_market_price_v2(self.sub_client)   # self, 인자로 받을 수 있음
                        except Exception as e:
                            sys_log.error('error in get_market_price_v2 (close_order phase): {}'.format(e))
                            continue
                    else:
                        exit_price = log_tp
                    # ------ limit order ------ #
                    ...
                    # ------ market order ------ #
                    post_order_res = request_client.post_order(symbol=config.trader_set.symbol, side=close_side,
                                                               ordertype=OrderType.MARKET,
                                                               quantity=str(close_remain_quantity),
                                                               reduceOnly=True)

            except Exception as e:
                sys_log.error('error in close order : {}'.format(e))
                ...
                if '-4003' in str(e):
                    error_code = '-4003'
                    sys_log.error('error code check (-4003) : {}'.format(e))
                    break
        ...
        if config.out_set.out_type != OrderType.MARKET:
            ...
            sys_log.info("config.trader_set.exit_execution_wait - datetime.now().second : {}"
        ...
        if error_code in ['-2022', '-4003']:    # post_order_res 가 정의되지 않은 경우
            sys_log.info('market close order executed')
            return
        else:
            if check_exec_by_order_info(config.trader_set.symbol, post_order_res, quantity_precision):
                sys_log.info('market close order executed')
                return
