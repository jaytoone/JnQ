...
def get_trade_result_table(bank, point_index_short, point_index_long, priceBox_indicator, priceBox_value, interval):
    
    """
    Generate trade result table based on given parameters.

    Parameters:
    - bank: Object containing necessary data and methods for trade analysis.
    - point_index_short: List or array of indices for short trades.
    - point_index_long: List or array of indices for long trades.
    - priceBox_indicator: Indicator for price box calculation.
    - priceBox_value: Value for price box calculation.
    - interval: Interval for price box calculation.

    Returns:
    - table_trade_result: DataFrame containing trade results with specified columns.
    """
...
def get_minimum_assets(trade_data):
    ...
    assets_over_time = current_assets.cumsum().to_numpy()
