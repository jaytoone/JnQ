...
def low_high(reg_target, sigma, crop_size=0, realtime=0):

    none_indexs = sum(np.isnan(reg_target.values))
    trade_state = [np.nan] * len(reg_target)
    gaussian_target = gaussian_filter1d(reg_target, sigma=5)
    ...
    if crop_size != 0:
        ...
        # reg_target_copy = scaler.fit_transform(reg_target.values[none_indexs:].reshape(-1, 1))
        # for i in range(crop_size, len(reg_target_copy)):
        #     group_x = reg_target_copy[i + 1 - crop_size:i + 1]
        #     dataX.append(group_x)

        # Made_X = np.array(dataX)
        # model = load_model('model/rapid_ascending %s_%s_ohlc.hdf5' % (10, 108))
        # reg_target_predict = model.predict(Made_X.reshape(-1, crop_size, 1))

        # reg_target_offset = np.full((len(reg_target_copy), 1), np.nan)
        # reg_target_offset[-len(reg_target_predict):] = reg_target_predict

        # print((reg_target_offset))
        # quit()
        # plt.plot(max_abs_scaler(reg_target_offset[sum(np.isnan(reg_target_offset)):]), color='orange')
        # plt.plot(max_abs_scaler(reg_target[10:]), color='fuchsia')
        # plt.show()

        reg_target_offset = reg_target.shift(-10)
        # print(reg_target_offset)
        # quit()
        ...
        # reg_target_copy_for_offset = reg_target.copy()
        # reg_target_copy_for_offset[none_indexs:] = reg_target_offset.reshape(-1, )
        # reg_target[none_indexs:] = reg_target_copy.reshape(-1, )
        # # reg_target_copy_for_offset = max_abs_scaler(reg_target_copy_for_offset)
        # # reg_target = max_abs_scaler(reg_target)
        # for i in range(len(reg_target)):
        #     if reg_target_copy_for_offset[i] > reg_target[i]:
        #         trade_state[i] = 'up'
        #     else:
        #         trade_state[i] = 'down'
        # return reg_target_copy_for_offset, reg_target, trade_state

        for i in range(len(reg_target)):
            if reg_target_offset[i] > reg_target[i]:
                trade_state[i] = 'up'
            else:
                trade_state[i] = 'down'

        # plt.plot(reg_target_copy_for_offset.values)
        # plt.plot(reg_target.values)
        # spanlist = list()
        # for i in range(len(reg_target)):
        #     if trade_state[i] == 'up':
        #         spanlist.append((i, i + 1))
        # for i in range(len(spanlist)):
        #     plt.axvspan(spanlist[i][0], spanlist[i][1], facecolor='c', alpha=0.5)
        # plt.show()
        return reg_target_offset, reg_target, trade_state
    ...
    #           for non realtime svr        #
        ...
        for i in range(none_indexs, len(y_smooth)):
            if y_smooth[i - 1] < y_smooth[i]:
                trade_state[i] = 'up'
            elif y_smooth[i - 1] >= y_smooth[i]:
                trade_state[i] = 'down'

        return None, y_smooth, trade_state


def profitage(Coin, reg_target, short, long, signal, sigma, crop_size, realtime, Date='2019-09-25', excel=0, get_fig=0):
    ...
    # print(df[reg_target])
    # quit()
    osc_reg_0, osc_reg, trade_state = low_high(df[reg_target], sigma, crop_size, realtime=realtime)
    df['osc_reg_0'] = osc_reg_0
    df['osc_reg'] = osc_reg
    df['trade_state'] = trade_state
    df = df.iloc[-sum(~np.isnan(osc_reg)):]

    # osc_1 = max_abs_scaler(df['MACD_OSC'].copy())
    # macd(df, short=short, long=long, signal=signal-10)
    # osc_0 = max_abs_scaler(df['MACD_OSC'].copy())
    #
    # df['trade_state'] = np.nan
    # for i in range(len(df)):
    #     if osc_0[i] > osc_1[i]:
    #         df['trade_state'].iloc[i] = 'up'
    #     else:
    #         df['trade_state'].iloc[i] = 'down'
    #
    # df = df.iloc[-sum(~np.isnan(df.trade_state.values)):]
    ...
    while m <= length:
        ...
        while True:
            ...
            #           Realtime SVR            #
            #   down >> up 이면 매수 등록    #
            if (df['trade_state'].iloc[m - 1] == 'down') and (df['trade_state'].iloc[m] == 'up'): #  and (df['osc_reg'].iloc[m] > 0)
    ...
    if len(profits) == 0:
        ...
        plt.subplot(312)
        plt.plot(df.osc_reg_0.values, 'orange', label='gaussian_0')
        # plt.plot(osc_0.values, 'orange', label='gaussian_0')
        plt.plot(df.osc_reg.values, 'fuchsia', label='gaussian_1')
        # plt.plot(osc_1.values, 'fuchsia', label='gaussian_1')
        plt.xlim(0, len(ochl))
        ...
        for i in range(1, len(df)):
            # if df['osc_reg'][i - 1] <= df['osc_reg'][i]:
            #     span_list.append((i, i + 1))
            if df.trade_state[i] == 'up':
                span_list.append((i, i + 1))
        for i in range(len(span_list)):
            plt.axvspan(span_list[i][0], span_list[i][1], facecolor='c', alpha=0.5)

        plt.subplot(313)
        _, osc_reg, _ = low_high(df[reg_target], sigma=10)
        plt.plot(osc_reg, 'r', label='oscillator', linewidth=1.0)
        ...
        # plt.show(block=False)
        # plt.pause(0.3)
        # plt.savefig("./Figure_reg/reg_result/%s %s %s %s.png" % (Date, Coin, sigma, crop_size), dpi=300)
        plt.close()
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['TMTG', 'APIX', 'FNB', 'LBA', 'SNT', 'QKC', 'BTG', 'LUNA', 'MXC', 'ORBS']
    # TopCoin = ['WOM']
                ...
                for Coin in TopCoin:
                    ...
                    # try:
                        print(Coin, profitage(Coin, 'MACD_OSC',
                                              short, long, signal, sigma=10, crop_size=10, realtime=0, Date=Date, get_fig=1, excel=0))
