from funcs.upbit.shield_module_v4_2 import ShieldModule
from funcs.public.broker import intmin_np
from funcs.upbit.constant import *
...
class Shield(ShieldModule):
    ...
    def load_data(self):
        ...
        # 2. open_remain_data_dict restoration.
        try:
        ...
        # 3. close_remain_data_dict restoration.
        try:
    ...
    def get_income_info(self, real_balance, leverage, ideal_profit, real_profit, mode="PROD", currency="USDT"):  # 복수 pos 를 허용하기 위한 api 미사용
        ...
        if mode == "PROD":
            self.accumulated_profit *= 1 + real_tmp_profit  # 0.0033
    ...
    def run(self):
        ...
        while 1:
            ...
            for code, open_data in self.open_data_dict.copy().items():
                ...
                tp, ep, out, liqd_p = [self.get_tick_size(price_) for price_ in [tp, ep, out, liqd_p]]  # includes half-dynamic tp
                ...
                if not self.config.trader_set.backtrade and not fake_order:
                    ...
                    post_order_res, self.over_balance, error_code = self.post_order(code, self.config.ep_set.entry_type, open_side, pos_side, ep, open_quantity, order_data)
            ...
            for code, open_remain_data in self.open_remain_data_dict.copy().items():
                ...
                # 2. limit
                    ...
                    if not self.config.trader_set.backtrade:
                        ...
                        #   [ API ] : order_info
                        realtime_price = self.get_market_price_v4(code)
            ...
            for code, close_data in self.close_data_dict.copy().items():
                ...
                # price_precision, quantity_precision = self.get_precision(code)
                quantity_precision = 8
                partial_tps, partial_qtys = self.get_partial_tp_qty(ep, tp, open_exec_qty, None, quantity_precision, close_side)
                ...
                if not self.config.trader_set.backtrade and not fake_order:
                    ...
                    # [ API ]
                    post_order_res_list = self.close_order_limit(code, post_order_res_list, partial_tps, partial_qtys, close_side, pos_side, open_exec_qty, quantity_precision)
            ...
            for code, close_remain_data in self.close_remain_data_dict.copy().items():
                ...
                if not self.config.trader_set.backtrade and not fake_order:
                    ...
                    all_executed, tp_exec_price_list = self.check_close_limit_exec_v2(post_order_res_list, quantity_precision)
                ...
                if market_close_on:
                    ...
                    if not self.config.trader_set.backtrade and not fake_order:
                        ...
                        out_exec_price_list = self.close_order_market(code, post_order_res_list, close_side, pos_side, open_exec_qty)
                    else:
                ...
                if all_executed or market_close_on:
                    ...
                    if not fake_order:
                        self.get_income_info(real_balance, leverage, ideal_profit, real_profit, mode=self.config.trader_set.profit_mode, currency="KRW")
