...
class ARIMA_Bot:
    ...
    def __init__(self, symbol, interval, tp, tp2, leverage_, initial_asset, stacked_df_on=False):

        self.last_index = -1
        self.symbol = symbol
        self.interval = interval
        self.tp, self.tp2, self.leverage_ = tp, tp2, leverage_
            ...
            while 1:
                ...
                if init_arima:
                    try:
                        ...
                        #         ARIMA process          #
                        df, pred_close, err_range = arima_profit(stacked_df.iloc[:-1, :], tp=self.tp, tp2=self.tp2, leverage=self.leverage_)  # <-- we should use complete data
                        init_arima = False
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                if order.entry_type == OrderType.MARKET:
                    ...
                    if open_side == OrderSide.BUY:
                        ...
                        tp_level = df['long_tp_level'].iloc[self.last_index]

                    else:
                        ep = df['short_ep'].iloc[self.last_index]
                        tp_level = df['short_tp_level'].iloc[self.last_index]
                ...
                tp_level = calc_with_precision(tp_level, price_precision)
                ...
                if order.entry_type == OrderType.MARKET:
                    ...
                    while 1:  # <-- loop for complete open order
                        try:
                            ...
                            if 'insufficient' in str(e):
                                available_balance = max_available_balance * (0.9 - 0.01 * open_retry_cnt)
                                #          Get available quantity         #
                                quantity = available_balance / ep * leverage
                                quantity = calc_with_precision(quantity, quantity_precision)
                                print('available_balance :', available_balance)
                    ...
                    while 1:
                        ...
                        #       100 % 체결 확인     #
                        if datetime.now().second >= 59:  # <-- api limit 을 피하기 위한 line
                            try:
                            ...
                            if exec_quantity >= quantity * 0.99:
                                break
            ...
            #       position / 체결량 이 존재하면 close order 진행      #
                ...
                while 1:  # limit close loop
                    ...
                    while 1:
                        ...
                        if current_datetime.timestamp() >= datetime.timestamp(stacked_df.index[-1]) - 4:
                        # if current_datetime.second >= (fundamental.bar_close_second - 4):
                            ...
                            if quantity == 0.0:
                                print('Limit close order executed.')
                                tp_switch = True
                ...
                if open_side == OrderSide.BUY:
                    if tp_switch:
                        back_tmp_profit = tp_level / ep - self.trading_fee
                    else:
                        back_tmp_profit = back_df['close'].iloc[-2] / ep - self.trading_fee
                else:
                    if tp_switch:
                        back_tmp_profit = ep / tp_level - self.trading_fee
                    else:
