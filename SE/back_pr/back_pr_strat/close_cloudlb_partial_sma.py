    ...
    for senkou_a, senkou_b in zip(senkoua_list, senkoub_list):
        ...
        while 1:
            ...
            if entry[i] in short_entry:
                ...
                # -------------- sma const. -------------- #
                if res_df['close'].shift(1).iloc[i] < res_df[sma].shift(1).iloc[i]: # and \
            ...
            #                  long  phase                #
                ...
                #     continue
                #
                # entry_input_x = min_max_scale(
                #     res_df[selected_price_colname].iloc[initial_i - input_size:initial_i].values)
                #
                # re_entry_input_x = expand_dims(entry_input_x)
                #
                # entry_vector = model.predict(re_entry_input_x, verbose=0)
