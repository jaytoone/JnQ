...
class Trader:
    ...
    def run(self):
        ...
        while 1:
            ...
            # df_path = dir_path + '/%s.xlsx' % init_set.symbol

            if init_set.symbol_changed:
                initial_run = 1
            ...
            if initial_run:
                ...
                while 1:    # for completion
                    ...
                    try:
                        request_client.change_margin_type(symbol=init_set.symbol, marginType=FuturesMarginType.ISOLATED)
                    ...
                    try:
                        limit_leverage = get_limit_leverage(symbol_=init_set.symbol)
                    except Exception as e:
                    ...
                    # --- 3. sub_client --- #
                    try:
                        sub_client.subscribe_aggregate_trade_event(init_set.symbol.lower(), callback, error)
            ...
            while 1:
                ...
                if entry_const_check:
                    ...
                    # if init_set.stacked_df_exist:

                    try:

                        new_df_, _ = concat_candlestick(init_set.symbol, init_set.interval, days=1, limit=init_set.use_rows,
                        ...
                        new_df2_, _ = concat_candlestick(init_set.symbol, init_set.interval2, days=1, limit=init_set.use_rows2,
                                                         timesleep=0.2)
                        if datetime.timestamp(new_df2_.index[-1]) < datetime.now().timestamp():
                            continue

                        new_df3_, _ = concat_candlestick(init_set.symbol, init_set.interval3, days=1, limit=init_set.use_rows3,
                                                         timesleep=0.2)
                        if datetime.timestamp(new_df3_.index[-1]) < datetime.now().timestamp():
                            continue

                        new_df = new_df_.iloc[-init_set.use_rows:].copy()
                        new_df2 = new_df2_.iloc[-init_set.use_rows2:].copy()
                        new_df3 = new_df3_.iloc[-init_set.use_rows3:].copy()
                    ...
                    #
                    #         try:
                    #             # if not init_set.stacked_df_exist:
                    #             days = calc_train_days(interval=init_set.interval, use_rows=init_set.use_rows)
                    #             stacked_df, _ = concat_candlestick(init_set.symbol, init_set.interval, days=days, timesleep=0.2)
                    #             stacked_df = stacked_df.iloc[-init_set.use_rows:, :]
                    #             init_set.stacked_df_exist = True
                    #
                    #             stacked_df.to_excel(df_path)
                    #
                    #         except Exception as e:
                    #             print("error in load new stacked_df :", e)
                    #             init_set.stacked_df_exist = False
                    #             continue
                    ...
                    try:
                        ...
                        short_out, long_out, short_tp, long_tp = enlist_epouttp(res_df, tp_set.tp_gap)
                    ...
                    try:
                        ...
                        if res_df['close'].iloc[init_set.last_index] <= res_df['cloud_bline_3m'].iloc[init_set.last_index]:

                            # ----------- minor_markup ----------- #
                            if entry[init_set.last_index] == -1:

                                # -------------- ep scheduling -------------- #
                                    # ----- st_gap ----- #
                                if res_df['close'].iloc[init_set.last_index] \
                                    >= res_df['st_lower2_5m'].iloc[init_set.last_index] + ep_set.ep_protect_gap * res_df['st_gap_5m'].iloc[init_set.last_index]:
                                ...
                                # -------------- tp scheduling -------------- #
                                    ...
                                    if (short_tp.iloc[init_set.last_index] / res_df['close'].iloc[init_set.last_index] - self.trading_fee) / abs(
                                            short_tp.iloc[init_set.last_index] / res_df['close'].iloc[init_set.last_index] + self.trading_fee) > tp_set.pgfr:
                        ...
                        if res_df['close'].iloc[init_set.last_index] >= res_df['cloud_bline_3m'].iloc[init_set.last_index]:
                            ...
                            if entry[init_set.last_index] == 1:
                                ...
                                # -------------- ep scheduling -------------- #
                                    # ----- st_gap ----- #
                                if res_df['close'].iloc[init_set.last_index] \
                                        <= res_df['st_upper2_5m'].iloc[init_set.last_index] - ep_set.ep_protect_gap * \
                                        res_df['st_gap_5m'].iloc[init_set.last_index]:

                                # -------------- tp scheduling -------------- #
                                    # ----- pgfr ----- #
                                    if (long_tp.iloc[init_set.last_index] / res_df['close'].iloc[init_set.last_index] - self.trading_fee) / abs(
                                            long_tp.iloc[init_set.last_index] / res_df['close'].iloc[init_set.last_index] + self.trading_fee) > tp_set.pgfr:
                ...
                if open_side is not None:
                    ...
                    if check_entry_sec > ep_set.check_entry_sec:
                        open_side = None
                ...
                # ------- open_side is None [ waiting zone ] ------- #
                    ...
                    time.sleep(init_set.realtime_term)  # <-- term for realtime market function
                    ...
                    # time.sleep(init_set.close_complete_term)   # <-- term for close completion
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                # if ep_set.entry_type == OrderType.MARKET:
                #     #             ep with market            #
                #     try:
                ...
                if open_side == OrderSide.BUY:
                    ep = short_ep.iloc[init_set.last_index]  # inversion mode adjusted
                    out = short_out.iloc[init_set.last_index]
                    tp = short_tp.iloc[init_set.last_index]
                else:
                    ep = long_ep.iloc[init_set.last_index]
                    out = long_out.iloc[init_set.last_index]
                    tp = long_tp.iloc[init_set.last_index]

                leverage = lvrg_set.leverage

                #         get price, volume precision --> reatlime 가격 변동으로 인한 precision 변동 가능성       #
                try:
                    price_precision, quantity_precision = get_precision(init_set.symbol)
                except Exception as e:
                ...
                try:
                    request_client.change_initial_leverage(symbol=init_set.symbol, leverage=leverage)
                except Exception as e:
                ...
                while 1:  # <-- loop for complete open order
                    ...
                    try:
                        if ep_set.entry_type == OrderType.MARKET:
                            ...
                            #           market order            #
                            request_client.post_order(symbol=init_set.symbol, side=open_side, ordertype=OrderType.MARKET,
                            ...
                            request_client.post_order(timeInForce=TimeInForce.GTC, symbol=init_set.symbol,
                                                               side=open_side,
                                                               ordertype=ep_set.entry_type,
                        ...
                        if '-1111' in str(e):
                            code_1111 += 1
                            try:
                                _, quantity_precision = get_precision(init_set.symbol)
                                if code_1111 % 3 == 0:
                ...
                if ep_set.entry_type == OrderType.MARKET:
                    ...
                    time.sleep(60 - init_set.bar_close_second)
                else:
                    ...
                    while 1:
                        ...
                        if datetime.now().timestamp() > datetime.timestamp(res_df.index[-1]):
                            # if datetime.now().timestamp() + ep_set.entry_execution_wait > datetime.timestamp(res_df.index[-1]):
                            break

                        time.sleep(init_set.realtime_term)  # <-- for realtime price function

                #           1. when, open order time expired or executed              #
                #           2. regardless to position exist, cancel all open orders       #
                while 1:
                    try:
                        request_client.cancel_all_orders(symbol=init_set.symbol)
                        print()
                    except Exception as e:
            ...
            while 1:  # <-- for complete exec_open_quantity function
                try:
                    exec_open_quantity = get_remaining_quantity(init_set.symbol)
                except Exception as e:
            ...
            #       position / 체결량 이 존재하면 close order 진행      #
                ...
                if tp_set.static_tp and out_set.static_out:
                ...
                while 1:
                    ...
                    #        3. dynamic_out 도 있음
                    if not tp_set.static_tp or not out_set.static_out:
                        ...
                        if load_new_df2:    # dynamic_out & tp phase
                            ...
                            try:
                                ...
                                out = out_series.iloc[init_set.last_index]
                                tp = tp_series.iloc[init_set.last_index]
                        ...
                        if not tp_set.static_tp:

                            if tp_series.iloc[init_set.last_index] != tp_series.iloc[init_set.last_index - 1]:
                                ...
                                try:
                                    remained_tp_id = remaining_order_check(init_set.symbol)
                    ...
                    if tp_set.tp_type == OrderType.LIMIT:
                        ...
                        try:
                            result = partial_limit(init_set.symbol, tp_list, close_side, quantity_precision,
                                                   tp_set.partial_qty_divider)
                    ...
                    while 1:
                        ...
                        if tp_set.tp_type == OrderType.LIMIT:
                            try:
                                tp_remain_quantity = get_remaining_quantity(init_set.symbol)
                            except Exception as e:
                            ...
                            if tp_remain_quantity <= 1 / (10 ** quantity_precision):
                                ...
                                if not tp_set.static_tp:
                                    tp_exec_dict[res_df.index[-2]] = [tp_series.iloc[init_set.last_index - 1], tp]
                                else:
                        ...
                        try:
                            ...
                            if open_side == OrderSide.BUY:
                                ...
                                #       if tp_set.tp_type == OrderType.MARKET,
                                if new_df_['close'].iloc[init_set.last_index] >= tp:
                                    market_close_on = True
                                    log_tp = tp
                                    print("tp :", tp)
                    ...
                    #               market close phase - get market signal               #
                        ...
                        while 1:  # <--- This loop for out close & complete close
                            ...
                            if not remain_tp_canceled:
                                ...
                                try:
                                    remained_orderId = remaining_order_check(init_set.symbol)
                            ...
                            while 1:  # <-- loop for complete close order
                                try:
                                    ...
                                    if out_set.out_type != OrderType.MARKET:
                                        ...
                                        # request_client.post_order(timeInForce=TimeInForce.GTC, symbol=init_set.symbol,
                                        #                           side=close_side,
                                        #                           ordertype=out_set.out_type, stopPrice=str(stop_price),
                                        ...
                                        request_client.post_order(timeInForce=TimeInForce.GTC, symbol=init_set.symbol,
                                                                  side=close_side,
                                                                  ordertype=out_set.out_type,
                                                                  quantity=str(close_remain_quantity), price=str(exit_price),
                                                                  reduceOnly=True)

                                    else:
                                        #           market order            #
                                        request_client.post_order(symbol=init_set.symbol, side=close_side,
                                    ...
                                    out_set.out_type = OrderType.MARKET   # just 재시도
                                    continue
                            ...
                            if out_set.out_type != OrderType.MARKET:
                                ...
                                #       wait for bar ends      #
                                time.sleep(out_set.exit_execution_wait - datetime.now().second)
                                print("out_set.exit_execution_wait - datetime.now().second :",
                                      out_set.exit_execution_wait - datetime.now().second)
                            ...
                            if close_remain_quantity == 0.0:
                                ...
                                out_set.out_type = OrderType.MARKET
                                continue
                ...
                while 1:
                    ...
                    if datetime.now().timestamp() > datetime.timestamp(lastest_close_timeindex):
                        # try:
                        #     back_df, _ = concat_candlestick(init_set.symbol, init_set.interval, days=1, timesleep=0.2)
                        #
                ...
                for q_i, (k_ts, v_tp) in enumerate(sorted(tp_exec_dict.items(), key=lambda x: x[0], reverse=True)):
                    ...
                    if len(tp_exec_dict) == 1:
                        ...
                        if q_i != len(tp_exec_dict) - 1:
                            temp_qty = r_qty / tp_set.partial_qty_divider
                        else:
                            temp_qty = r_qty
                ...
                while 1:
                    try:
                        income = total_income(init_set.symbol, start_timestamp, end_timestamp)
