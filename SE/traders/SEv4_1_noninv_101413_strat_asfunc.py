        ...
        while 1:
            ...
            while 1:
                ...
                if mr_score_check:
                    ...
                    try:
                        ...
                        res_df = sync_check(new_df, third_df=new_df2, fourth_df=new_df3)
                    ...
                    try:
                        res_df = enlist_epouttp(res_df, config)
                        ...
                        mr_score_check = True  # res_df 에 data 가 덮여씌워질 것임
                        continue
                    ...
                    try:
                        ...
                        mr_score = mr_check(res_df, config)

                        # --------- score summation --------- #
                        if mr_score == config.ep_set.short_mr_score:
                            open_side = OrderSide.SELL  # inversion
                        elif mr_score == -config.ep_set.short_mr_score:
                    ...
                    mr_score_check = False   # once used, turn off const_check
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                if open_side == OrderSide.BUY:
                    ep = res_df['short_ep'].iloc[config.init_set.last_index]  # inversion mode adjusted
            ...
            #       position / 체결량 이 존재하면 close order 진행      #
                    ...
                    if not config.tp_set.static_tp or not config.out_set.static_out:
                        ...
                        if load_new_df2:    # dynamic_out & tp phase
                            ...
                            try:
                                ...
                                if open_side == OrderSide.BUY:
                                    out_series = res_df['short_out']   # inversion mode adjusted
                ...
                for q_i, (k_ts, v_tp) in enumerate(sorted(tp_exec_dict.items(), key=lambda x: x[0], reverse=True)):
                    ...
                    if open_side == OrderSide.SELL:
                        calc_tmp_profit += (ep / real_tp - config.init_set.fee - 1) * temp_qty
                        # calc_tmp_profit += ep / real_tp - config.init_set.fee
                    else:
                        calc_tmp_profit += (real_tp / ep - config.init_set.fee - 1) * temp_qty

                    print("real_tp :", real_tp)
                    print("ep :", ep)
                    print("config.init_set.fee :", config.init_set.fee)
