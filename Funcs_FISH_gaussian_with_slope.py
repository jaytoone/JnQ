...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    for i in range(1, len(df)):
        ...
        if df['Fisher_long_gaussian'].iloc[i] <= long_signal_value:
            ...
            # if df['Fisher_long_gaussian'].iloc[i] > df['Fisher_long_gaussian'].iloc[i - 1] and df['MA_slope'].iloc[i] >= slope_value:
                trade_state[i] = 1
        ...
        # elif df['Fisher_short_gaussian'].iloc[i] >= 1.0:
        #     if df['Fisher_short_gaussian'].iloc[i] < df['Fisher_short_gaussian'].iloc[i - 1]:
        #         trade_state[i] = 2

        # elif df['Fisher_short_gaussian'].iloc[i] < df['Fisher_short_gaussian'].iloc[i - 1]:
        #         trade_state[i] = 2
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        elif df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3:
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['TMTG', 'QBZ', 'STEEM', 'OMG', 'THETA', 'META', 'EL', 'HYC', 'XVG', 'ORBS']
    # TopCoin = ['QKC']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = -1.8, 2.5
