...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            # if df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3 or df.close.iloc[m] / bprelay.bprelay.iloc[m] <= 0.94:
                break
        ...
        # elif df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3 or df.close.iloc[m] / bprelay.bprelay.iloc[m] <= 0.94:
    ...
    if len(profits) == 0:
        ...
        plt.title('Fish revised - %.3f %.3f %.3f' % (float(profits.iloc[-1]), float(profits.iloc[-1]) / Minus_Profits, Minus_Profits))
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['APIX', 'TMTG', 'TRV', 'AMO', 'STEEM', 'DAC', 'ORBS', 'RNT', 'META', 'FCT']
    # TopCoin = ['APIX']
