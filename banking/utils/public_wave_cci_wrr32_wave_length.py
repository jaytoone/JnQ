...
def public_indi(res_df, config, np_timeidx, order_side="OPEN"):
    ...
    try:
        ...
        if itv_to_number(wave_itv1) > 1:
            ...
            """
            htf_df = get_roll_wave_data_v2(htf_df, valid_co_prime_idx, roll_co_idx_arr,
                                           'wave_high_fill_{}{}'.format(wave_itv1, wave_period1),
                                           roll_hl_cnt)
            cols += list(htf_df.columns[-roll_hl_cnt:])

            htf_df = get_roll_wave_data_v2(htf_df, valid_cu_prime_idx, roll_cu_idx_arr,
                                           'wave_low_fill_{}{}'.format(wave_itv1, wave_period1), roll_hl_cnt)
            cols += list(htf_df.columns[-roll_hl_cnt:])

            htf_df = wave_range_ratio_v4_2(htf_df, wave_itv1, wave_period1, roll_hl_cnt=roll_hl_cnt)
            cols += list(htf_df.columns[-4:])

            res_df = get_wave_length(res_df, valid_co_prime_idx, valid_cu_prime_idx, roll_co_idx_arr, roll_cu_idx_arr,
                                     wave_itv1, wave_period1, roll_hl_cnt=roll_hl_cnt)
            cols += list(htf_df.columns[-4:])

            # ------ 필요한 cols 만 join (htf's idx 정보는 ltf 와 sync. 가 맞지 않음 - join 불가함) ------ #
            res_df.drop(cols, inplace=True, axis=1, errors='ignore')
            res_df = res_df.join(to_lower_tf_v3(res_df, htf_df, cols, backing_i=0, ltf_itv='T').loc[res_df.index],
                                 how='inner')
            ...
            res_df = get_roll_wave_data_v2(res_df, valid_co_prime_idx, roll_co_idx_arr,
                                           'wave_high_fill_{}{}'.format(wave_itv1, wave_period1), roll_hl_cnt)
            res_df = get_roll_wave_data_v2(res_df, valid_cu_prime_idx, roll_cu_idx_arr,
                                           'wave_low_fill_{}{}'.format(wave_itv1, wave_period1), roll_hl_cnt)
    ...
    #                 # htf_df = wave_range_cci_v4_1(htf_df, wave_period2)
    ...
    #                 # ------ 필요한 cols 만 join (htf's idx 정보는 ltf 와 sync. 가 맞지 않음 - join 불가함) ------ #
    #                 res_df.drop(cols, inplace=True, axis=1, errors='ignore')
    #                 res_df = res_df.join(to_lower_tf_v3(res_df, htf_df, cols, backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능한 것 아닌가.
...
def ep_loc_p1_v3(res_df, config, np_timeidx, show_detail=True, ep_loc_side=OrderSide.SELL):
    ...
    if config.loc_set.zone1.use_zone:
        ...
        # ------------ outer_price ------------ #
        # ------ cci ------ #
        cci_ = res_df['cci_15T20'].to_numpy()
        # cci_ = res_df['cci_5T20'].to_numpy()
        # b1_cci_ = res_df['cci_15T20'].shift(15).to_numpy()
        # base_value = -100

        if ep_loc_side == OrderSide.SELL:
            mr_res *= cci_ < 0
            # mr_res *= cci_ < b1_cci_
            # mr_res *= (cci_ > base_value) & (cci_ < 0)
            # if show_detail:
            # sys_log.warning("cci_ < base_value : {:.5f} {:.5f} ({})".format(cci_[c_i], base_value, mr_res[c_i]))
        else:
            mr_res *= cci_ < 0
            # mr_res *= cci_ > b1_cci_
            # mr_res *= (cci_ > base_value) & (cci_ < 0)
...
def ep_loc_p2_v3(res_df, config, np_timeidx, show_detail=True, ep_loc_side=OrderSide.SELL):
    ...
    if config.loc_set.point2.csd_period != "None":
        ...
        if ep_loc_side == OrderSide.SELL:
            ...
            if show_detail:
                sys_log.warning(
                    "dc_upper_ <= csdbox : {:.5f} {:.5f} ({})".format(dc_upper_[c_i], csdbox[c_i], mr_res[c_i]))
        else:
            ...
            if show_detail:
                sys_log.warning(
                    "dc_lower_ >= csdbox : {:.5f} {:.5f} ({})".format(dc_lower_[c_i], csdbox[c_i], mr_res[c_i]))
