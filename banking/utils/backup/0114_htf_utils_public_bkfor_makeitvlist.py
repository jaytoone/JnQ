...
def sync_check(df_, row_list, order_side="OPEN"):
    ...
    df_5T_ = to_htf(df_, itv_='5T', offset='1h')
    df_15T_ = to_htf(df_, itv_='15T', offset='1h')
    df_30T_ = to_htf(df_, itv_='30T', offset='1h')
    df_4H_ = to_htf(df_, itv_='4H', offset='1h')
    # print("len(df_4H) :", len(df_4H))

    make_itv_list = ['5T', '15T', '30T', '4H']
    offset_list = ['1h', '1h', '1h', '1h']

    assert len(make_itv_list) == len(offset_list), "length of itv & offset_list should be equal"
    df_5T_, df_15T_, df_30T_, df_4H_ = [to_htf(df_, itv_=itv_, offset=offset_) for itv_, offset_ in zip(make_itv_list, offset_list)]

    # --------- slicing (in trader phase only) --------- #
    df = df_.iloc[-row_list[0]:].copy()
    df_5T = df_5T_.iloc[-row_list[1]:].copy()
    df_15T = df_15T_.iloc[-row_list[2]:].copy()
    df_30T = df_30T_.iloc[-row_list[2]:].copy()
    df_4H = df_4H_.iloc[-row_list[3]:].copy()
