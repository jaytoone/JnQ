...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
        ...
        for j in range(start_j, len(index_df_short)):
            ...
            if j == len(index_df_short) - 1:
                if index_df_short[j] <= index_df[i]:
                    df_modified.values[[i], :] = df_short.values[[j], :]

            if index_df_short[j - 1] <= index_df[i] < index_df_short[j]:
                df_modified.values[[i], :] = df_short.values[[j - 1], :]
                start_j = j

    # print('elapsed time :', time.time() - stime)
    df['Fisher_short'] = df_modified['Fisher_long']
    # print(df)
    # print(df_short.tail(10))
    # quit()
    ...
    for i in range(period, len(df)):
        ...
        y_smooth_short = gaussian_filter1d(y_short, sigma=sigma, mode='reflect')
    ...
    while m <= length:
        ...
        while True:
            ...
            if df.iloc[m]['low'] <= bprelay["bprelay"].iloc[m]:  # and (df.iloc[m]['high'] != df.iloc[m]['low']):  # 조건을 만족할 경우 spp 생성
                ...
                #   SINGAL 다음 시점에서 CLOSE와 매수가의 GAP이 0.5% 미만이면, CLOSE로 매수한다.
                if df['close'].iloc[m] / bp <= 1.005:
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            elif exit_signal == 2 and df['Fisher_long'].iloc[m] >= 0:
                exit_switch = True
                break
        ...
        #             break
        #
        #         if df['Fisher_long'].iloc[m] > short_signal_value:
        #             break
        # else:
        m += 1
        # if m > length:
    ...
    if len(profits) == 0:
        ...
        for trade_num in range(len(span_list_target)):
            plt.axvspan(span_list_target[trade_num][0], span_list_target[trade_num][1], facecolor='red', alpha=0.7)
...
if __name__ == "__main__":
    ...
    # TopCoin = pybithumb.get_top_coin(20)
    # print(TopCoin)
    # quit()
    TopCoin = ['XSR', 'MBL', 'MIX', 'TRV', 'COS', 'PLX', 'EL', 'AMO', 'FCT', 'ORBS', 'HDAC', 'PCM', 'BOA', 'SOC', 'ITC', 'ANKR', 'META', 'HYC', 'WPX', 'PIVX']
    TopCoin = ['BOA', 'SOC', 'ITC', 'ANKR', 'META', 'HYC', 'WPX', 'PIVX']
    # TopCoin = ['PLX', 'MIX', 'ZRX', 'COS']
    # TopCoin = ['XSR']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = -1.0, 2.5
