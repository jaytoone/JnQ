...
while True:
    ...
    while True:
        ...
        for Coin in TopCoin:
            ...
            if df['Fisher_long'].iloc[-2] <= long_signal_value:
                if df['Fisher_long'].iloc[-1] > df['Fisher_long'].iloc[-2] < df['Fisher_long'].iloc[-3]:
                        ...
                        if df['DC_TREND'].iloc[-1] == 'UP':
                            ...
                            while True:
                                ...
                                if df['CLOSE_DC_GAP'].iloc[go_to_prev_i] <= dc_accept_range:
                                    ...
                                    print('Fisher :', df['Fisher_long'].iloc[-1], df['Fisher_long'].iloc[-2],
                                          df['Fisher_long'].iloc[-3])
    ...
    while True:
        ...
        #               ADJUST COIN TO CHART DRIVER               #
        try:
            if get_coin_chart:
                close = System_Selenium_Funcs.get_coinly_data(driver, Coin, interval_key1)
                if close is None:
                    continue
                get_coin_chart = 0
        except Exception as e:
            print('Error in get_coinly_data :', e)
            continue
