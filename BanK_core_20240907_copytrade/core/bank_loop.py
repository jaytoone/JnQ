...
def init_table_trade(self):
    ...
    self.sys_log.debug(f"InitTableTrade : elasped time, replace_table account & trade (send={self.config.database.usage})  : %.4fs" % (time.time() - start_time)) 
    self.sys_log.debug("------------------------------------------------")
...
def loop_table_trade(self):
    ...
    if self.table_trade.empty:
        ...
        for idx, row in self.table_trade.iterrows(): # for save iteration, use copy().
            ...
            # preventing losing orderId, run in every loop.      
            self.sys_log.debug("------------------------------------------------")
            start_time = time.time()
