...
def low_high(reg_target, sigma, crop_size=0, realtime=0):
    ...
    if crop_size != 0:
        ...
        for i in range(none_indexs + crop_size, len(reg_target)):
            ...
            y = reg_target.values[none_indexs:i + 1] # == y[none_indexs + crop_size]부터 이전으로 crop_size만큼의 데이터를 smoothing한다.
            y_smooth = gaussian_filter1d(y, sigma=sigma)

            print(i)
            realtime_y_smooth[i] = y_smooth[-1]

            # 실시간 가우시안 라인에서 trade_state 정하는 방법
            if realtime == 1:
        ...
        # 실시간 가우시안 라인을 누적해서 trade_state 정하는 방법
        if realtime == 0:
...
if __name__ == "__main__":
    ...
    # TopCoin = ['BNP', 'LBA', 'DAC', 'ZIL', 'LAMB', 'WTC', 'INS', 'LUNA', 'HC', 'OGO', ]
    TopCoin = ['LUNA']
                ...
                for Coin in TopCoin:
                    ...
                    try:
                        print(Coin, profitage(Coin, 'MACD_OSC',
                                              short, long, signal, sigma=10, crop_size=100, realtime=0, Date=Date, get_fig=1, excel=0))
