...
import os
from Make_X_low_high_gaussian_acc import low_high
import warnings
        ...
        for Coin in TopCoin:
            try:
                ...
                if (datetime.now().minute % 5) in [0, 1, 2]:
                    X_test, buy_price, _, exit_price = low_high(Coin, input_data_length, crop_size=input_data_length)
                else:
                    X_test, buy_price, _, exit_price = low_high(Coin, input_data_length, ip_limit='on', crop_size=input_data_length)

                if X_test is not None:
                    X_test = np.array(X_test)
                    row = X_test.shape[1]
                    col = X_test.shape[2]
                    X_test = X_test.astype('float32').reshape(-1, row, col, 1)[:, :, [0, 1, 2, 3]]
        ...
        while True:
            ...
            try:
                ...
                if datetime.now().second == 55:
                    ...
                    while True:
                        try:
                            X_test, closeprice, _, _ = low_high(Coin, input_data_length,
                    ...
                    #       손절가 이하로 종가가 정해지면 시장가 매도하자       #
                    if closeprice <= exit_price:
                        exit_signal = 1
                    ...
                    #   매도 진행
                    if Y_pred[-1] == 2. or exit_signal == 1:
                        balance = bithumb.get_balance(Coin)
