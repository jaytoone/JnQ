...
os.chdir(pkg_path)
# sys_log.info("os.getcwd() : {}".format(os.getcwd()))
# quit()
            ...
            #       position / 체결량 이 존재하면 close order 진행      #
                ...
                while 1:
                    ...
                    if limit_tp:
                        ...
                        while 1:
                            try:
                                result = partial_limit(config.trader_set.symbol, tp_list, close_side, quantity_precision,
                    ...
                    while 1:
                        ...
                        try:
                            ...
                            if config.tp_set.static_tp and config.out_set.static_out:
                                ...
                                try:
                                    if load_new_df3:
                                        new_df_, _ = concat_candlestick(config.trader_set.symbol, config.trader_set.interval_list[0], days=1, limit=config.trader_set.row_list[0],
                                                                        timesleep=0.2)
                                        if datetime.timestamp(new_df_.index[-1]) < datetime.now().timestamp():
                                            continue
