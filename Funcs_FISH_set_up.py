...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    scaled_data = min_max_scaler(df.values[sum(df.MA.isna()):, [0, 1, 2, 3, 5]])
    slope_period = 20
    MA_slope = list()
    for n in range(slope_period, len(scaled_data)):
        MA_slope.append(stats.linregress([i for i in range(slope_period)],
                                         scaled_data[n + 1 - slope_period:n + 1, [-1]].reshape(-1, ))[0])

    MA_slope = [np.nan] * (len(df) - len(MA_slope)) + MA_slope
    ...
    for i in range(1, len(df)):
        ...
        #     if df['Fisher_long'].iloc[i] > df['Fisher_long'].iloc[i - 1] and df['MA_slope'].iloc[i] >= slope_value:
        #         trade_state[i] = 1
        ...
        # if df['Fisher_long_gaussian'].iloc[i] <= long_signal_value:
        #     trade_state[i] = 1

        elif df['Fisher_short'].iloc[i] >= 1.0:
            if df['Fisher_short'].iloc[i] < df['Fisher_short'].iloc[i - 1]:
                trade_state[i] = 2

        # elif df['Fisher_short'].iloc[i] < df['Fisher_short'].iloc[i - 1]:
        #     trade_state[i] = 2

        # elif df['Fisher_short'].iloc[i] >= short_signal_value:
        #     trade_state[i] = 2

        elif df['Fisher_long'].iloc[i] < long_signal_value:
            trade_state[i] = -1

        else:
            for high_passed_signal_value in [1.0]:
                if df['Fisher_short_gaussian'].iloc[i - 1] >= high_passed_signal_value > df['Fisher_short_gaussian'].iloc[i]:
                    trade_state[i] = 3
    ...
    #
    # plt.subplot(313)
    # plt.plot(df.Fisher_short.values)
    # plt.plot(df.Fisher_short_gaussian.values)
    #
    # for i in range(len(span_list)):
    #     plt.axvspan(span_list[i][0], span_list[i][1], facecolor='c', alpha=0.7)
    # for i in range(len(span_list2)):
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        if m > length:
            ...
            print('slope at %s : %s' % (start_m, df['MA_slope'].iloc[start_m]))
    ...
    if len(profits) == 0:
    ...
    elif (float(profits.iloc[-1]) != 1.0 and get_fig == 1) or get_fig != 0:
        ...
        plt.title('Fish gaussian with slope - %.3f %.3f %.3f' % (float(profits.iloc[-1]), float(profits.iloc[-1]) / Minus_Profits, Minus_Profits))
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['OMG', 'THETA', 'META', 'EL', 'HYC', 'XVG', 'ORBS']
    # TopCoin = ['QKC']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = -1, 2.5
