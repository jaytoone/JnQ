...
def low_high(Coin, input_data_length, ip_limit=None, trade_limit=None, crop_size=None, lowhigh_point=None):
    ...
    ohlcv_excel.iloc[:, :4] = max_abs_scaler(ohlcv_excel.iloc[:, :4])
    # print(ohlcv_excel.head())
    # quit()

    for i in range(1, len(ohlcv_excel)):
        #   이상의 고저폭을 가지는 양봉을 찾는다.
        #   코인의 전체적인 폭이 높은 경우는 어떤식으로 고려해주어야하나..
        #   급상승 폭의 기준 = ?
        if ohlcv_excel['close'].iloc[i] / ohlcv_excel['close'].iloc[i - 1] >= fluc_range:
            trade_state[i] = 1
        else:
            trade_state[i] = 0
    ...
    if len(ohlcv_data) != 0:
        ...
        for i in range(crop_size, len(ohlcv_data)):  # 마지막 데이터까지 다 긇어모은다.
            ...
            scaled_price = max_abs_scaler(price)
            # plt.subplot(212)
...
def made_x(file, input_data_length, model_num, get_fig, crop_size=None):
    ...
    if not file.endswith('.xlsx'):
        ohlcv_excel = pybithumb.get_ohlcv(file, 'KRW', 'minute30')
    else:
