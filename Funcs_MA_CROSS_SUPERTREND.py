...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    df['MA_LONG_OFFSET'] = df['MA_LONG'] + chart_height * 0.01
    ...
    df['MA_CO'] = np.where((df['MA_SHORT'].shift(1) < df['MA_LONG'].shift(1)) & (df['MA_SHORT'] >= df['MA_LONG']), 1, 0)

    df['SUPERTREND'] = supertrend(df, 100, 6)
    df['RED2GREEN'] = np.where((df['SUPERTREND'].shift(1) == df['Upper Band'].shift(1)) & (df['SUPERTREND'] == df['Lower Band']), 1, 0)
    ...
    for i in range(2, len(df)):
        ...
        if df['RED2GREEN'].iloc[i] == 1:
            copy_i = i
            while copy_i < len(df):
                if df['MA_CO'].iloc[copy_i] == 1:
                    trade_state[copy_i] = 1.5
                    print('entry tick gap at %s :' % copy_i, copy_i - i)
                    break
                copy_i += 1
                if copy_i - i > tick_gap:
                    break

        if df['MA_CO'].iloc[i] == 1:
            copy_i = i
            while copy_i < len(df):
                if df['RED2GREEN'].iloc[copy_i] == 1:
                    trade_state[copy_i] = 1.5
                    print('entry tick gap at %s :' % copy_i, copy_i - i)
                    break
                copy_i += 1
        ...
        # if df['MA_LONG'].iloc[i] > df['MA_SHORT'].iloc[i]:
        #     trade_state[i] = 2

        if df['SUPERTREND'].iloc[i - 1] == df['Lower Band'].iloc[i - 1]:
    ...
    df['BuyPrice'] = np.where(df['trade_state'] == 1.5, df['MA_LONG_OFFSET'], np.nan)
    ...
    while m <= length:
        ...
        while True:
            ...
            if df.iloc[m]['low'] <= bprelay["bprelay"].iloc[m]:  # and (df.iloc[m]['high'] != df.iloc[m]['low']):  # 조건을 만족할 경우 spp 생성
                ...
                #     break
                if df['SUPERTREND'].iloc[m] == df['Upper Band'].iloc[m]:
                    break
    ...
    # elif get_fig == 1 and float(profits.iloc[-1]) != 1:
        ...
        plt.plot(df['MA_LONG'])
        plt.plot(df['MA_LONG_OFFSET'], 'fuchsia')
...
if __name__ == "__main__":
    ...
    # quit()
    # TopC oin = pybithumb.get_tickers()
