...
#       Todo - 임시로 이곳에 놓음 --> 추후, funcs-indi. 로 옮길 예정       #
# def normalize_data(res_df, norm_cols, target_col, norm_period, i):
#     target_data = res_df[norm_cols].iloc[i + 1 - norm_period:i + 1].copy().to_numpy()
#     norm_range = target_data.max() - target_data.min()
#     target_diff = res_df[target_col].diff(norm_period).iloc[i].values
#     norm_res = target_diff / norm_range
#
#     #       OOM occurs..        #
#     # rolled_data = res_df[norm_cols].rolling(norm_period)
#     # norm_range = rolled_data.max().max(axis=1) - rolled_data.min().min(axis=1)
#     # target_diff = res_df[target_col].diff(norm_period)
#     # norm_res = target_diff.values / norm_range.values  # --> 여기서 터짐
#
#     return norm_res
...
def candle_game_short(res_df, config, i, prev_hclose_idx, mr_const_cnt, mr_score, show_detail):
    strat_version = config.strat_version
    if i < 0 or (i >= 0 and prev_hclose_idx >= 0):  # trader or colab env.
        ...
        if config.loc_set.point.body_score != "None":
            ...
            const_bool_ = eval(const_)
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, h_body_score, config.loc_set.point.body_score,
                                             show_detail)

        if config.loc_set.point.back_wick_score != "None":
            const_ = "h_back_wick_score >= config.loc_set.point.back_wick_score"
            const_bool_ = eval(const_)
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, h_back_wick_score, config.loc_set.point.back_wick_score,
                                             show_detail)

    return mr_const_cnt, mr_score


def candle_game_long(res_df, config, i, prev_hclose_idx, mr_const_cnt, mr_score, show_detail):
    strat_version = config.strat_version
...
def short_ep_loc(res_df, config, i, np_timeidx, show_detail=True):
    ...
    if config.loc_set.point.front_wick_score != "None":
        ...
        if strat_version in ['v5_2', '1_1']:
            ...
            const_ = "front_wick_score <= -config.loc_set.point.front_wick_score"
            const_bool_ = eval(const_)
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, front_wick_score,
                                             -config.loc_set.point.front_wick_score, show_detail)

    # ------ candle_score_v1 (previous)  ------ #
    wick_score_arr = np.array([config.loc_set.point.front_wick_score, config.loc_set.point.body_score, config.loc_set.point.back_wick_score])
    if np.sum(wick_score_arr == "None") != wick_score_arr.size:
        prev_hclose_idx = i - (np_timeidx[i] % config.loc_set.zone.c_itv_ticks + 1)
        mr_const_cnt, mr_score = candle_game_short(res_df, config, i, prev_hclose_idx, mr_const_cnt, mr_score, show_detail)
    ...
    wick_score2_arr = np.array([config.loc_set.point.front_wick_score2, config.loc_set.point.body_score2, config.loc_set.point.back_wick_score2])
    if np.sum(wick_score2_arr == "None") != wick_score2_arr.size:

        # => future_data warning 으로 아래와같이 변경함
        shared_ticks = np_timeidx[i] % config.loc_set.zone.c_itv_ticks + 1
        ...
        if config.loc_set.point.front_wick_score2 != "None":
            const_ = "h_front_wick_score <= -config.loc_set.point.front_wick_score2"
            const_bool_ = eval(const_)
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, h_front_wick_score, -config.loc_set.point.front_wick_score2,
                                             show_detail)

        if config.loc_set.point.body_score2 != "None":
            const_ = "h_body_score >= config.loc_set.point.body_score2"
            const_bool_ = eval(const_)
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, h_body_score, config.loc_set.point.body_score2, show_detail)

        if config.loc_set.point.back_wick_score2 != "None":
            const_ = "h_back_wick_score >= config.loc_set.point.back_wick_score2"
            const_bool_ = eval(const_)
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, h_back_wick_score, config.loc_set.point.back_wick_score2,
                                             show_detail)
    ...
    if config.loc_set.zone.zone_rejection:
        ...
        if strat_version in ['3']:
            close = res_df['hopen_H'].iloc[i]
            ema_1m = res_df['ema_1m'].iloc[i]

            norm_cols = ['open', 'high', 'low', 'close', 'ema_1m']
            target_col = ['ema_1m']
            norm_period = 120
            if i < 0 or (i > norm_period):  # considering ide & trader platform
                # norm_res = normalize_data(res_df, norm_cols, target_col, norm_period, i)
            ...
            # const_ = "close < ema_1m"
            # const_bool_ = eval(const_)
            # mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, close, ema_1m, show_detail)
...
def long_ep_loc(res_df, config, i, np_timeidx, show_detail=True):
    ...
    if config.loc_set.point.front_wick_score != "None":
        ...
        if strat_version in ['v5_2', '1_1']:
            ...
            const_ = "front_wick_score >= config.loc_set.point.front_wick_score"
            const_bool_ = eval(const_)
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, const_bool_, front_wick_score, config.loc_set.point.front_wick_score,
                                             show_detail)

    wick_score_arr = np.array([config.loc_set.point.front_wick_score, config.loc_set.point.body_score, config.loc_set.point.back_wick_score])
    if np.sum(wick_score_arr == "None") != wick_score_arr.size:
        # prev_hclose_idx = i - (np_timeidx[i] % config.loc_set.zone.c_itv_ticks + 1)
        prev_hclose_idx_list = [i - ((np_timeidx[i] % config.loc_set.zone.c_itv_ticks) * (nbar_ + 1) + 1) for nbar_ in range(len(wick_score_arr))]
        for prev_hclose_idx in prev_hclose_idx_list:    # Todo - recursive on mr_const & score
            mr_const_cnt, mr_score = candle_game_long(res_df, config, i, prev_hclose_idx, mr_const_cnt, mr_score, show_detail)

    # ------ candle_score_v2 (current) ------ #
    # wick_score1 과 동시적용 기능을 위해 병렬 구성
    wick_score2_arr = np.array([config.loc_set.point.front_wick_score2, config.loc_set.point.body_score2, config.loc_set.point.back_wick_score2])
    if np.sum(wick_score2_arr == "None") != wick_score2_arr.size:
