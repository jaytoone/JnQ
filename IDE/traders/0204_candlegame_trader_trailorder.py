...
from funcs_binance.funcs_order_logger_trailorder import limit_order, partial_limit_v2, cancel_order_list, market_close_order
                    ...
                    while 1:
                        ...
                        if first_exec_qty_check or time.time() - check_time >= self.config.trader_set.qty_check_term:
                            try:
                                ...
                                sys_log.error('error in get open_executedQty : {}'.format(e))
                                time.sleep(self.config.trader_set.api_retry_term)
                                continue
                            else:
                            ...
                            if abs(open_executedQty / open_quantity) >= self.config.trader_set.breakout_qty_ratio:
                                break

                # ------ 1. when, open order time expired or executed ------ #
                #               a. regardless to position exist, cancel open orders       #
            ...
            if open_executedQty == 0.0:
                ...
                while 1:
                    ...
                    if limit_tp:
                        ...
                        while 1:
                            try:
                                post_order_res_list = partial_limit_v2(self, self.config, tp_list, close_side,
                                                                       open_executedQty - tp_executedQty, quantity_precision,
                                                                       self.config.tp_set.partial_qty_divider)
                            except Exception as e:
                                sys_log.error("error in partial_limit_v2 : {}".format(e))
                                time.sleep(self.config.trader_set.api_retry_term)
                                continue
                            else:
                    ...
                    while 1:
                        ...
                        if self.config.tp_set.tp_type == OrderType.LIMIT or self.config.tp_set.tp_type == "BOTH":
                            order_exec_check_list = [check_exec_by_order_info(self.config.trader_set.symbol, post_order_res, quantity_precision)
                                                     for post_order_res in post_order_res_list if post_order_res is not None]
                            #        if tp_remain_quantity < 1 / (10 ** quantity_precision): 로 변경되어야할 것
                ...
                sys_log.info('temporary profit : {:.2%} ({:.2%}) %%'.format(tmp_profit, (calc_tmp_profit - 1) * leverage))
