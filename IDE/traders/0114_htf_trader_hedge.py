...
from funcs_binance.funcs_trader_modules_0114_htf_flexioneline import read_write_cfg_list, get_income_info_v2, calc_ideal_profit_v2, \
    ...
    get_eptpout, check_ei_k_onbarclose, init_set, get_open_side
from funcs_binance.funcs_order_logger_hedge_0114_htf import limit_order, partial_limit_order_v2, cancel_order_list, market_close_order
    ...
    def run(self):
        ...
        while 1:
            ...
            while 1:
                ...
                if open_side is not None:
                    ...
                    if strat_version in ['v5_2']:
                        ...
                        ep_out = check_ei_k_onbarclose(self, res_df_init, res_df, open_side, e_j, tp_j)
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                post_order_res, self.over_balance, res_code = limit_order(self, self.config.ep_set.entry_type, open_side, pos_side, ep,
            ...
            if open_executedQty == 0.0:  # open_executedQty 는 분명 정의됨
                ...
                while 1:
                        ...
                        if len(post_order_res_list) == 0:  # 아무 limit_tp 진행하지 않은 상태
                            ...
                            if not self.config.tp_set.static_tp:    # np.nan != np.nan
                                if tp_series.iloc[self.config.trader_set.complete_index] != \
                                    ...
                                    tp_executedPrice_list, tp_executedQty = cancel_order_list(self.config.trader_set.symbol, post_order_res_list)
                                    limit_tp = 1
                    ...
                    if limit_tp:
                        ...
                        tp_list = [tp]
                        try:    # currently, allow only partial_tp
                            tp_list, tp_exec_dict = log_sub_tp_exec(self, res_df, tp_list, post_order_res_list, quantity_precision, tp_series_list,
                                                                    tp_exec_dict)
                        except:
                            pass
                        ...
                        while 1:
                            try:
                                post_order_res_list = partial_limit_order_v2(self, self.config, tp_list, close_side, pos_side,
                    ...
                    while 1:
                        ...
                        all_executed, close_executedPrice_list = check_limit_tp_exec(self, post_order_res_list, quantity_precision, return_price=True)
                        if all_executed:
                        ...
                        try:
                            ...
                            market_close_on, log_tp = check_out(self, res_df, market_close_on, log_tp, open_side, out)
                            if not market_close_on:  # log_tp 갱신 방지
                                market_close_on, log_tp, cross_on = check_market_tp(self, res_df, market_close_on, log_tp, cross_on, open_side, tp)
                            else:
                    ...
                    if not market_close_on:  # = load_new_df2
                        ...
                        # ------ market close phase ------ #
                        fee += self.config.trader_set.market_fee
                        remain_tp_canceled = False  # 미체결 limit tp 존재가능함    # Todo : 추후, 없애는거 고려 - 현재 큰 의미없어보임
                        close_executedPrice_list = market_close_order(self, remain_tp_canceled,
                                                                      post_order_res_list, close_side, pos_side, open_executedQty, out, log_tp)
                ...
                # ------------ calc_ideal_profit ------------ #
                #       Todo
                #        1. limit_tp 와 market_close close_executedPrice_list 의 합산 여부 결정해야함
                #           a. 현재 독립적으로 정의됨 - 추후 tp & market_close 중복시 문제될 것
                #        2. 현재, partial_tp -> market_close 미지원
                ideal_profit, real_profit, trade_log = calc_ideal_profit_v2(self, open_side, ep, tp_exec_dict, open_executedPrice_list,
                                                                            close_executedPrice_list, res_df, fee, trade_log)
                with open(trade_log_fullpath, "wb") as dict_f:
                    pickle.dump(trade_log, dict_f)
