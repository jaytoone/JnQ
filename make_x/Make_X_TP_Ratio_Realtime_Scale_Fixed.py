...
def made_x(file, input_data_length, scale_window_size):
    ...
    if len(ohlcv_data) != 0:
        ...
        for i in range(scale_window_size, len(ohlcv_data)):  # 마지막 데이터까지 다 긇어모은다.
            ...
            #           Check min max variation         #
            current_min, current_max = np.min(group_x[:, :18]), np.max(group_x[:, :18])
            # print('current_min, current_max :', current_min, current_max)
...
if __name__ == '__main__':
    ...
    for file in ohlcv_list:
        ...
        model_num += 1
        try:
            result = made_x(file, input_data_length, scale_window_size)
            # print(result)
            # quit()
