...
def low_high(Coin, input_data_length, ip_limit=None, trade_limit=None, crop_size=None, lowhigh_point=None):
    ...
    if len(ohlcv_data) != 0:
        ...
        for i in range(crop_size, len(ohlcv_data)):  # 마지막 데이터까지 다 긇어모은다.
            ...
            x = np.concatenate(
                (scaled_pricema[:, :4], volume, scaled_pricema[:, 4:], CMO, scaled_OBV, RSI,
                 MACD), axis=1)
            #          데이터 전처리         #
            #   Fixed X_data    #
...
def made_x(file, input_data_length, model_num, check_span, get_fig, crop_size=None, sudden_death=0):
    ...
    for i in range(check_span, len(ohlcv_excel)):
        ...
        if ohlcv_excel['close'][i - check_span:i].min() >= ohlcv_excel['close'][i]:
            # if ohlcv_excel['close'][i - check_span:i + 1].value_counts().sort_index().iloc[0] <= 3:
            trade_state[i] = 1
            # else:
            #     trade_state[i] = 0
        #   앞으로 올라갈 것으로 예상
    ...
    # plt.plot(ohlcv_excel['close'].values, 'gold')
    # span_list = list()
    # span_list2 = list()
    ...
    if len(ohlcv_data) != 0:
        ...
        if get_fig == 1:
            ...
            plt.subplot(313)
            plt.plot(max_abs_scaler(ohlcv_data[:, [-3]]), 'g', label='osc')
            # plt.plot(max_abs_scaler(ohlcv_data[:, [-5]]), 'r', label='macd')
            plt.plot(ohlcv_data[:, [-2]], 'g', label='zero')
            plt.legend(loc='upper right')
