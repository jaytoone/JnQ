...
def made_x(file, input_data_length, model_num, check_span, get_fig, crop_size=None, sudden_death=0):
    ...
    for i in range(len(ohlcv_excel) - check_span):
        #   저점
        if ohlcv_excel['MA20'][i - check_span:i].min() >= ohlcv_excel['MA20'][i]:
            if ohlcv_excel['MA20'][i + 1:i + 1 + check_span].min() >= ohlcv_excel['MA20'][i]:
                # if ohlcv_excel['MA20'][i - check_span:i + 1 + check_span].value_counts().sort_index().iloc[0] <= 3:
                if 1 not in trade_state[i - check_span:i]:
                    trade_state[i] = 1
                else:
                    trade_state[i] = 0
                ...
                trade_state[i] = 0
        #   고점
        elif ohlcv_excel['MA20'][i - check_span:i].max() <= ohlcv_excel['MA20'][i]:
            if ohlcv_excel['MA20'][i + 1:i + 1 + check_span].max() <= ohlcv_excel['MA20'][i]:
                # if ohlcv_excel['MA20'][i - check_span:i + 1 + check_span].value_counts().sort_index().iloc[-1] <= 3:
                if 2 not in trade_state[i - check_span:i]:
                    trade_state[i] = 2
                else:
                    trade_state[i] = 0
...
if __name__ == '__main__':
    ...
    # ohlcv_list = ['2020-01-11 ETH ohlcv.xlsx']
