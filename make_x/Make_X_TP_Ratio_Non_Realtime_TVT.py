...
def made_x(file, input_data_length, scale_window_size, data_amount, label_type='Test'):
    ...
    if len(ohlcv_data) != 0:
        ...
        #           Global Scaling (= Non-Realtime)          #

        #           OHLC EMA ST Up & Down ST         #
        ohlcv_data[:, :24] = min_max_scaler(ohlcv_data[:, :24])
        #           Just for Tester_Scaling         #
        # ohlcv_data[:, :4] = min_max_scaler(ohlcv_data[:, :4])
        # ohlcv_data[:, 4:18] = min_max_scaler(ohlcv_data[:, 4:18])
...
if __name__ == '__main__':
    ...
    from Funcs_for_TP_Ratio_TVT_Bias_Binarize_for_nonReal import profitage
