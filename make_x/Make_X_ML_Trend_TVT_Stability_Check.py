...
if __name__ == '__main__':
    ...
    from funcs.funcs_for_pb_tp_ratio_plotly_idea import profitage
    import os
    from funcs.funcs_for_trade import to_timestamp
    ...
    for i in range(date_iter):

        if i:
            endTime -= a_day / 1000 * days
            end_date = str(datetime.fromtimestamp(endTime)).split(' ')[0]
        else:
            endTime = to_timestamp(pd.to_datetime(end_date))
