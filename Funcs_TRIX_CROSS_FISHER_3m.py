...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    df['TRIX'] = TA.TRIX(df, period=18) + 0.005
    df['TRIX'] = np.where(np.isnan(df['TRIX']), 0, df['TRIX'])

    df['TRIX_ORIGIN'] = TA.TRIX(df, period=18)
    df['TRIX_ORIGIN'] = np.where(np.isnan(df['TRIX_ORIGIN']), 0, df['TRIX_ORIGIN'])
    df['TRIX_TREND'] = np.where(df['TRIX_ORIGIN'].shift(1) <= df['TRIX_ORIGIN'], 'UP', 'DOWN')
    ...
    for i in range(2, len(df)):
        ...
        if df['TRIX_LATER'].iloc[i] <= long_signal_value < df['TRIX_LATER'].iloc[i - 1]: #and df['close'].iloc[i] > clearance(df['SMMA'].iloc[i] * smma_accept_range):
            for j in range(i + 1, len(df)):
                #       FIND TRIX CROSS     #
                if df['TRIX'].iloc[j] >= df['TRIX_LATER'].iloc[j] and df['TRIX'].iloc[j - 1] < df['TRIX_LATER'].iloc[j - 1]:
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            #       매수 시그널의 TRIX_GAUSSIAN 값보다 작아지면 손절        #
            if m - start_m > trix_horizontal_ticks \
                    and df['TRIX'].iloc[m] - df['TRIX'].iloc[buy_signal_m] < -0.01:
                exit_switch = True
                break
    ...
    if len(profits) == 0:
        ...
        plt.title('TRIX CROSS FISHER - %s %.3f %.3f %.3f' % (Coin,
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['TRV', 'APIX', 'XSR', 'THETA', 'LRC', 'DVP', 'FCT']
    # TopCoin = ['ZIL', 'VET', 'HYC', 'FX', 'REP', 'STRAT']
