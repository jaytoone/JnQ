...
def low_high(reg_target, sigma, crop_size=0, realtime=0):
    ...
    if crop_size != 0:
        ...
        for i in range(none_indexs + crop_size, len(reg_target)):
            ...
            #     y_smooth = gaussian_filter1d(y_smooth, sigma=sigma, mode='reflect')

            print(i)
            realtime_y_smooth[i] = np.convolve(y_smooth, window, mode='valid')[-1]
        ...
        if realtime == 0:
            for i in range(1, len(realtime_y_smooth)):
                ...
                if realtime_y_smooth[i - 1] < realtime_y_smooth[i]:
                    trade_state[i] = 'up'
                elif realtime_y_smooth[i - 1] >= realtime_y_smooth[i]:
                    trade_state[i] = 'down'

        return realtime_y_smooth, trade_state
...
def profitage(Coin, reg_target, short, long, signal, sigma, crop_size, realtime, Date='2019-09-25', excel=0, get_fig=0):
    ...
    # print(df[reg_target])
    # quit()
    osc_reg, trade_state = low_high(df[reg_target], sigma, crop_size, realtime=realtime)
    ...
    if len(profits) == 0:
        ...
        plt.subplot(312)
        plt.plot(df['osc_reg'], 'red', label='oscillator', linewidth=1.0)
        ...
        plt.subplot(313)
        osc_reg, _ = low_high(df[reg_target], sigma=10)
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['MXC', 'CHR', 'WOM', 'TMTG', 'ENJ', 'QBZ', 'STRAT', 'CTXC', 'XLM', 'WET']
    # TopCoin = ['CHR']
