...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    if chart_gap < 1.10:
        print('chart_gap dissatified : %.3f' % chart_gap)
        return
    ...
    df['FISHER_LONG'] = TA.FISH(df, period=60)
    ...
    df['STOCH_RSI'] = stochrsi(df, period_rsi=14, period_sto=60, k=3, d=10)
    ...
    for i in range(2, len(df)):
        ...
        if df['MA_SHORT'].iloc[i] > df['MA_LONG'].iloc[i] > df['MA_SHORT'].iloc[i - 1]:
            trade_state[i] = 1.5
...
if __name__ == "__main__":
    ...
    # TopCoin = ['MXC', 'ARPA', 'ADA', 'ETH', 'XRP', 'BSV', 'ETC', 'BCH', 'BTG', 'DASH']
