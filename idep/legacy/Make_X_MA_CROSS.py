...
def made_x(file, input_data_length, model_num):
    ...
    if len(ohlcv_data) != 0:
        ...
        #   MACD
        ohlcv_data[:, [8, 9, 10]] = max_abs_scaler(ohlcv_data[:, [8, 9, 10]])
        ...
        for i in range(input_data_length, len(ohlcv_data)):  # 마지막 데이터까지 다 긇어모은다.
            ...
            group_x = ohlcv_data[i + 1 - input_data_length: i + 1, :-1]

            group_x[:, [0, 1, 2, 3, 4, 5, 6]] = min_max_scaler(group_x[:, [0, 1, 2, 3, 4, 5, 6]])
