...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    # df['MA_SHORT_OFFSET'] = df['MA_SHORT'] - chart_height * 0.03
    ...
    df['STOCH_RSI'] = stochrsi(df, period_rsi=14, period_sto=30, k=3, d=10)
    # df['STOCH_RSI'] = np.where(np.isnan(df['STOCH_RSI']), 50, df['STOCH_RSI'])
    df['STOCH_RSI_TREND'] = np.where(df['STOCH_RSI'] > df['STOCH_RSI'].shift(1), 'UP', 'DOWN')

    df['SUPERTREND'] = supertrend(df, 100, 6)
    df['ICHIMOKU'] = TA.ICHIMOKU(df, 9, 60, 52, 26)

    # print(df['STOCH_RSI'].head())
    # quit()
    ...
    for i in range(2, len(df)):
        ...
        # if df['STOCH_RSI'].iloc[i - 1] <= long_signal_value:
        #     if df['STOCH_RSI_TREND'].iloc[i] == 'UP' and df['STOCH_RSI_TREND'].iloc[i - 1] == 'DOWN':
        #         if df['close'].iloc[i] >= df['MA_SHORT'].iloc[i] >= df['MA_LONG'].iloc[i]:
        #             if df['MA_SHORT_SLOPE'].iloc[i] > 0:
        #                 trade_state[i] = 1.5
        ...
        # if df['STOCH_RSI'].iloc[i] > long_signal_value > df['STOCH_RSI'].iloc[i - 1]:
        #     trade_state[i] = 1.5

        if df['STOCH_RSI'].iloc[i - 1] >= short_signal_value:
        ...
        # if df['STOCH_RSI'].iloc[i] < long_signal_value:
        #     trade_state[i] = 2
    ...
    for i in range(1, len(target)):
        ...
        elif df['STOCH_RSI_TREND'].iloc[i] == 'UP' and df['STOCH_RSI_TREND'].iloc[i - 1] == 'DOWN':
    ...
    # elif get_fig == 1 and float(profits.iloc[-1]) != 1:
        ...
        plt.plot(df['MA_SHORT'], color='gold')
        # plt.plot(df['MA_SHORT_OFFSET'], color='orange')
        plt.plot(df['MA_LONG'])
    ...
    # plt.legend(loc='upper right', fontsize=5)
        ...
        plt.subplot(212)
        plt.plot(df['STOCH_RSI'].fillna(50))
...
if __name__ == "__main__":
    ...
    TopCoin = ['APIX', 'DAC', 'INS', 'COS', 'QKC', 'CHR', 'SXP', 'KNC', 'MIX', 'FNB']
    TopCoin = [ 'QKC', 'CHR', 'SXP', 'KNC', 'MIX', 'FNB']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = 20, 80
