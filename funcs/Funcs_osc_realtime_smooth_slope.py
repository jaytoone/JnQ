...
def low_high(reg_target, sigma, crop_size=0, realtime=0):
    ...
    if crop_size != 0:
        ...
        return gaussian_target, realtime_y_smooth, trade_state
...
def profitage(Coin, reg_target, short, long, signal, sigma, crop_size, realtime, Date='2019-09-25', excel=0, get_fig=0):
    ...
    period = 3
    for i in range(period - 1, len(df)):
        df['osc_slope'].iloc[i], intercept, r_value, p_value, stderr = stats.linregress([x for x in range(period)], df.osc_reg[i + 1 - period:i + 1])

    # print(df.osc_slope.max())
    # quit()
    for i in range(len(df)):
        if df['osc_slope'].iloc[i] <= 0:
    ...
    if len(profits) == 0:
        ...
        plt.subplot(312)
        plt.plot(max_abs_scaler(df['osc_reg']), 'orange', label='gaussian_0')
        plt.plot(max_abs_scaler(df['osc_slope']), 'fuchsia', label='gaussian_1')
        plt.xlim(0, len(ochl))
