...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    if len(profits) == 0:
        ...
        plt.title('Fish gaussian - %.3f %.3f %.3f' % (float(profits.iloc[-1]), float(profits.iloc[-1]) / Minus_Profits, Minus_Profits))
        for trade_num in range(len(span_list_target)):
            plt.axvspan(span_list_target[trade_num][0], span_list_target[trade_num][1], facecolor='gold', alpha=0.7)
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['COS', 'XSR', 'THETA', 'AMO', 'TMTG', 'RNT', 'CHR', 'MIX', 'AOA', 'WAXP']
    # TopCoin = ['WPX', 'FCT', 'RNT', 'THETA', 'BHP', 'PLX', 'BASIC', 'PCM']
    # TopCoin = ['BASIC', 'PCM']
    TopCoin = ['AOA']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_period, short_period = 30, 30

                #       Make folder      #
                try:
                    os.mkdir("./Fisher_result/%s_%s_%s_%s/" % (long_signal_value, short_signal_value, long_period, short_period))
