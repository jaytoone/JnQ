...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    for n in range(slope_period, len(scaled_data)):
        MA_slope.append(stats.linregress([i for i in range(slope_period)], scaled_data[n + 1 - slope_period:n + 1, [-1]].reshape(-1, ))[0])
    ...
    for i in range(1, len(df)):
        ...
        # if (df['Fisher_long'].iloc[i] <= long_signal_value) and df['MA_slope'].iloc[i] >= slope_value:
        #     trade_state[i] = 1
    ...
    while m <= length:
        ...
        # print('slope at %s :' % m, df['MA_slope'].iloc[m] * 1000)
    ...
    if len(profits) == 0:
        ...
        plt.title('Fish revised with slope - %.3f %.3f %.3f' % (float(profits.iloc[-1]), float(profits.iloc[-1]) / Minus_Profits, Minus_Profits))
