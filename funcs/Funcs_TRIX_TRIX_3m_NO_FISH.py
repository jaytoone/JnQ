...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    df['STOCH_RSI'] = TA.STOCHRSI(df, rsi_period=15, stoch_period=30)
    df['STOCH_RSI'] = df['STOCH_RSI'].rolling(15).mean()
    # df['STOCH_RSI'] = df['STOCH_RSI'].rolling(10).mean()
    # df['STOCH_RSI'] = df['STOCH_RSI'].rolling(15).mean()
    df['STOCH_RSI'] = np.where(np.isnan(df['STOCH_RSI']), 0, df['STOCH_RSI'])
    df['STOCH_RSI_TREND'] = np.where(df['STOCH_RSI'].shift(1) <= df['STOCH_RSI'], 'UP', 'DOWN')
    ...
    #     y_short = df['TRIX_Origin'].values[i + 1 - period:i + 1]
    ...
    for i in range(2, len(df)):
        ...
        #       TRIX LONG      #
        # if df['TRIX_TREND'].iloc[i - 1] == 'DOWN' and df['TRIX_TREND'].iloc[i] == 'UP' and 20 > df['TRIX'].iloc[i] >= -15:
        #     trade_state[i] = 1.5
    ...
    span_list_target_exit = list()
    target = df['trade_state']
    for i in range(1, len(target)):
        if df['TRIX_TREND'].iloc[i] == 'DOWN' and df['TRIX_TREND'].iloc[i - 1] == 'UP':
            span_list_target.append((i, i + 1))
        if target.iloc[i] == 1.5:
            span_list_target_ic.append((i, i + 1))
        elif df['TRIX_TREND'].iloc[i] == 'UP' and df['TRIX_TREND'].iloc[i - 1] == 'DOWN':
            span_list_target_entry.append((i, i + 1))
        if df['STOCH_RSI_TREND'].iloc[i] == 'DOWN' and df['STOCH_RSI_TREND'].iloc[i - 1] == 'UP':
            span_list_target_exit.append((i, i + 1))
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            if df.trade_state.iloc[m] == 2:
                ...
                if (df['TRIX'].iloc[m] - df['TRIX'].iloc[m - 1]) / (df['TRIX'].max() - df['TRIX'].min()) >= trix_exit_previous_gap:
                ...
                elif m - start_m > trix_horizontal_ticks:
                    #       자잘거림 보정     #
                    #       이전 변곡점과의 상하좌우 폭 비교      #
                    copy_m = m
                    while True:
                    ...
                    if abs(df['TRIX'].iloc[m] - df['TRIX'].iloc[copy_m]) / (df['TRIX'].max() - df['TRIX'].min()) < trix_vertical_gap:
                        pass
                    else:
                        sell_switch = True
                        break
    ...
    if len(profits) == 0:
        ...
        for trade_num in range(len(span_list_target_exit)):
            plt.axvspan(span_list_target_exit[trade_num][0], span_list_target_exit[trade_num][1], facecolor='black', alpha=0.5)
        ...
        #     plt.axvspan(span_list_target_fisher[trade_num][0], span_list_target_fisher[trade_num][1], facecolor='red',
        #                 alpha=0.5)
        # for trade_num in range(len(spanlist_limit)):
        #     plt.axvspan(spanlist_limit[trade_num][0], spanlist_limit[trade_num][1], facecolor='c', alpha=0.5)
        # for trade_num in range(len(spanlist_breakaway)):
