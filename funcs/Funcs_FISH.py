...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    for i in range(len(df)):
        ...
        # elif df['Fisher_short'].iloc[i - 1] >= high_passed_signal_value > df['Fisher_short'].iloc[i]:
        #     trade_state[i] = 3
        #
        # elif df['Fisher_short'].iloc[i - 1] >= high_passed_signal_value_2 > df['Fisher_short'].iloc[i]:
        #     trade_state[i] = 3
    ...
    if len(profits) == 0:
        ...
        plt.title('Fish - %.3f %.3f %.3f' % (float(profits.iloc[-1]), float(profits.iloc[-1]) / Minus_Profits, Minus_Profits))
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['HDAC', 'ANKR', 'BOA', 'KNC', 'ZIL', 'REP', 'SNT', 'CHR', 'LBA', 'BCD']
    # TopCoin = ['COS']
