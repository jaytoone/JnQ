from funcs.upbit.futures_module_v4 import UpbitModule
...
class ShieldModule(UpbitModule):

    """
    Shield 의 정상적인 동작을 위한 각 phase 의 method 가 모인 Class
    v4_1 -> v4_2
        1. 수정 필요 method / 아닌 method 구분.
        2. limit_order_v2 --> post_order.
    """

    def __init__(self, config):
        api_key, secret_key = self.load_key(r"D:\Projects\System_Trading\JnQ\Bank_upbit\api_keys\upbit_key.pkl")
        UpbitModule.__init__(self, config, api_key=api_key, secret_key=secret_key)
    ...
    def check_open_exec_qty_v2(self, post_order_res, open_quantity):
        ...
        order_info = self.get_order_info(post_order_res['market'], post_order_res['uuid'])
        open_exec = (abs(self.get_exec_qty(order_info) / open_quantity) >= self.config.trader_set.open_exec_qty_ratio) or order_info['state'] == 'done'
        return open_exec
    ...
    def get_partial_tp_qty(self, ep, tp, open_exec_qty, price_precision, quantity_precision, close_side):
        ...
        """
        거래소별, 수정 필요 method
            1. self.calc_with_precision --> self.get_tick_size
        """
        ...
        partial_tps = list(map(lambda x: self.get_tick_size(x), partial_tps))
    ...
    def check_close_limit_exec_v2(self, post_order_res_list, quantity_precision):
    ...
    def post_order(self, code, order_type, order_side, pos_side, order_price, order_quantity, order_data=None):
        ...
        order_quantity_org = order_quantity
        ...
        while 1:
            try:
                ...
                if order_type in OrderType.MARKET:
                    # a. market order
                    if order_side in OrderSide.BUY:
                        post_order_res = self.buy_market_order(code, order_quantity * order_price)
                    else:
                        post_order_res = self.sell_market_order(code, order_quantity)
                    ...
                    if order_side == OrderSide.BUY:
                        post_order_res = self.buy_limit_order(code, order_price, order_quantity)
                    else:
                        post_order_res = self.sell_limit_order(code, order_price, order_quantity)
    ...
    def close_order_limit(self, code, post_order_res_list, partial_tps, partial_qtys, close_side, pos_side, open_exec_qty, quantity_precision):
        ...
        while 1:
            ...
            """
            post_order_res, _, error_code = self.post_order(code, OrderType.LIMIT, close_side, pos_side, partial_tp, partial_qty)
    ...
    def cancel_order_list(self, post_order_res_list, order_type=OrderType.LIMIT):
        ...
        if order_type == OrderType.LIMIT:
            [self.cancel_order_wrapper(post_order_res['market'], post_order_res['uuid']) for post_order_res in post_order_res_list if post_order_res is not None]
    ...
    def close_order_market(self, code, post_order_res_list, close_side, pos_side, open_exec_qty):

        """
        거래소별, 수정 필요 method

        1. retry_count updated.
        """
        ...
        while 1:
            ...
            while 1:
                try:
                    if close_side == OrderSide.SELL:
                        post_order_res = self.sell_market_order(code, close_remain_quantity)
                    else:
                        pass    # short order unavailable.
                        # post_order_res = self.buy_market_order(code, close_remain_quantity * order_price)
            ...
            if error_code in ['-2022', '-4003']:
                ...
                if float(post_order_res['volume']) - market_exec_qty < 1 / (10 ** quantity_precision):
                    ...
                    self.sys_log.info("close_order_market reorder")
