...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    df['Fisher_trend'] = TA.FISH(df, period=150)
    df['Fisher_long_short'] = TA.FISH(df, period=30)
    ...
    for i in range(period, len(df)):
        y = df['Fisher_trend'].values[i + 1 - period:i + 1]
        y_short = df['Fisher_long_short'].values[i + 1 - period:i + 1]
    ...
    df['Fisher_trend_gaussian'] = smoothed_curve
    df['Fisher_long_short_gaussian'] = smoothed_curve_short
    ...
    for i in range(1, len(df)):
        ...
        if df['Fisher_long_short'].iloc[i] <= long_signal_value and df['Fisher_trend'].iloc[i] > 0:
            if df['Fisher_long_short'].iloc[i] > df['Fisher_long_short'].iloc[i - 1]:
                trade_state[i] = 1

        # elif df['Fisher_short_gaussian'].iloc[i] >= short_signal_value:
        #     if df['Fisher_short_gaussian'].iloc[i] < df['Fisher_short_gaussian'].iloc[i - 1]:
        #         trade_state[i] = 2

        if df['Fisher_long_short'].iloc[i] >= short_signal_value:
            trade_state[i] = 2
        ...
        if df['Fisher_trend'].iloc[i] < 0:
            trade_state[i] = -1

        for high_passed_signal_value in [2.3, 2.0]:
            if df['Fisher_long_short'].iloc[i - 1] >= high_passed_signal_value > \
                    df['Fisher_long_short'].iloc[i]:
                trade_state[i] = 3
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        elif df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3 or df.trade_state.iloc[m] == -1:
        ...
        # 체결시 재시작
        # if df.close.iloc[m] / bprelay.bprelay.iloc[m] <= 0.94:
        #     while True:
        #         m += 1
        #         if m >= length:
        #             break
    ...
    if len(profits) == 0:
    ...
    elif (float(profits.iloc[-1]) != 1.0 and get_fig == 1) or float(profits.iloc[-1]) < 0.98 or exit_fluc_mean < 0.98:
        ...
        plt.subplot(312)
        plt.plot(df.Fisher_trend)
        # plt.plot(df.Fisher_long_gaussian)
        plt.plot(df.Fisher_trend_gaussian)
        ...
        plt.subplot(313)
        plt.plot(df.Fisher_long_short)
        plt.plot(df.Fisher_long_short_gaussian)
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    # TopCoin = ['BHP', 'FZZ', 'WAXP', 'QBZ', 'INS', 'PLX', 'OMG', 'WPX', 'FNB', 'ANKR']
    TopCoin = ['OMG']
