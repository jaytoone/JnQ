...
# def to_lower_tf_v2(ltf_df, htf_df, column, output_len=None, show_info=False, backing_i=-2):
#
#     start_0 = time.time()
#
#     last_datetime_index = ltf_df.index[-1]
...
#         # print('type(interval) :', type(interval))
#
#     # --------------- sliced_index_len phase --------------- #
#     #       realtime - trader 에서 중요함      #
#     #       backi-2 를 사용한 backtest 와 realtime 의 sync. 역할
#     if interval <= 60:
#         sliced_index_len = last_min % interval + 1  # +1 하는 이유는, 나머지가 0 인 경우에 대해서도 value 를 채워주어야하기 때문임
#                                                     # 채우지않으면, ltf 에 np.nan 이 생김
#     else:
...
#         #        1. 4h, 1d timestamp 시작 시간이 다르기 때문에 각 interval 의 기준으로 재정의해야할 것      #
#         #           a. 1d 는 9:00:00 기준으로 현재까지의 시간을 모두 minute 으로 변경 (sliced_index_len)
#         #               i. 1d tf 의 time 기준이 9:00:00 로 설정되어잇음
#         #               ii. last_hour -> last_hour - 9, if < 0 => + 24
#         #           b. 4h 는 1, 5, 9... 시 기준으로 현재까지의 시간을 minute 으로 변경 (sliced_index_len)
#         #               i. last_hour -> last_hour % 4(h) - 1, if < 0 => + 4
#
#     if show_info:
...
#
#     #           1. backing_i 만큼 이전 데이터를 htf 로부터 추출함            #
#     #            1_1. htf_data 는 ltf_data 와 같은 interval 내에서 서로 매칭될 수 없음 - using future_data
#     value_list = htf_df[column].iloc[[backing_i]].values      # v2, 1-row 라서 [] 씌워줌 -> 2-dimensional shape
#
#     #           2. 해당 htf data 를 ltf 로 변환함 (sliced_len 만큼 변환함)         #
#     value_list = np.repeat(value_list, sliced_index_len, axis=0)     # v2
#
#     # print(value_list)
#     # print(type(value_list))
#     # quit()
...
#     #       Todo        #
#     #        1. np.repeat 사용하기
#     #           1_1. 필요한 temp_list len 계산해서 preproc. 진행
#     temp_len = (len(ltf_df) - len(value_list)) // interval + 1
#     temp_list = htf_df[column].iloc[backing_i - temp_len:backing_i].values
#     repeated_arr = np.repeat(temp_list, interval, axis=0)   # 앞으로는 arr, list 구분
#
#     value_list = np.vstack((repeated_arr, value_list))
#
#     return value_list[-len(ltf_df):]


def to_lower_tf_v2(ltf_df, htf_df, column, show_info=False, backing_i=1):

    ltf_itv = pd.infer_freq(ltf_df.index)

    #        1. 필요한 len 계산해서 preproc. 진행 --> open_idx 를 동일하게 맞춰놓았고, shift 적용 상태이기 때문에 불필요함
    resampled_df = htf_df[column].shift(backing_i).resample(ltf_itv).bfill()
