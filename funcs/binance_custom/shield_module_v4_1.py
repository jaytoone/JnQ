...
class ShieldModule(FuturesModule):
    ...
    def __init__(self, config):
        api_key, secret_key = self.load_key(r"D:\Projects\SystemTrading\JnQ\Bank\api_keys\binance_mademerich.pkl")
    ...
    def get_new_df_v2(self, code, calc_rows=True, mode="OPEN"):
        ...
        while 1:
            try:
                ...
                if last_row_timestamp < datetime.now().timestamp():
                    ...
                    if time.time() - start_time > self.config.trader_set.get_new_df_timeout:
                        return
    ...
    def get_tpepout_v3(self, open_side, res_df_open, res_df):

        """
        v1 --> v3
            1. return out2 for exiting method.
        """
        ...
        if open_side == OrderSide.BUY:
            ...
            out2 = res_df['long_out2_{}'.format(selection_id)].to_numpy()[out_j]
            ...
            out2 = res_df['short_out2_{}'.format(selection_id)].to_numpy()[out_j]
            tp = res_df_open['short_tp_{}'.format(selection_id)].to_numpy()[tp_j]

        return tp, ep, out, out2, open_side

    def get_tpepout_v2(self, open_side, res_df_open, res_df):

        """
        v1 --> v2
            1. return leverage for ML mode.
        """
        ...
        if open_side == OrderSide.BUY:
            ...
            leverage_ = res_df_open['long_lvrg_needed_{}'.format(selection_id)].to_numpy()[ep_j]
            ...
            leverage_ = res_df_open['short_lvrg_needed_{}'.format(selection_id)].to_numpy()[ep_j]

        return tp, ep, out, leverage_, open_side
