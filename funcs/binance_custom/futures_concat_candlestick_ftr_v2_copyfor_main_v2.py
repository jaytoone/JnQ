...
key_abspath = r"D:\Projects\SystemTrading\JnQ\Bank\api_keys\binance_mademerich.pkl"
...
def concat_candlestick_v2(symbol, interval, days, limit, end_date=None, show_process=False, timesleep=None):

    """
    v1 --> v2
        1. aggregate dataframe by timestamp_unit.
    """

    if show_process:
        print(symbol)

    assert limit <= candle_limit, "assert limit < candle_limit ({})".format(candle_limit)
    ...
    start_time_arr = np.arange(start_timestamp, end_timestamp, timestamp_unit)
    end_time_arr = start_time_arr + timestamp_unit
    ...
    for start_time, end_time in zip(start_time_arr, end_time_arr):
        try:
            ...
            print("error in concat_candlestick_v2 :", e)
...
if __name__ == '__main__':
    ...
    # with open(r'D:\Projects\System_Trading\JnQ\database\ticker_list\ticker_in_futures.pkl', 'rb') as f:
    with open(r"D:\Projects\System_Trading\JnQ\Bank\tickers\binance_20231017.pkl", 'rb') as f:
        symbol_list = pickle.load(f)

    # print(um_futures_client.exchange_info()['symbols'])
    # symbol_list = [data['symbol'] for data in um_futures_client.exchange_info()['symbols']]  # if data.symbol not in ['BTCUSDT', 'ETHUSDT']]  # Binance all exchange symbol list
