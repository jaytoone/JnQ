from funcs.binance.futures_modules import OrderSide, OrderType, get_market_price_v2, \
    ...
    callback, error, check_execution, get_execPrice, get_execQty
...
def get_income_info_v2(self, real_balance, leverage, ideal_profit, real_profit):
...
def check_market_tp(self, res_df, market_close_on, log_tp, cross_on, open_side, tp):
    ...
    #        2. inversion 에 대한 고려 진행된건가 - 안된것으로 보임
    if open_side == OrderSide.SELL:
        # ------------ market_close ------------ #
        # ------ short ------ #
        if self.config.tp_set.tp_type == "MARKET" or self.config.tp_set.tp_type == "BOTH":
            ...
            #   Todo - ideal_pr 과의 오차율 이슈로, open_index 로 변경
            if self.config.tp_set.time_tp:
                if res_df['short_close_{}'.format(self.config.strat_version)][self.config.trader_set.open_index] == self.config.ep_set.short_entry_score:
                    while 1:    # wait for bar_ending time
                        if datetime.now().second >= self.config.trader_set.bar_close_second:  # Todo - datetime sec. 의 max_value = 59
                            break
                    market_close_on = True
        ...
        if self.config.tp_set.tp_type == "MARKET" or self.config.tp_set.tp_type == "BOTH":
            ...
            # ------ time_tp ------ # - income() waiting issue 로 complete_index 사용함
            if self.config.tp_set.time_tp:
                if res_df['long_close_{}'.format(self.config.strat_version)][self.config.trader_set.open_index] == -self.config.ep_set.short_entry_score:
                    while 1:    # wait for bar_ending time
                        if datetime.now().second >= self.config.trader_set.bar_close_second:
                            break
                    market_close_on = True

            if market_close_on:
                log_tp = res_df['close'].iloc[j]    # j 때문에 이곳에 배치
                sys_log.info("tp : {}".format(tp))
...
def check_limit_tp_exec(self, post_order_res_list, quantity_precision, return_price=False):
    ...
    return all_executed, []  # return length 는 유지해야할 것
...
def check_ei_k(self, res_df_init, open_side):
    ...
    if self.config.loc_set.zone.ei_k != "None":  # Todo -> 이거 None 이면 "무한 대기" 발생 가능함
        ...
        if self.config.tp_set.time_tp:
            ...
            if time.time() - datetime.timestamp(res_df_init.index[-1]) >= self.config.loc_set.point.tf_entry * 30:    # half_time (seconds)
                ep_out = 1
        else:
...
def check_ei_k_onbarclose(self, res_df_init, res_df, open_side, e_j, tp_j):     # for point2
    ep_out = 0
    strat_version = self.config.strat_version
    if open_side == OrderSide.SELL:
