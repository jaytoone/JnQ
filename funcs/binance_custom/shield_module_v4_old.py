from funcs.binance.futures_module_v3 import FuturesModule
...
class ShieldModule(FuturesModule):
    ...
    v3 -> v4
        1. trader : Shield version 에 맞도록 ShieldModule Update.
    """
    ...
    def check_hl_out_v2(self, res_df, market_close_on, log_out, out, liqd_p, open_side):
        ...
        realtime_price = self.get_market_price_v2()
        # market_close_one loop end phase 에 term 있음.
    ...
    def partial_limit_order_v4(self, partial_tps, partial_qtys, close_side, pos_side, open_exec_qty, quantity_precision):
        ...
        while 1:
            ...
            #       a. 현재 open_exec_qty 를 수정하기 위해서는 open_orderId 에서 exec_qty 를 다시 가져와야함. --> 다소 번거로움.
                ...
                # a. possible errors 에 대하여, post_order_res_list 를 return 하도록 함.
        ...
        while 1:  # <--- This loop for complete close
            ...
            while 1:  # <-- loop for complete close order
                try:
                    ...
                    self.sys_log.error('error in close order : {}'.format(e))  # 윗 error 는 logging 하지않음
