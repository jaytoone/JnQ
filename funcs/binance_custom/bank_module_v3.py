from funcs.binance.futures_module_v3_1 import FuturesModule
from binance_f.model import *

from funcs.binance.futures_concat_candlestick_ftr_v2_copyfor_main_v2 import concat_candlestick
...
class BankModule(FuturesModule):
    """
    Bank 의 정상적인 동작을 위한 각 phase 의 method 가 모인 Class
    v2 -> v3
        1. UMFutures 도입.
            a. get_market_price_v3 used.
    """
    ...
    def check_hl_out_v2(self, res_df, market_close_on, log_out, out, liqd_p, open_side):
    ...
    def limit_order(self, order_type, limit_side, pos_side, limit_price, limit_quantity, order_info=None):
        ...
        while 1:
            try:
                ...
                #       b. -1111 : Precision is over the maximum defined for this asset
                ...
                #       c. limit_order() 에서 해결할 error 일 경우, continue
                ...
                if '-4014' in str(e):
                    try:
                        ...
                        self.sys_log.error('error in get_market_price_v3 (open_order phase): {}'.format(e))
                    continue

                #       d.  -2019 : Margin is insufficient
                # Todo - for tp_exec_qty miscalc., self.overbalance 수정되었으면, return 으로 받아야할 것 => 무슨 소리야..?
                open_retry_cnt += 1
                if '-2019' in str(e):
                    if order_info is not None:  # None 인 경우, leverage 가 정의되지 않음.
                        try:
                            # i. 예기치 못한 오류로 인해 over balance 상태가 되었을때의 조치
                            available_balance = self.get_available_balance() * 0.9

                            # ii. get available quantity
                            ...
                            self.sys_log.error('error in get_availableBalance (-2019 phase) : {}'.format(e))
    ...
    def partial_limit_order_v4(self, partial_tps, partial_qtys, close_side, pos_side, open_exec_qty, quantity_precision):
        ...
        while 1:
            ...
            # 3. errors
            #       a. -1111 : quantity precision error
            #               i. all_cancel 하고 처음부터 다시 시작. (temp-solved)
            ...
            #           i. 당장에 처리하기 힘든 error 처리의 경우 alert 기능을 도입한다. (Todo)
            #           ii. 가설 1 : open_exec_qty 가 잘못되면 발생하지 않을까.
            #                   a. 현재 open_exec_qty 를 수정하기 위해서는 open_orderId 에서 exec_qty 를 다시 가져와야함. --> 다소 번거로움.
                ...
                #   c. possible errors 에 대하여, post_order_res_list 를 return 하도록 함.
                #           i. -1021, "Timestamp for this request was 1000ms ahead of the server's time."
                #           ii. -2019 (Margin is insufficient) or something else
        ...
        while 1:
            ...
            while 1:
                try:
                    ...
                    self.sys_log.error('error in close order : {}'.format(e))
                    ...
                    if '-1111' in str(e):
                        ...
                        try:
                            ...
                            close_remain_quantity = self.calc_with_precision(close_remain_quantity, quantity_precision, def_type='floor')
                            self.sys_log.info('modified qty & precision : {} {}'.format(close_remain_quantity, quantity_precision))

                        except Exception as e:
                            self.sys_log.error('error in get modified qty_precision : {}'.format(e))
                        continue
