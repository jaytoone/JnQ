...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    for i in range(2, len(df)):
        ...
        #       TRIX LONG      #
        if df['Trix_TREND'].iloc[i - 1] == 'DOWN' and df['Trix_TREND'].iloc[i] == 'UP':
            trade_state[i] = 1.5
    ...
    target = df['Trix_TREND']
    for i in range(len(target)):
        if target.iloc[i] in [1.5]:
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['WOM', 'BOA', 'EGG', 'DVP', 'OGO', 'FX', 'BZNT', 'ITC', 'FCT', 'EM', 'AION', 'META', 'SOC', 'DAD', 'RNT', 'WAXP', 'CHR', 'COS', 'MTL', 'LRC']
    TopCoin = ['DVP', 'XPR', 'CHR', 'EM', 'SOC', 'ARPA', 'WTC']
    # TopCoin = ['APIX', 'QBZ']
