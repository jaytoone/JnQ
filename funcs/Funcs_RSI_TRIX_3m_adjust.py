...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    df['RSI_TRIX'] = TA.TRIX(df, period=18, column='RSI')
    for i in range(20):
        df['RSI_TRIX'].iloc[i] = 0
    df['RSI_TRIX_TREND'] = np.where(df['RSI_TRIX'].shift(1) <= df['RSI_TRIX'], 'UP', 'DOWN')

    df['TRIX_LATER'] = TA.TRIX(df, period=20)
    df['TRIX_LATER'] = np.where(np.isnan(df['TRIX_LATER']), 0, df['TRIX_LATER'])
    df['TRIX_LATER_TREND'] = np.where(df['TRIX_LATER'].shift(1) <= df['TRIX_LATER'], 'UP', 'DOWN')
    ...
    for i in range(period, len(df)):
        y = df['RSI_TRIX'].values[i + 1 - period:i + 1]
    ...
    df['RSI_TRIX_GAUSSIAN'] = smoothed_curve
    df['RSI_TRIX_GAUSSIAN_TREND'] = np.where(df['RSI_TRIX_GAUSSIAN'].shift(1) <= df['RSI_TRIX_GAUSSIAN'], 'UP', 'DOWN')
    #
    ...
    dc_bp_accept_gap_range = 0.13
    dc_low_entry_low_accept_gap_range = 0.08
    ...
    for i in range(2, len(df)):
        ...
        #       RSI TRIX TREND UP WITH DC LOW ASCEND        #
        # if df['RSI_TRIX'].iloc[i - 1] <= long_signal_value and df['RSI_TRIX'].iloc[i] > long_signal_value:
        if df['RSI_TRIX_TREND'].iloc[i - 1] == 'DOWN' and df['RSI_TRIX_TREND'].iloc[i] == 'UP':
        ...
        # if df['TRIX_LATER'].iloc[i] / (df['TRIX'].max() - df['TRIX'].min()) <= long_signal_value: #and df['close'].iloc[i] > clearance(df['SMMA'].iloc[i] * smma_accept_range):
        #     #       FIND TRIX CROSS     #
        ...
        #         while True:
        #             copy_i -= 1
        #             if df['TRIX_TREND'].iloc[copy_i] != df['TRIX_TREND'].iloc[copy_i - 1]:
        #                 break
        #         print('ENTRY VERTICAL GAP :', i,
        #               abs(df['TRIX'].iloc[i] - df['TRIX'].iloc[copy_i]) / (df['TRIX'].max() - df['TRIX'].min()))
        #
        #         if abs(df['TRIX'].iloc[i] - df['TRIX'].iloc[copy_i]) / (
        #                 df['TRIX'].max() - df['TRIX'].min()) < trix_entry_vertical_gap:
        #             continue
        #
        ...
        #             i += 1
        #
        #             if df['TRIX_LATER'].iloc[i] / (df['TRIX'].max() - df['TRIX'].min()) > long_signal_value:
        #                 break
        #         # if df['TRIX_GAUSSIAN'].iloc[i] - df['TRIX_GAUSSIAN'].iloc[i - 1] >= 1.0:
        #         #     if df['Fisher_long'].iloc[i] < 0:
        #
        #         if buy_signal:
        #
        #             print('ENTRY LONG SIGNAL VALUE :', i,
        #                   df['TRIX_LATER'].iloc[i] / (df['TRIX'].max() - df['TRIX'].min()))
        #             print('PREVIOUS TRIX GAP :', i,
        #                   (df['TRIX'].iloc[i] - df['TRIX'].iloc[i - 1]) / (df['TRIX'].max() - df['TRIX'].min()))
        #             #           TRIX 변화율 비교         #
        #             # if (df['TRIX'].iloc[i] - df['TRIX'].iloc[i - 1]) / (df['TRIX'].max() - df['TRIX'].min()) >= trix_entry_previous_gap:
        #
        #             if df['DC_TREND'].iloc[i] == 'UP':
        #             # and upper_line > df['TRIX_GAUSSIAN'].iloc[i] >= lower_line\
        #             # and df['TRIX'].iloc[i] != 0:  # 초기에 진입하는거를 방지하고 TRIX와 비교하기 위해서
        #                 trade_state[i] = 1.5
        #
        ...
        # if df['TRIX_TREND'].iloc[i - 1] == 'UP' and df['TRIX_TREND'].iloc[i] == 'DOWN':
        #     trade_state[i] = 2

        #       CROSS가 나을까 DOWN TREND가 나을까      #q
        if df['RSI_TRIX_TREND'].iloc[i - 1] == 'UP' and df['RSI_TRIX_TREND'].iloc[i] == 'DOWN':
            trade_state[i] = 2
    ...
    for i in range(1, len(target)):
        ...
        if df['RSI_TRIX_TREND'].iloc[i] == 'DOWN' and df['RSI_TRIX_TREND'].iloc[i - 1] == 'UP':
            span_list_fisher_gaussian.append((i, i + 1))
        elif df['RSI_TRIX_TREND'].iloc[i] == 'UP' and df['RSI_TRIX_TREND'].iloc[i - 1] == 'DOWN':
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            if df.trade_state.iloc[m] == 2:
                ...
                #       이전 TRIX와의 GAP이 작으면 PASS     #
                print('PREVIOUS TRIX GAP :', m,
                      (df['TRIX'].iloc[m] - df['TRIX'].iloc[m - 1]) / (df['TRIX'].max() - df['TRIX'].min()))
                print('TRIX GAP:', m, (df['TRIX'].iloc[m] - df['TRIX'].iloc[start_m]) / (df['TRIX'].max() - df['TRIX'].min()))

                # if (df['TRIX'].iloc[m] - df['TRIX'].iloc[m - 1]) / (df['TRIX'].max() - df['TRIX'].min()) >= trix_exit_previous_gap:
                #     pass

                if m - start_m > trix_horizontal_ticks:
                    #       자잘거림 보정     #
                    #       이전 변곡점과의 상하좌우 폭 비교      #
                    copy_m = m
                    while True:
                        copy_m -= 1
                        if df['TRIX_TREND'].iloc[copy_m] != df['TRIX_TREND'].iloc[copy_m - 1]:
                            break

                    if abs(df['TRIX'].iloc[m] - df['TRIX'].iloc[copy_m]) / (df['TRIX'].max() - df['TRIX'].min()) < trix_exit_vertical_gap:
                        pass
                    else:
                        sell_switch = True
                        break
            ...
            # #       1. 매수 시그널의 TRIX 값보다 작아지면 손절        #
            # if m - start_m > trix_horizontal_ticks \
            #         and (df['TRIX'].iloc[m] - df['TRIX'].iloc[buy_signal_m]) / (df['TRIX'].max() - df['TRIX'].min()) < trix_exit_gap:
            #     print('EXIT TRIX GAP :', m, (df['TRIX'].iloc[m] - df['TRIX'].iloc[buy_signal_m]) / (df['TRIX'].max() - df['TRIX'].min()))
            #     exit_switch = True
            #     break
            #
            # #       2. DC DESCENDING      #
            # if (df['DC_LOW'].iloc[m] - df['DC_LOW'].iloc[start_m]) / chart_height < -0.02:
            #     print('DC DESCEND GAP at %s :' % m, (df['DC_LOW'].iloc[m] - df['DC_LOW'].iloc[start_m]) / chart_height)
            #     exit_switch = True
            #     break
    ...
    if len(profits) == 0:
        ...
        plt.title('TRIX TRIX - %s %.3f %.3f %.3f' % (Coin,
        ...
        for trade_num in range(len(span_list_target)):
            plt.axvspan(span_list_target[trade_num][0], span_list_target[trade_num][1], facecolor='red', alpha=0.5)
        ...
        plt.plot(df['RSI_TRIX'])
        plt.plot(df['RSI_TRIX_GAUSSIAN'], color='m')
        # plt.plot(df['Fisher_GAUSSIAN'], color='limegreen')
        # plt.plot(df.Fisher_short2, color='gold')
        # plt.plot(df.Fisher_short_gaussian)
        plt.axhline(long_signal_value, color='red', alpha=0.5)
        # plt.axhline(short_signal_value)
        for trade_num in range(len(span_list_fisher_gaussian)):
            plt.axvspan(span_list_fisher_gaussian[trade_num][0], span_list_fisher_gaussian[trade_num][1], facecolor='red', alpha=0.7)
        for trade_num in range(len(span_list_target_entry)):
            plt.axvspan(span_list_target_entry[trade_num][0], span_list_target_entry[trade_num][1], facecolor='green',
                        alpha=0.7)
        ...
        plt.subplot(313)
        plt.plot(df.TRIX)
        plt.plot(df.TRIX_LATER, color='orange')
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['ITC', 'REP', 'EL', 'ITC', 'STRAT', 'LOOM']
    TopCoin = ['BOA']
    # TopCoin = ['CHR', 'EGG']
    # TopCoin = ['2020-06-10 IOST ohlcv.xlsx']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = -0.25, 1.9
