...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    for i in range(period, len(df)):
        y = df['KST_Origin'].values[i + 1 - period:i + 1]
    ...
    df['KST_GAUSSIAN_Fast'] = smoothed_curve
    ...
    for i in range(2, len(df)):
        ...
        #     trade_state[i] = 1.5
        if df['KST_GAUSSIAN'].iloc[i] > df['KST'].iloc[i] and df['KST_GAUSSIAN'].iloc[i - 1] < df['KST'].iloc[i - 1] \
                and upper_line > df['KST_GAUSSIAN'].iloc[i] >= below_line\
                and df['KST'].iloc[i] != 0:
            trade_state[i] = 1.5
        ...
        #             # if df['SMMA_DC_GAP'].iloc[j - 1] >= smma_accept_range:
        ...
        #                     # while True:
        #                     #     j_copy -= 1
        #                     #     if j_copy == 0 or df['Fisher_long'].iloc[j_copy] > long_signal_value:
        #                     #         break
        #                     #     if df['DC_LOW'].iloc[j_copy] < df['DC_LOW'].iloc[j_copy - 1]:
        #                     #         dc_down = True
        ...
        #                     trade_state[j] = 1.5
        #
        #                     print('%s CLOSE_DC_GAP = %.3f | SMMA_DC_GAP AT = %.3f' % (j, df['CLOSE_DC_GAP'].iloc[j], df['SMMA_DC_GAP'].iloc[j]))
        #
        #                     # if df['CLOSE_DC_GAP'].iloc[j] <= dc_accept_range:
        #                     #     trade_state[j] = 1.5
        #                     #     print('%s [ENTRY] CLOSE_DC_GAP = %.3f | SMMA_DC_GAP AT = %.3f' % (
        #                     #         j, df['CLOSE_DC_GAP'].iloc[j], df['SMMA_DC_GAP'].iloc[j]))
        #                     # j_copy = j
        #                     # while True:
        #                     #     j_copy -= 1
        #                     #     if j_copy == 0 or df['Fisher_long'].iloc[j_copy] > long_signal_value:
        #                     #         break
        #                     #
        #                     #     #       매수 포인트에서 CLOSE DC GAP이 작아야한다.     #
        #                     #     #               BLUE LINE POINT              #
        #                     #     if df['CLOSE_DC_GAP'].iloc[j_copy] <= dc_accept_range:
        #                     #         trade_state[j] = 1.5
        #                     #         print('%s [ENTRY] CLOSE_DC_GAP = %.3f | SMMA_DC_GAP AT = %.3f' % (
        #                     #         j_copy, df['CLOSE_DC_GAP'].iloc[j_copy], df['SMMA_DC_GAP'].iloc[j_copy]))
        #                     #         break
        #
        #         #        FISHER LONG > LONG SIGNAL VALUE까지 구간의 IP POINT를 모두 찾는다.        #
    ...
    span_list_target_kst = list()
    span_list_target_kst_ic = list()
    target = df['trade_state']
    for i in range(len(target)):
        if df['KST_TREND'].iloc[i] != df['KST_TREND'].iloc[i - 1]:
            span_list_target_kst.append((i, i + 1))
        if df['KST_TREND'].iloc[i - 1] == 'DOWN' and df['KST_TREND'].iloc[i] == 'UP' and upper_line > df['KST'].iloc[i] >= below_line:
            span_list_target_kst_ic.append((i, i + 1))

    span_list_target_gaussian = list()
    span_list_target_gaussian_ic = list()
    target = df['trade_state']
    for i in range(1, len(target)):
        if df['KST_GAUSSIAN_TREND'].iloc[i] != df['KST_GAUSSIAN_TREND'].iloc[i - 1]:
            span_list_target_gaussian.append((i, i + 1))
        if df['KST_GAUSSIAN_TREND'].iloc[i - 1] == 'UP' and df['KST_GAUSSIAN_TREND'].iloc[i] == 'DOWN':
            span_list_target_gaussian_ic.append((i, i + 1))
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            if df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3:
                ...
                print('KST_GAUSSIAN m, start_m :', df['KST_GAUSSIAN'].iloc[m], df['KST_GAUSSIAN'].iloc[start_m])
                # #       자잘거림 보정     #
                # if abs(df['KST_GAUSSIAN'].iloc[m] - df['KST_GAUSSIAN'].iloc[start_m]) < 1:
                #     pass
                # else:
                break
            ...
            #       1. 매수 시그널이 다시 낳왔는데 첫 매수 시그널보다 그 값이 작은 경우    #
            # if m != start_m and df['trade_state'].iloc[m] == 1.5:
            #     #      바로 전 매수 시그널을 찾는다.     #
            #     copy_m = m
            #     while True:
            #         copy_m -= 1
            #         if df['trade_state'].iloc[copy_m] == 1.5:
            #             break
            #     if df['KST_GAUSSIAN'].iloc[copy_m] > df['KST_GAUSSIAN'].iloc[m]:
            #         exit_switch = True
            #         break

            #       매수 시그널의 KST_GAUSSIAN 값보다 작아지면 손절        #
            if m != buy_signal_m and df['KST_GAUSSIAN'].iloc[buy_signal_m] > df['KST_GAUSSIAN'].iloc[m]:
                exit_switch = True
                break
    ...
    if len(profits) == 0:
        ...
        plt.title('KST GAUSSIAN CROSS - %s %.3f %.3f %.3f' % (Coin,
        ...
        plt.axhline(below_line)
        for trade_num in range(len(span_list_target)):
            plt.axvspan(span_list_target[trade_num][0], span_list_target[trade_num][1], facecolor='red', alpha=0.3)
        for trade_num in range(len(span_list_target_ic)):
            plt.axvspan(span_list_target_ic[trade_num][0], span_list_target_ic[trade_num][1], facecolor='red', alpha=0.3)
        for trade_num in range(len(span_list_target_kst)):
            plt.axvspan(span_list_target_kst[trade_num][0], span_list_target_kst[trade_num][1], facecolor='blue', alpha=0.3)
        # for trade_num in range(len(span_list_target_kst_ic)):
        #     plt.axvspan(span_list_target_kst_ic[trade_num][0], span_list_target_kst_ic[trade_num][1], facecolor='blue', alpha=0.7)
        for trade_num in range(len(span_list_target_gaussian)):
            plt.axvspan(span_list_target_gaussian[trade_num][0], span_list_target_gaussian[trade_num][1], facecolor='green', alpha=0.3)
        # for trade_num in range(len(span_list_target_gaussian_ic)):
        #     plt.axvspan(span_list_target_gaussian_ic[trade_num][0], span_list_target_gaussian_ic[trade_num][1], facecolor='green', alpha=0.7)
        # for trade_num in range(len(spanlist_limit)):
        #     plt.axvspan(spanlist_limit[trade_num][0], spanlist_limit[trade_num][1], facecolor='c', alpha=0.5)
        # for trade_num in range(len(spanlist_breakaway)):
        #     plt.axvspan(spanlist_breakaway[trade_num][0], spanlist_breakaway[trade_num][1], facecolor='m', alpha=0.5)
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['IOST', 'BZNT', 'PCM', 'KNC', 'ELF', 'AMO', 'AE', 'FLETA', 'LBA', 'CRO']
    TopCoin = ['APIX', 'PCM', 'ZIL', 'FLETA', 'FX', 'VET', 'RNT']
    # TopCoin = ['APIX', 'QBZ']
