...
def low_high(reg_target, model, crop_size=0):
    ...
    if crop_size != 0:
        ...
        #       Regression Twice        #
        none_indexs_twice = np.sum(np.isnan(realtime_osc_reg))
        realtime_osc_reg_twice = [np.nan] * len(reg_target)
        # print(none_indexs_twice)
        # quit()
        for i in range(none_indexs_twice + crop_size, len(reg_target)):
            x = np.arange(len(reg_target))[i - crop_size:i].reshape(-1, 1)
            y = np.array(realtime_osc_reg)[i - crop_size:i].reshape(-1, 1)
            ...
            model.fit(x, y)
            osc_reg = model.predict(x)
            realtime_osc_reg_twice[i] = osc_reg[-1]
        ...
        none_indexs_triple = np.sum(np.isnan(realtime_osc_reg_twice))
        realtime_osc_reg_triple = [np.nan] * len(reg_target)
        for i in range(none_indexs_triple + crop_size, len(reg_target)):
            x = np.arange(len(reg_target))[i - crop_size:i].reshape(-1, 1)
            y = np.array(realtime_osc_reg_twice)[i - crop_size:i].reshape(-1, 1)
            ...
            model.fit(x, y)
            osc_reg = model.predict(x)
            realtime_osc_reg_triple[i] = osc_reg[-1]

        trade_state = [np.nan] * len(reg_target)
        for i in range(none_indexs + crop_size, len(reg_target)):
            if realtime_osc_reg_triple[i - 1] < realtime_osc_reg_triple[i]:
                trade_state[i] = 'up'
            else:
                trade_state[i] = 'down'

        return realtime_osc_reg_triple, trade_state
...
def profitage(Coin, short, long, signal, C, gamma, epsilon, model, crop_size, Date='2019-09-25', excel=0, get_fig=0):
    ...
    reg_target = df['close']
    osc_reg, trade_state = low_high(reg_target, model, crop_size)
    ...
    if len(profits) == 0:
        ...
        plt.subplot(313)
        osc_reg, _ = low_high(reg_target, model)
        plt.plot(osc_reg, 'gold', label='oscillator', linewidth=1.0)
        plt.xlim(0, len(ochl))
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['BASIC', 'SOC', 'AION', 'POWR', 'LBA', 'GNT', 'CON', 'BHP', 'ELF', 'LUNA']
    # TopCoin = ['LUNA']
        ...
        for long in range(short + 3, short + 100, 5):
            for signal in range(5, 50, 3):
                ...
                for Coin in TopCoin:
                    # print(Coin)
                    # Coin = excel_list[3]
                    print(Coin, profitage(Coin, short, long, signal, C, gamma, epsilon, model, crop_size=200, Date=Date, get_fig=1, excel=0))
                quit()
