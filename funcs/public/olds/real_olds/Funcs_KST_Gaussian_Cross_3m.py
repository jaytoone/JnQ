...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    for i in range(period2, len(df)):
        y_short = df['KST_Origin'].values[i + 1 - period2:i + 1]
        y_smooth_short = gaussian_filter1d(y_short, sigma=sigma2)
    ...
    for i in range(2, len(df)):
        ...
        #     trade_state[i] = 1.5
        if df['KST_GAUSSIAN_Fast'].iloc[i] > df['KST_GAUSSIAN'].iloc[i] and df['KST_GAUSSIAN_Fast'].iloc[i - 1] < \
                df['KST_GAUSSIAN'].iloc[i - 1]:
            # if upper_line > df['KST_GAUSSIAN_Fast'].iloc[i] >= below_line:
            if df['KST_GAUSSIAN_Fast'].iloc[i] != 0:
                trade_state[i] = 1.5
    ...
    for i in range(1, len(target)):
        if df['KST_GAUSSIAN_Fast'].iloc[i] < df['KST_GAUSSIAN'].iloc[i] \
                and df['KST_GAUSSIAN_Fast'].iloc[i - 1] > df['KST_GAUSSIAN'].iloc[i - 1]:
