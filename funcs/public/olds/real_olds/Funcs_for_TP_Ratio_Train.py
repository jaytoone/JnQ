...
def profitage(coin, period1=30, period2=60, period3=120, date='2019-09-25', excel=0, get_fig=0):
    ...
    if not coin.endswith('.xlsx'):
        ...
        limit = 6000  # <-- should be None, if you want all data from concated chart
        df = pd.read_excel(os.path.join(dir, '1m', coin), index_col=0)[-limit:]
    ...
    for i in range(tick_in_box, len(df)):
        ...
        #           Long            #
        if df['ep'].iloc[i] >= df['long_sl'].iloc[i] + abs(
    ...
    if excel:
        if coin.endswith('.xlsx'):
            ...
            df.to_excel("labeled_data/TP_Ratio_TIB/%s/%s" % (tick_in_box, coin))
        else:
