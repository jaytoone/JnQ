import numpy as np
import pandas as pd
from funcs.public.broker import to_lower_tf_v2, intmin, ffill, to_itvnum, to_htf
import talib
...
def h_candle_v4(ltf_df, htf_df, columns=None):
...
def donchian_channel_v4(df, period):
  ...
  ll = talib.MIN(['low'], period).to_numpy()
...
def wave_range_v9(df, period1, period2, ltf_df=None, wave_min_bars=20, wave_max_bars=40):  # v2 for period1 only

    itv = pd.infer_freq(df.index)

    df = donchian_channel_v4(df, period1)
    df = donchian_channel_v4(df, period2)
    ...
    # amax touch 에는 shift(1) 적용, point's 의 amax_touch 는 제외함
    # Todo, bmax 에 shift(1) 넣을지 말지 고민 중
    short_upper_touch_idx = pd.Series(np.where(high >= dc_upper2_, np.arange(len_df), np.nan)).shift(1).rolling(wave_max_bars,
    ...
    short_open_res *= (high >= short_base) & (short_upper_touch_idx < short_lower_touch_idx)
    short_open_res *= (short_lower_touch_idx - short_upper_touch_idx) >= wave_min_bars
    ...
    long_lower_touch_idx = pd.Series(np.where(low <= dc_lower2_, np.arange(len_df), np.nan)).shift(1).rolling(wave_max_bars,
    ...
    long_open_res *= (long_base >= low) & (long_lower_touch_idx < long_upper_touch_idx)
    long_open_res *= (long_upper_touch_idx - long_lower_touch_idx) >= wave_min_bars
    ...
    df['long_wave_point_{}{}{}'.format(itv, period1, period2)] = long_open_res  # multi selection_id 로, formatting 하는 거임
...
def dc_line_v3(ltf_df, dc_period=20):

    interval = pd.infer_freq(ltf_df.index)
    if interval not in ['T', '1m']:
        if interval in ['D']:
            htf_df = to_htf(ltf_df, interval, offset='9h')
        else:
            htf_df = to_htf(ltf_df, interval, offset='1h')
        donchian_channel_v3(htf_df, dc_period)
        return ltf_df.join(to_lower_tf_v2(ltf_df, htf_df, [i for i in range(-3, 0, 1)]), how='inner')
    else:
        donchian_channel_v3(ltf_df, dc_period)
