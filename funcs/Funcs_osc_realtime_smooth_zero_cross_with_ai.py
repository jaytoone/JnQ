...
def low_high(reg_target, sigma, crop_size=0, realtime=0):
    ...
    model = load_model('./model/rapid_ascending %s_%s.hdf5' % (30, 106))
    ...
    if crop_size != 0:
        ...
        for i in range(none_indexs + crop_size, len(reg_target)):
            ...
            y_pred = model.predict(y.reshape(-1, 30, 1))
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['WET', 'ETH', 'VSYS', 'EOS', 'LINK', 'LBA', 'XLM', 'ZEC', 'WICC', 'LTC']
    # TopCoin = ['LUNA']
