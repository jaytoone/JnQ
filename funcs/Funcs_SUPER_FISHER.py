...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    df['hclose'] = (df['open'] + df['close'] + df['high'] + df['low']) / 4
    df['hclose'] = df['hclose'].apply(lambda x: hclearance(x))
    df['hopen'] = df['open']
    # df['hopen'].iloc[0] = df['open'].iloc[0]
    for i in range(1, len(df)):
        # df['hopen'].iloc[i] = ((df['hopen'].iloc[i - 1] + df['hclose'].iloc[i - 1]) / 2)
        df['hopen'].iloc[i] = hclearance((df['hopen'].iloc[i - 1] + df['hclose'].iloc[i - 1]) / 2)

    df['hhigh'] = df[['high', 'hopen', 'hclose']].max(axis=1)
    df['hlow'] = df[['low', 'hopen', 'hclose']].min(axis=1)
    df['HTREND'] = np.where(df['hclose'] > df['hopen'], 'UP', 'DOWN')
    df['HTREND'] = np.where(df['hclose'] == df['hopen'], df['HTREND'].shift(1), df['HTREND'])
    df['HSUPPORT'] = np.NaN
    for i in range(1, len(df)):
        if df['HTREND'].iloc[i] == 'DOWN' and df['HTREND'].iloc[i - 1] == 'UP':
            for j in range(i + 1, len(df)):
                if df['HTREND'].iloc[j] == 'UP':
                    df['HSUPPORT'][i:j] = df['hlow'][i:j].min()
                    break
    for i in range(1, len(df)):
        if df['HTREND'].iloc[i] == 'UP':
            df['HSUPPORT'].iloc[i] = df['HSUPPORT'].iloc[i - 1]
            # print(i, df['HSUPPORT'].iloc[i - 1])
    df['HSUPPORT'] = df['HSUPPORT'].rolling(20).min()

    columns = df.columns.to_list()
    columns = columns[:5] + columns[6:7] + columns[5:6] + columns[7:]
    df = df[columns]
    # print(df.tail())

    df['MA_SHORT'] = df['close'].rolling(20).mean()
    df['MA_SHORT_TREND'] = np.where((df['MA_SHORT'] + df['MA_SHORT'].shift(2)) / 2 > df['MA_SHORT'].shift(1), 'UP', 'DOWN')
    ...
    df['SUPERTREND'] = supertrend(df, 100, 3)
    df['ICHIMOKU'] = TA.ICHIMOKU(df, 9, 30, 52, 26)
    ...
    for i in range(2, len(df)):
        ...
        #     trade_state[i] = 1
        #     if df['SUPERTREND'].iloc[i] == df['Lower Band'].iloc[i]:
        #       trade_state[i] = 1.5

        # if df['HTREND'].iloc[i - 1] == 'DOWN' and df['HTREND'].iloc[i] == 'UP':
        if df['HTREND'].iloc[i] == 'UP':
            if (df['hclose'].iloc[i] + df['hopen'].iloc[i]) / 2 >= df['MA_LONG'].iloc[i] >= df['hlow'].iloc[i]:
                if df['MA_SHORT'].iloc[i] > df['MA_LONG'].iloc[i]:
                    if df['MA_TREND'].iloc[i] == 'UP':
                        if df['MA_SHORT_TREND'].iloc[i] == 'UP':
                            trade_state[i] = 1.5

        # if df['SUPERTREND'].iloc[i] == df['Lower Band'].iloc[i] and df['SUPERTREND'].iloc[i - 1] == df['Upper Band'].iloc[i - 1]:
        #     if df['SUPERTREND'].iloc[i - 1] >= df['MA_SHORT'].iloc[i - 1] >= df['MA_LONG'].iloc[i - 1]:
        #         if df['MA_TREND'].iloc[i - 1] == 'UP':
        #             trade_state[i] = 1.5
        ...
        if df['MA_SHORT_TREND'].iloc[i] == 'DOWN':
            trade_state[i] = 2

        # if df['SUPERTREND'].iloc[i] == df['Upper Band'].iloc[i]:
        #   if trade_state[i] != 1.5:
        #     trade_state[i] = 2

        # if df['HTREND'].iloc[i - 1] == 'UP' and df['HTREND'].iloc[i] == 'DOWN':
        #     trade_state[i] = 2

        # if df['hclose'].iloc[i] < df['MA_SHORT'].iloc[i]:
        #     trade_state[i] = 2

        # if df['HSUPPORT'].iloc[i] < df['HSUPPORT'].iloc[i - 1]:
        #     trade_state[i] = 2
    ...
    df['BuyPrice'] = np.where(df['trade_state'] == 1.5, (df['hclose'] + df['hopen']) / 2, np.nan)
    # df['BuyPrice'] = np.where(df['trade_state'] == 1.5, df['MA_SHORT'], np.nan)
    ...
    while m <= length:
        ...
        while True:
            ...
            # 매수 체결 조건
            if df.iloc[m]['low'] <= bprelay["bprelay"].iloc[
                m]:  # and (df.iloc[m]['high'] != df.iloc[m]['low']):  # 조건을 만족할 경우 spp 생성
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        if m > length:
            ...
            if limit_sell:
                ...
                sell_price = df['MA_SHORT'].iloc[m]
                # sell_price = df['hopen'].iloc[m]
    ...
    if len(profits) == 0:
        ...
        mf.candlestick_ochl(ax, ochl, width=0.5, colorup='limegreen', colordown='fuchsia')
        plt.xticks(fontsize=10)
        plt.yticks(fontsize=10)
        plt.plot(df['MA_SHORT'], color='orange')
        plt.plot(df['MA_LONG'], color='darkorange')
        plt.plot(df['HSUPPORT'], color='crimson')
        # df['UP_TREND'] = np.where(df['SUPERTREND'] == df['Lower Band'], df['SUPERTREND'], np.NaN)
        # df['DOWN_TREND'] = np.where(df['SUPERTREND'] == df['Upper Band'], df['SUPERTREND'], np.NaN)
        # plt.plot(df['UP_TREND'].values, color='green')
        # plt.plot(df['DOWN_TREND'].values, color='red')
        # plt.plot(df['ICHIMOKU'].values, color='deeppink')
        # plt.legend(loc='upper right', fontsize=5)
        plt.title('FISHER CHECKER - %s %.3f %.3f %.3f' % (Coin,
                                                          float(profits.iloc[-1]),
                                                          float(profits.iloc[-1]) / Minus_Profits, Minus_Profits),
                  color='black')
...
if __name__ == "__main__":
    ...
    TopCoin = ['FCT', 'QBZ', 'QKC', 'VALOR', 'MCO', 'DVP', 'FX', 'REP', 'LAMB', 'TRUE']
    TopCoin = ['VALOR', 'MCO', 'DVP', 'FX', 'REP', 'LAMB', 'TRUE']
