...
class BankModule(FuturesModule):
    ...
    def posmode_margin_leverage(self):
        while 1:  # for completion
            ...
            try:
                self.change_position_mode(dualSidePosition=True)
        ...
        while 1:  # <-- loop for complete open order
            try:
                ...
                if order_type == OrderType.MARKET:
                    # ------ market order ------ #
                    post_order_res = self.post_order(symbol=self.config.trader_set.symbol,
                                                     side=limit_side,
                    ...
                    post_order_res = self.post_order(timeInForce=TimeInForce.GTC, symbol=self.config.trader_set.symbol,
                                                     side=limit_side,
                                                     positionSide=pos_side,
    ...
    def partial_limit_order_v4(self, partial_tps, partial_qtys, close_side, pos_side, open_exec_qty, quantity_precision):
        ...
        while 1:
            ...
            # Todo
            #   1. 어떻게 대처해야할지 잘모르겠어서, 일단은 limit_order 로 회귀시켜놓음. -> 추후 원인파악 및 대응방안 설립.
            else:
                continue
    ...
    def market_close_order_v2(self, post_order_res_list, close_side, pos_side, open_exec_qty):
        while 1:  # <--- This loop for complete close
            ...
            if error_code in ['-2022', '-4003']:
                ...
                if post_order_res.origQty - market_exec_qty < 1 / (10 ** quantity_precision):
                    self.sys_log.info('market close order executed')
