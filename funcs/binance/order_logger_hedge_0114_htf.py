...
def partial_limit_order_v2(self, config, tp_list, close_side, pos_side, open_executedQty, quantity_precision, partial_qty_divider):
    tp_count = 0
    # retry_cnt = 0
    post_order_res_list = []
    ...
    while 1:  # loop for partial tp
        ...
        post_order_res, _, res_code = limit_order(self, config.tp_set.tp_type, close_side, pos_side, tp_level, quantity)
        post_order_res_list.append(post_order_res)
...
def market_close_order(self, remain_tp_canceled, post_order_res_list, close_side, pos_side, open_executedQty, out, log_tp):
    while 1:  # <--- This loop for complete close
