from funcs.binance.futures_modules import OrderSide, get_market_price_v2, \
...
def read_write_cfg_list(cfg_path_list, mode='r', edited_cfg_list=None):
    try:
        ...
        self.sys_log.error("error in read_write_cfg_list :", e)
...
def init_set(self):
    while 1:  # for completion
        ...
        try:
            ...
            self.sys_log.error('error in change_position_mode : {}'.format(e))
        else:
            self.sys_log.info('position mode --> hedge_mode')

        # ------ 1. leverage type => "isolated" ------ #
        try:
            request_client.change_margin_type(symbol=self.ticker,
        ...
        try:
            limit_leverage = get_limit_leverage(symbol_=self.ticker)
        except Exception as e:
            self.sys_log.error('error in get limit_leverage : {}'.format(e))
            continue
        else:
        ...
        # ------ 3. sub_client ------ #
        try:
            sub_client.subscribe_aggregate_trade_event(self.ticker.lower(), callback, error)
        except Exception as e:
            self.sys_log.error('error in get sub_client : {}'.format(e))
            continue
        else:
...
def get_open_side(self, res_df, np_timeidx):
    ...
    for utils_, cfg_ in zip(self.utils_list, self.config_list):
        ...
        if res_df['short_open_{}'.format(cfg_.selection_id)][cfg_.trader_set.latest_index] == cfg_.ep_set.short_entry_score:
            self.sys_log.warning("[ short ] ep_loc.point executed strat_ver : {}".format(cfg_.selection_id))
            #       ban      #
            if not cfg_.pos_set.short_ban:
                #       ep_loc      #
                res_df, open_side, _ = self.utils_public.short_ep_loc(res_df, cfg_,
                                                                ...
                                                                np_timeidx)
                self.sys_log.warning("open_side : {}".format(open_side))
                #       assign      #
        ...
        if res_df['long_open_{}'.format(cfg_.selection_id)][cfg_.trader_set.latest_index] == \
            ...
            self.sys_log.warning("[ long ] ep_loc.point executed strat_ver : {}".format(cfg_.selection_id))
            #       ban      #
            if not cfg_.pos_set.long_ban:  # additional const. on trader
                #       ep_loc      #
...
def get_new_df(self, calc_rows=True, mode="OPEN"):
    while 1:
        try:
            ...
            new_df_, _ = concat_candlestick(self.ticker, '1m',
                                            days=days,
                                            limit=use_rows,
                                            timesleep=None,
                                            show_process=0)
...
def check_breakout_qty(self, first_exec_qty_check, check_time, post_order_res, open_quantity):
    ...
    if first_exec_qty_check or time.time() - check_time >= self.config.trader_set.qty_check_term:
        order_info = get_order_info(self.ticker, post_order_res.orderId)
...
def get_p_tpqty(self, price_precision, quantity_precision):
    p_ranges, p_qty_ratio = literal_eval(self.p_ranges), literal_eval(self.p_qty_ratio)
    len_p = len(p_ranges)
    assert len_p == len(p_qty_ratio)
    en_ps, tps, open_qtys = [np.tile(arr_, (len_p,)) for arr_ in [self.ep, self.tp, self.open_executedQty]]

    if self.close_side == OrderSide.BUY:  # short
...
def check_limit_tp_exec_v2(self, post_order_res_list, quantity_precision, return_price=False):
    all_executed = 0
    order_info_list = [get_order_info(self.ticker, post_order_res.orderId)
...
#         # order_exec_check_list = [check_exec_by_order_info(self.ticker, post_order_res, quantity_precision, return_price, order_type)
#         #                          for post_order_res in post_order_res_list if post_order_res is not None]
#         order_info_list = [get_order_info(self.ticker, post_order_res.orderId)
#                            for post_order_res in post_order_res_list if post_order_res is not None]
#         order_exec_check_list = [check_execution(order_info, quantity_precision) for order_info in order_info_list]
...
def log_sub_tp_exec(self, res_df, tp_list, post_order_res_list, quantity_precision, tp_series_list, tp_exec_dict):
    if len(tp_list) > 1:  # allow only partial_tp
        ...
        #               i. exec_check logic
        order_exec_check_list = [check_exec_by_order_info(self.ticker, post_order_res, quantity_precision)
                                 for post_order_res in post_order_res_list if post_order_res is not None]
        if np.sum(order_exec_check_list) != 0:  # = limit_tp execution exist
            ...
            self.sys_log.info("executed_sub_prev_tp, executed_sub_tp : {}, {}"
            ...
            # tp_series_list.pop()
            self.sys_log.info("tp_list.pop() executed")
...
#     while 1:
#         try:
#             income = total_income(self.ticker, start_timestamp, end_timestamp)
#             self.accumulated_income += income
#             self.sys_log.info("income : {} USDT".format(income))
#         except Exception as e:
#             self.sys_log.error("error in total_income : {}".format(e))
#         else:
#             break
#
...
#
#     self.sys_log.info('calc_income : {} USDT'.format(real_balance * (real_profit - 1)))
#     self.sys_log.info('temporary profit : {:.2%} {:.2%} ({:.2%})'.format(tmp_profit, real_tmp_profit, ideal_tmp_profit))
...
#                         self.sys_log.info("market tp executed !")
#                     else:
#                         real_tp = tp
#                 else:
#                     if tp < res_df['open'].loc[k_ts]:
...
#
#         self.sys_log.info("real_tp : {}".format(real_tp))
#         self.sys_log.info("ep : {}".format(ep))
#         self.sys_log.info("fee : {}".format(fee))
#         self.sys_log.info("temp_qty : {}".format(temp_qty))
#
#         # ------------ save exit data ------------ #
...
def calc_ideal_profit_v2(self, open_side, ideal_ep, tp_exec_dict, open_executedPrice_list, close_executedPrice_list, res_df, fee, trade_log):
    ...
    for q_i, ((k_ts, v_tp), real_tp) in enumerate(zip(sorted(tp_exec_dict.items(), key=lambda x: x[0], reverse=True), close_executedPrice_list)):
        ...
        self.sys_log.info("------------ bills ------------")
        self.sys_log.info("ideal_ep : {}".format(ideal_ep))
        self.sys_log.info("ideal_tp : {}".format(ideal_tp))
        self.sys_log.info("real_ep : {}".format(real_ep))
        self.sys_log.info("real_tp : {}".format(real_tp))
        self.sys_log.info("fee : {}".format(fee))
        # self.sys_log.info("temp_qty : {}".format(temp_qty))
