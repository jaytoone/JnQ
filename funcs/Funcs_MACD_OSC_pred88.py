...
def profitage(Coin, short, long, signal, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    #     return 1.0, 1.0, 1.0
    # df = pybithumb.get_ohlcv(Coin, 'KRW', 'minute1')
    df = pd.read_excel('./BestSet/Pred_ohlc88/%s' % Coin, index_col=0)
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            #       고점 예측시 매도       #
            if df.iloc[m]['Y_pred'] == 1. or df.iloc[m]['trade_state'] == 3.:
                break
        ...
        elif df.iloc[m]['Y_pred'] == 1. or df.iloc[m]['trade_state'] == 3.:
...
if __name__=="__main__":
    ...
    excel_list = os.listdir('./BestSet/Pred_ohlc88/')
    ...
    for Coin in excel_list:
        result = profitage(Coin, short, long, signal, Date=Date, get_fig=get_fig)
