...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    if not Coin.endswith('.xlsx'):
        df = pybithumb.get_ohlcv(Coin, 'KRW', 'minute5')
    ...
    # df['KST_GAUSSIAN'] = smoothed_curve
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            if df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3:
                ...
                # #       자잘거림 보정     #
                # if abs(df['KST'].iloc[m] - df['KST'].iloc[start_m]) < 1:
                #     pass
                # else:
                break
            ...
            #       매수 시그널의 KST 값보다 작아지면 손절        #
            if m != buy_signal_m and df['KST'].iloc[buy_signal_m] > df['KST'].iloc[m]:
                exit_switch = True
                break
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['IOST', 'ZIL', 'ELF', 'APIX', 'FNB', 'FX', 'RNT', 'LRC', 'ZRX', 'KNC']
    TopCoin = ['PCM', 'ZIL', 'FLETA', 'FX', 'VET', 'RNT']
