...
def profitage(df, second_df, third_df, fourth_df=None, symbol=None, date=None, save_path=None, excel=0, get_fig=0, only_profit=False, show_time=False, label_type='Profit'):
    ...
    third_df['b_upper'], third_df['b_lower'], third_df['bbw'] = bb_width(third_df, 20, 2)
    df = df.join(pd.DataFrame(index=df.index, data=to_lower_tf(df, third_df, [i for i in range(-3, 0, 1)]),
    ...
    for i in range(ichimoku_lookback, len(df) - 1):
        ...
        if df['fisher_state'].iloc[i] == 1:
            ...
            #             continue
                ...
                #           Ichimoku Set            #
                # if df['close'].iloc[i - ichimoku_lookback:i].min() >= np.maximum(df['senkou_a'].iloc[i], df['senkou_b'].iloc[i]) or \
                #         df['close'].iloc[i - ichimoku_lookback:i].max() <= np.minimum(df['senkou_a'].iloc[i], df['senkou_b'].iloc[i]):#and \
