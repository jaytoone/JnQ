...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    ...
    df['Fisher_long_ma'] = df.Fisher_long.rolling(30).mean()
    df['Fisher_short_ma'] = df.Fisher_short.rolling(30).mean()
    ...
    for i in range(1, len(df)):
        ...
        if df['Fisher_long_ma'].iloc[i] <= long_signal_value:
            if df['Fisher_long_ma'].iloc[i] > df['Fisher_long_ma'].iloc[i - 1] and df['MA_slope'].iloc[i] >= slope_value:
                trade_state[i] = 1

        # if df['Fisher_long_ma'].iloc[i] > df['Fisher_long_ma'].iloc[i - 1]:
        #     trade_state[i] = 1

        # if df['Fisher_long'].iloc[i] <= long_signal_value:
        #     trade_state[i] = 1

        # elif df['Fisher_short_ma'].iloc[i] >= long_signal_value:
        #     if df['Fisher_short_ma'].iloc[i] < df['Fisher_short_ma'].iloc[i - 1]:
        #         trade_state[i] = 2

        # elif df['Fisher_short_ma'].iloc[i] < df['Fisher_short_ma'].iloc[i - 1]:
        #         trade_state[i] = 2
            ...
            for high_passed_signal_value in [1.0]:
                if df['Fisher_short'].iloc[i - 1] >= high_passed_signal_value > df['Fisher_short'].iloc[i]:
                    trade_state[i] = 3
    ...
    #
    # plt.subplot(313)
    # plt.plot(df.Fisher_long_ma.values)
    #
    # for i in range(len(span_list)):
    #     plt.axvspan(span_list[i][0], span_list[i][1], facecolor='c', alpha=0.7)
    # for i in range(len(span_list2)):
