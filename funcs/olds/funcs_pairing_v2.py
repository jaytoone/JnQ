...
def get_res_v10(res_df, open_info_df_list, ohlc_list, config_list, np_timeidx, funcs, inversion=False, test_ratio=0.3, plot_is=True, signi=False, show_detail=False):
    ...
    # ------------ get paired_res ------------ #
    start_0 = time.time()
    paired_res = en_ex_pairing_v10(res_df, [sample_open_idx1, sample_open_idx2], [open_info1, open_info2], ohlc_list, config_list, np_timeidx, funcs, show_detail)
...
def en_ex_pairing_v10(res_df, open_idx_list, open_info_list, ohlc_list, config_list, np_timeidx, funcs,
    ...
    net_p1_idx_list, p1_idx_list, p2_idx_list, pair_idx_list, pair_price_list, lvrg_list, fee_list, tpout_list, tr_list = [
        [] for li in range(9)]
    len_df = len(res_df)
...
def check_entry_v7(res_df, config, entry_type, op_idx1, op_idx2, wave1, wave_gap, len_df, open_side, np_datas, expiry):
    ...
    """
    expiry() 에 wave1, wave_gap 을 없애면서, tp 를 기준한 expiration 사용 <-> 기존은 wave_1 기준이였음.
    """
