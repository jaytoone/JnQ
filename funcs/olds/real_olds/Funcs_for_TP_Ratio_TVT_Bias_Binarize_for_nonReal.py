...
def profitage(df, second_df, third_df, symbol=None, date=None, excel=0, get_fig=0, show_time=False, label_type='Test'):
    ...
    df = df.join(pd.DataFrame(index=df.index, data=to_lower_tf(df, third_df, [i for i in range(-9, 0, 1)]),
    ...
    df = df.join(pd.DataFrame(index=df.index, data=to_lower_tf(df, second_df, [-1]), columns=['minor_Trix']))
    ...
    tp_level_list, sl_level_list = list(), list()
    ...
    if label_type == 'Train':
        ...
        for i in range(tick_in_box, len(df)):
            ...
            #           Label Balancing         #
            if df['ep'].iloc[i] + (df['long_tp'].iloc[i] - df['ep'].iloc[i]) * 3 / 5 <= df['high'].max() and \
                    df['ep'].iloc[i] + (df['short_tp'].iloc[i] - df['ep'].iloc[i]) * 3 / 5 >= df['low'].min():
                pass
            else:
                continue
            ...
            if df['box_low'].iloc[i] + abs(df['box_high'].iloc[i] - df['box_low'].iloc[i]) * \
                ...
                for j in range(i + 1, len(df)):
                    ...
                    if df['low'].iloc[j] < df['long_sl'].iloc[i]:
                        ...
                        #           up level diversify      #
                        # if max_high > df['ep'].iloc[i] + (df['long_tp'].iloc[i] - df['ep'].iloc[i]) * 5 / 5:
                        #     df['trade_state'].iloc[i] = 4
                        #     long_tp_marker_x.append(i), long_tp_marker_y.append(df['ep'].iloc[i])
                        # elif max_high > df['ep'].iloc[i] + (df['long_tp'].iloc[i] - df['ep'].iloc[i]) * 4 / 5:
                        #     df['trade_state'].iloc[i] = 3
                        #     long_tp_marker_x.append(i), long_tp_marker_y.append(df['ep'].iloc[i])
                        if max_high > df['ep'].iloc[i] + (df['long_tp'].iloc[i] - df['ep'].iloc[i]) * 3 / 5:
                            df['trade_state'].iloc[i] = 1
                            long_tp_marker_x.append(i), long_tp_marker_y.append(df['ep'].iloc[i])
                            # tp_level_list.append(df['ep'].iloc[i] + (df['long_tp'].iloc[i] - df['ep'].iloc[i]) * 3 / 5)
                            # sl_level_list.append(df['long_sl'].iloc[i])
                        # elif max_high > df['ep'].iloc[i] + (df['long_tp'].iloc[i] - df['ep'].iloc[i]) * 2 / 5:
                        #     df['trade_state'].iloc[i] = 1
                        #     long_tp_marker_x.append(i), long_tp_marker_y.append(df['ep'].iloc[i])
                        # elif max_high > df['ep'].iloc[i] + (df['long_tp'].iloc[i] - df['ep'].iloc[i]) * 1 / 5:
            ...
            #             #           down level diversify      #
            #             # if min_low < df['ep'].iloc[i] - abs(df['short_tp'].iloc[i] - df['ep'].iloc[i]) * 5 / 5:
            #             #     df['trade_state'].iloc[i] = 9
            #             #     short_tp_marker_x.append(i), short_tp_marker_y.append(df['ep'].iloc[i])
            #             # elif min_low < df['ep'].iloc[i] - abs(df['short_tp'].iloc[i] - df['ep'].iloc[i]) * 4 / 5:
            #             #     df['trade_state'].iloc[i] = 8
            #             #     short_tp_marker_x.append(i), short_tp_marker_y.append(df['ep'].iloc[i])
            #             if min_low < df['ep'].iloc[i] - abs(df['short_tp'].iloc[i] - df['ep'].iloc[i]) * 3 / 5:
            #                 df['trade_state'].iloc[i] = 1
            #                 short_tp_marker_x.append(i), short_tp_marker_y.append(df['ep'].iloc[i])
            #             # elif min_low < df['ep'].iloc[i] - abs(df['short_tp'].iloc[i] - df['ep'].iloc[i]) * 2 / 5:
            #             #     df['trade_state'].iloc[i] = 6
            #             #     short_tp_marker_x.append(i), short_tp_marker_y.append(df['ep'].iloc[i])
            #             # elif min_low < df['ep'].iloc[i] - abs(df['short_tp'].iloc[i] - df['ep'].iloc[i]) * 1 / 5:
            #             #     df['trade_state'].iloc[i] = 5
            #             #     short_tp_marker_x.append(i), short_tp_marker_y.append(df['ep'].iloc[i])
            #             else:
            #                 df['trade_state'].iloc[i] = 0
            #             break
        ...
        if get_fig:
            try:
                ...
                # plt.plot(long_tp_marker_x, tp_level_list, 'o', color='green')
                # plt.plot(long_tp_marker_x, sl_level_list, 'o', color='orange')
