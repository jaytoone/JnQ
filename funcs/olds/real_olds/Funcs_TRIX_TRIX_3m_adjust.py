...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    df['SCALED_FISHER'] = min_max_scaler(TA.FISH(df, period=9))
    ...
    for i in range(2, len(df)):
            ...
            if df['TRIX_TREND'].iloc[i] == 'UP' and df['TRIX_TREND'].iloc[i - 1] == 'DOWN':
                ...
                while True:
                    ...
                    if df['TRIX_TREND'].iloc[copy_i] != df['TRIX_TREND'].iloc[copy_i - 1]:
                        break
                print('TRIX ENTRY VERTICAL GAP :', i,
                ...
                if buy_signal:
                    ...
                    #       1개 이상일 경우 진입 가능     #
                        ...
                        #     while True:
                        #         copy_i -= 1
                        #         if df['DC_LOW'].iloc[copy_i] != df['DC_LOW'].iloc[copy_i - 1]:
                        #             break
                        #         if copy_i == 0:
                        #             break
                        #
                        #     if copy_i != 0:
                        #         for j in range(copy_i, i):
                        #             if trade_state[j] == 1:
                        #                 print('copy_i :', copy_i)
                        #                 print('TRADE STATE = 1 AT %s, %s WHILE DC DOWN' % (j, i))
                        #                 trade_state[i] = 1.5
                        #                 break
        ...
        if df['FISHER_TRIX_TREND'].iloc[i - 1] == 'UP' and df['FISHER_TRIX_TREND'].iloc[i] == 'DOWN':
            trade_state[i] = 2
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            if df.trade_state.iloc[m] == 2:
                ...
                # print('FISHER_TRIX GAP:', m, (df['FISHER_TRIX'].iloc[m] - df['FISHER_TRIX'].iloc[start_m]))
                ...
                if m - start_m > trix_horizontal_ticks:
                    ...
                    print('TRIX EXIT VERTICAL GAP:', buy_signal_m, m, (df['TRIX'].iloc[m] - df['TRIX'].iloc[buy_signal_m]) / (df['TRIX'].max() - df['TRIX'].min()))
                    if abs(df['TRIX'].iloc[m] - df['TRIX'].iloc[buy_signal_m]) / (df['TRIX'].max() - df['TRIX'].min()) < trix_exit_vertical_gap:
                        pass
                    else:
                        sell_switch = True
                        break
                ...
                #     while True:
                #         copy_m -= 1
                #         if df['FISHER_TRIX_TREND'].iloc[copy_m] != df['FISHER_TRIX_TREND'].iloc[copy_m - 1]:
                #             break
                #
                #     if abs(df['FISHER_TRIX'].iloc[m] - df['FISHER_TRIX'].iloc[copy_m]) < FISHER_trix_exit_vertical_gap:
                #         pass
                #     else:
                #         sell_switch = True
                #         break
