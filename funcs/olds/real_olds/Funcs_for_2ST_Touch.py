...
def profitage(Coin, period1=18, period2=60, period3=120, wait_tick=3, Date='2019-09-25', excel=0, get_fig=0):
    global length
    ...
    for i in range(2, len(df)):
        ...
        #           Long            #
        #           HM            #
        if (df['minor_ST2_Trend'].iloc[i] == 1 and df['minor_ST3_Trend'].iloc[i] == 1) or \
                (df['minor_ST1_Trend'].iloc[i] == 1 and df['minor_ST2_Trend'].iloc[i] == 1):
            if df['30EMA'].iloc[i] < df['close'].iloc[i] and df['100EMA'].iloc[i] < df['close'].iloc[i]:
                #           TC          #
                if (df['EMA_CB'].iloc[i] > 50 and df['EMA_CB'].iloc[i - 1] < 50) or \
                        (df['Fisher1'].iloc[i] > 0 and df['Fisher1'].iloc[i - 1] < 0):
                    #           Stable EP           #
                    if 25 < df['ST_Osc'].iloc[i] < 75:
                        trade_state[i] = 1
                        long_marker_x.append(i)
                        long_marker_y.append(df['EMA_CB'].iloc[i])
            ...
            if df['30EMA'].iloc[i] >= df['close'].iloc[i] and df['100EMA'].iloc[i] >= df['close'].iloc[i]:
                ...
                if (df['EMA_CB'].iloc[i] < 50 and df['EMA_CB'].iloc[i - 1] > 50) or \
                    ...
                    if 25 < df['ST_Osc'].iloc[i] < 75:
                        ...
                        short_marker_y.append(df['EMA_CB'].iloc[i])
...
if __name__ == "__main__":
    ...
    for Coin in TopCoin:
        start = time.time()
        # try:
        result = profitage(Coin, Date=Date, get_fig=1, excel=1)
        # quit()
        print(Coin, result)
