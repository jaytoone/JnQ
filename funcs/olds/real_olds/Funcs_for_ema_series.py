...
def profitage(df, second_df, third_df, symbol=None, date=None, excel=0, get_fig=0, show_time=False, label_type='Test'):
    ...
    df['ema1'] = ema(df['close'], 50)
    df['ema2'] = ema(df['close'], 100)
    df['ema3'] = ema(df['close'], 150)
    ...
    for i in range(ema_lookback, len(df)):
        ...
        if df['bbw'].iloc[i] > 0.015:
            ...
            if df['ema1'].iloc[i] > df['ema2'].iloc[i] > df['ema3'].iloc[i]:
                ...
                #           EMA Set         #
                if df['ema1'].iloc[i - 1] >= df['close'].iloc[i - 1] and \
                        df['ema1'].iloc[i] < df['close'].iloc[i]:

                    for j in range(i + 1 - ema_lookback, i + 1):

                        if df['ema1'].iloc[j - 1] < df['close'].iloc[j - 1] and \
                                df['ema1'].iloc[j] >= df['close'].iloc[j]:
                            ...
                            # ----------- SL ----------- #
                            #           by prev low        #
                            df['sl_level'].iloc[i] = df['low'].iloc[j:i + 1].min()

                            #           by ema2         #
                            # df['sl_level'].iloc[i] = df['ema2'].iloc[i]
                ...
                if df['ema1'].iloc[i - 1] <= df['close'].iloc[i - 1] and \
                    ...
                    for j in range(i + 1 - ema_lookback, i + 1):
                        ...
                        if df['ema1'].iloc[j - 1] > df['close'].iloc[j - 1] and \
                            ...
                            # ----------- SL ----------- #
                            #           by prev high        #
                            df['sl_level'].iloc[i] = df['high'].iloc[j:i + 1].max()
    ...
    while m <= length:
        ...
        while True:
            ...
            if order_type == 'Long':
                ...
                #           By fisher           #
                # if df['fisher'].iloc[m] < df['fisher'].iloc[m - 1] >= 1.5:
                #     TP_switch = True
    ...
    if len(profits) == 0:
        ...
        #       indicator       #
        plt.plot(df['ema1'], 'limegreen')
        plt.plot(df['ema2'], 'deepskyblue')
        plt.plot(df['ema3'], 'fuchsia')
        ...
        # plot 저장 & 닫기
        plt.show()
        # plt.savefig("./chart_check/%s.png" % symbol, dpi=300)
        #
