...
def profitage(Coin, input_data_length, model_num, wait_tick=3, check_span=10, Date='2019-09-25', excel=0):
    ...
    macd(df, short=5, long=35, signal=5)
    df['MACD2'] = df['close'].ewm(span=19, min_periods=19 - 1, adjust=False).mean() - \
                 df['close'].ewm(span=39, min_periods=39 - 1, adjust=False).mean()
    df['MACD_Signal2'] = df['MACD2'].ewm(span=9, min_periods=9 - 1, adjust=False).mean()
    df['MACD_OSC2'] = df.MACD2 - df.MACD_Signal2
    ...
    for i in range(2, len(df)):
        ...
        if df['MACD_OSC2'][i - 2] >= 0:
            if df['MACD_OSC2'][i - 1] < 0:
                trade_state[i] = 2

    df['trade_state'] = trade_state

    # 매수 시점 = 급등 예상시, 매수가 = 이전 종가
    df['BuyPrice'] = np.where(df['trade_state'] == 1., df['open'], np.nan)
    ...
    if len(profits) == 0:
        ...
        # plt.plot(df[['EMA_1']], 'g', label='ema1', linewidth=1.0)
        # plt.plot(df[['EMA_2']], 'r', label='ema2', linewidth=1.0)
        ...
        # plot 저장 & 닫기
        plt.show()
        # plt.savefig("./Figure_pred/%s_%s_profit/%s %s.png" % (input_data_length, model_num, Date, Coin), dpi=300)
        # plt.close()
...
if __name__=="__main__":
    ...
    #     #       Make folder      #
    #     try:
    #         os.mkdir('./Figure_pred/%s_%s_profit/' % (input_data_length, model_num))
    #
    #     except Exception as e:
    #         pass

    print(profitage(Coin, input_data_length, model_num, wait_tick, Date=Date, excel=0))
