...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    df['MA_TREND'] = np.where(df['MA_LONG'] > df['MA_LONG'].shift(1), 'UP', 'DOWN')
    ...
    df['FISHER_MA'] = df['FISHER_LONG'].rolling(120).mean()
    # print(df['FISHER_LONG'].tail())
    # quit()
    ...
    for i in range(2, len(df)):
        ...
        # if df['FISHER_LONG'].iloc[i - 1] < 0 and df['FISHER_LONG'].iloc[i] >= 0:
        #     trade_state[i] = 1.5
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            # if df['FISHER_LONG'].iloc[m] < -1.:
            #     sell_switch = True
            #     break

            # if df['close'].iloc[m] < df['MA_LONG'].iloc[m]:
            #     sell_switch = True
            #     break
        ...
        if m > length:
            ...
            #       매수 체결 후 바로 매도 체결 되어버리는 경우       #
            if limit_sell:
                sell_price = df['DC_LOW'].iloc[buy_signal_m] + chart_height * sell_price_gap
            else:
...
if __name__ == "__main__":
    ...
    TopCoin = ['SXP', 'COSM', 'CHR', 'MXC', 'BCD', 'CTXC', 'WAXP', 'WICC', 'OGO', 'FLETA']
    TopCoin = [ 'CHR', 'MXC', 'BCD', 'CTXC', 'WAXP', 'WICC', 'OGO', 'FLETA']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = -1.5, 1.5
                long_period, short_period = 30, 60
