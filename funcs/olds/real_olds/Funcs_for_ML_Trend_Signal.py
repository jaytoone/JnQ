...
def profitage(df, second_df, third_df, symbol=None, date=None, excel=0, get_fig=0, show_time=False, label_type='Test'):
    ...
    df['Trend'] = np.nan
    df['Tr'] = np.nan
    min_cp = df['close'].rolling(3).min().shift(-2)
    max_cp = df['close'].rolling(3).max().shift(-2)
    df['Tr'] = (df['close'] - min_cp) / (max_cp - min_cp) * .5

    if label_type == 'Train':

        startTime = time.time()
        for i in range(ma_lookback, len(df)):

            if df['close'].iloc[i] > df['MA'].iloc[i]:
                if list(df['MA'].iloc[i - ma_lookback:i]) == sorted(list(df['MA'].iloc[i - ma_lookback:i])):
                    df['Trend'].iloc[i] = 1

            else:
                if list(df['MA'].iloc[i - ma_lookback:i]) == sorted(list(df['MA'].iloc[i - ma_lookback:i]), reverse=True):
                    df['Trend'].iloc[i] = 0

        df['Tr'] = np.where(df['Trend'] == 1, df['Tr'] + .5, df['Tr'])

        for i in range(ma_lookback, len(df)):

            #           Crossover           #
            if df['Tr'].iloc[i] > .5 and df['Tr'].iloc[i - 1] <= .5:
                df['trade_state'].iloc[i] = 1

            #           Crossunder          #
            elif df['Tr'].iloc[i] <= .5 and df['Tr'].iloc[i - 1] > .5:
