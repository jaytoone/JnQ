...
def profitage(Coin, short, long, signal, model, Date='2019-09-25', excel=0, get_fig=0):
    # try:
    ...
    #     else:
    #         span_list_down.append((i, i + 1))
    #
    # #
    # for i in range(len(span_list)):
    #     plt.axvspan(span_list[i][0], span_list[i][1], facecolor='b', alpha=0.7)
    # # for i in range(len(span_list_down)):
    # #     plt.axvspan(span_list_down[i][0], span_list_down[i][1], facecolor='m', alpha=0.7)
    #
    # plt.subplot(312)
    # # plt.plot(close_reg)
    ...
    #
    # plt.show()
    # # plt.savefig('figure_reg/%s.png' % end_index)
    # plt.close()
    # quit()
    ...
    df['BuyPrice'] = np.where((df['trade_state'].shift(1) == 'down') & (df['trade_state'] == 'up'), df['close'], np.nan)
...
if __name__ == "__main__":
    ...
    model = SVR(kernel='rbf', C=500., gamma=20., epsilon=1.)     # kernel='rbf', C=100, gamma=10, epsilon=0.1
    #  500.0   20.0     1.00
    #  0.1  100.0     0.50
        ...
        for long in range(short + 3, short + 100, 5):
            for signal in range(5, 50, 3):
                ...
                for Coin in TopCoin:
                    # print(Coin)
                    # Coin = excel_list[3]
                    print(Coin, profitage(Coin, short, long, signal, model, Date=Date, get_fig=1, excel=0))
