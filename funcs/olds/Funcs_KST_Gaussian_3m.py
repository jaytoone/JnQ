...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    for i in range(2, len(df)):
        ...
        #     trade_state[i] = 1.5
        if df['KST_GAUSSIAN_TREND'].iloc[i - 1] == 'DOWN' and df['KST_GAUSSIAN_TREND'].iloc[i] == 'UP':
                # if upper_line > df['KST_GAUSSIAN'].iloc[i] >= below_line:
                    if df['KST'].iloc[i] != 0:  # 초기에 진입하는거를 방지하고 KST와 비교하기 위해서

                        # #   이전 변곡점과의 높이차이가 10이상    #
                        # copy_i = i
                        # while True:
                        #     copy_i -= 1
                        #     if df['KST_GAUSSIAN_TREND'].iloc[copy_i - 1] == 'UP' and df['KST_GAUSSIAN_TREND'].iloc[copy_i] == 'DOWN':
                        #         break
                        #     if copy_i == 0:
                        #         break
                        #
                        # if copy_i != 0:
                        #     if df['KST_GAUSSIAN'].iloc[copy_i] - df['KST_GAUSSIAN'].iloc[i] >= kst_vertical_gap:
                        #         print('KST_GAUSSIAN GAP :', i, df['KST_GAUSSIAN'].iloc[copy_i] - df['KST_GAUSSIAN'].iloc[i])
                                trade_state[i] = 1.5
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            if df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3:
                ...
                #       자잘거림 보정     #
                if abs(df['KST_GAUSSIAN'].iloc[m] - df['KST_GAUSSIAN'].iloc[start_m]) < kst_horizontal_gap:
                    pass
                else:
                    break
    ...
    if len(profits) == 0:
        ...
        plt.title('KST GAUSSIAN - %s %.3f %.3f %.3f' % (Coin,
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['ZIL', 'APIX', 'BZNT', 'WOM', 'KNC', 'VET', 'HYC']
