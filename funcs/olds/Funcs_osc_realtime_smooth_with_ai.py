...
def low_high(reg_target, sigma, crop_size=0, realtime=0):
    ...
    model = load_model('./model/rapid_ascending %s_%s_ohlc.hdf5' % (10, 112))

    #               for realtime svr            #
    if crop_size != 0:

        realtime_y_smooth = [np.nan] * len(reg_target)
        realtime_y_pred = [np.nan] * len(reg_target)
        for i in range(none_indexs + crop_size, len(reg_target)):
            ...
            #           pulling chart data curve / osc      #
            y_pred = model.predict(y_smooth.reshape(-1, 10, 1))

            print(i)
            realtime_y_smooth[i] = y_smooth[-1]
            realtime_y_pred[i] = y_pred[-1]
        ...
        if realtime == 0:
            for i in range(len(realtime_y_smooth)):
                ...
                if realtime_y_pred[i - 1] < realtime_y_pred[i]:
                    trade_state[i] = 'up'
                else:
                    trade_state[i] = 'down'
        ...
        return realtime_y_pred, trade_state
...
def profitage(Coin, reg_target, short, long, signal, sigma, crop_size, realtime, Date='2019-09-25', excel=0, get_fig=0):
    ...
    while m <= length:
        ...
        while True:
            ...
            if (df['trade_state'].iloc[m - 5] == 'down') and (df['trade_state'].iloc[m] == 'up') and (df['trade_state'].iloc[m - 1] == 'up'):
                print('매수 등록')
                break
    ...
    if len(profits) == 0:
        ...
        plt.subplot(312)
        plt.plot(osc_reg, 'red', label='oscillator', linewidth=1.0)
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    # TopCoin = ['MIX', 'XLM', 'APIS', 'ZEC', 'ZIL', 'WICC', 'TRV', 'ETH', 'EOS', 'LUNA']
    TopCoin = ['LUNA']
