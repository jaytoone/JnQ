...
def low_high(reg_target, model, crop_size=0):
    ...
    if crop_size != 0:
        ...
        for i in range(none_indexs + crop_size, len(reg_target)):
            ...
            x = np.arange(len(reg_target))[none_indexs:i].reshape(-1, 1)
            y = reg_target.values[none_indexs:i].reshape(-1, 1)
...
def profitage(Coin, short, long, signal, model, crop_size, Date='2019-09-25', excel=0, get_fig=0):
    ...
    if len(profits) == 0:
        ...
        # plot 저장 & 닫기
        plt.show()
        # plt.savefig("./figure_reg/reg_result/%s %s %s.png" % (Date, Coin, crop_size), dpi=300)
        # plt.close()
...
if __name__ == "__main__":
    ...
    C, gamma, epsilon = 0.1, 100, 0.5
    # model = SVR(kernel='rbf', C=C, gamma=gamma, epsilon=epsilon)     # kernel='rbf', C=100, gamma=10, epsilon=0.1
    model = SVR(kernel='rbf', C=1000, epsilon=0.1)     # kernel='rbf', C=100, gamma=10, epsilon=0.1
    #  500, 20, 1
    #  0.1, 100, 0.5
        ...
        for long in range(short + 3, short + 100, 5):
            for signal in range(5, 50, 3):
                ...
                for Coin in TopCoin:
                    ...
                    # Coin = excel_list[3]
                    #                                                   crop_size = base_size       #
                    print(Coin, profitage(Coin, short, long, signal, model, crop_size=200, Date=Date, get_fig=1, excel=0))
                quit()
