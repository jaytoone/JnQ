...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    if not Coin.endswith('.xlsx'):
        df = pybithumb.get_ohlcv(Coin, 'KRW', 'minute1')
        df_short = pybithumb.get_ohlcv(Coin, 'KRW', 'minute3')
    ...
    for i in range(period, len(df)):
        ...
        y_short = df['Fisher_short'].values[i + 1 - period:i + 1]
    ...
    # df['Fisher_long_gaussian'] = smoothed_curve
    df['Fisher_short_gaussian'] = smoothed_curve_short

    # long_signal_value = 0
    # short_signal_value = 2.0
    long_ic_signal_value = -2.0
    detect_close_value = 0.0
    high_check_value = 2.0
    ...
    for i in range(2, len(df)):
        ...
        #             and df['close'].iloc[i] > clearance(df['SMMA'].iloc[i] * accept_range):
            # if df['Fisher_long_gaussian'].iloc[i] > df['Fisher_long_gaussian'].iloc[i - 1] < df['Fisher_long_gaussian'].iloc[i - 2] \
            #         and df['close'].iloc[i] > clearance(df['SMMA'].iloc[i] * accept_range):
            #     trade_state[i] = 1

        if df['Fisher_long'].iloc[i] <= long_signal_value < df['Fisher_long'].iloc[i - 1]: #and df['close'].iloc[i] > clearance(df['SMMA'].iloc[i] * accept_range):
            for j in range(i + 1, len(df)):
                if df['Fisher_long'].iloc[j] > df['Fisher_long'].iloc[j - 1] < df['Fisher_long'].iloc[j - 2] \
                        and df['SMMA_DC_GAP'].iloc[j - 1] >= accept_range:
                        # and df['DC_TREND'].iloc[j] == 'UP':

                    trade_state[j] = 1
                    #           3M FISHER LONG 변곡점 탐색           #
                    for k in range(j, len(df)):

                        if k != j:
                            trade_state[k] = 1.25

                        # if df['Fisher_long_ic_TREND'].iloc[k] == 'UP' and df['Fisher_long_ic_TREND'].iloc[k - 1] ==
                        # 'DOWN' \
                        # if df['Fisher_long_ic_TREND'].iloc[k] == 'UP' \
                        # if df['Fisher_short_gaussian'].iloc[k] > df['Fisher_short_gaussian'].iloc[k - 1] \
                        #         and df['Fisher_long_ic'].iloc[k - 1] > long_ic_signal_value:
                        #
                        #     #       이전 변곡점 조사       #
                        #     #       본래는 FISHER LONG IC GAUSSIAN을 사용하는게 맞는게 LONG IC = SHORT 이니까 편의상 사용    #
                        #     k_copy = k
                        #     while True:
                        #         k_copy -= 1
                        #         if df['Fisher_short_gaussian'].iloc[k_copy - 1] > df['Fisher_short_gaussian'].iloc[k_copy]:
                        #             break
                        #
                        #     #       이전 변곡점이 long ic singal value 이상이면 진입        #
                        #     if df['Fisher_short_gaussian'].iloc[k_copy] >= long_ic_signal_value:
                        #         if k + 2 < len(df):
                        #             while True:
                        #                 if df['Fisher_long_ic'].iloc[k] != df['Fisher_long_ic'].iloc[k + 1]:
                        #                     break
                        #                 else:
                        #                     k += 1
                        #             if df['SMMA_DC_GAP'].iloc[k] >= accept_range:
                        #                 trade_state[k] = 1.5
                        #     break
                        if df['Fisher_short_gaussian'].iloc[k] > df['Fisher_short_gaussian'].iloc[k - 1] >= df['Fisher_short_gaussian'].iloc[k - 2]:
                            if k + 2 < len(df):
                                while True:
                                    if df['Fisher_long_ic'].iloc[k] != df['Fisher_long_ic'].iloc[k + 1]:
                                        break
                        ...
                        #       탐색 종료 시점     #
                        if df['Fisher_long'].iloc[k - 1] >= detect_close_value:
                            break

                if df['Fisher_long'].iloc[j - 1] > long_signal_value:
                    break

        # if df['Fisher_short'].iloc[i] >= short_signal_value:
        #     trade_state[i] = 2
        ...
        # for high_passed_signal_value in [2.5, 2.3, 2.0]:
        #     if df['Fisher_short'].iloc[i - 1] >= high_passed_signal_value > \
        #             df['Fisher_short'].iloc[i]:
        #         trade_state[i] = 3
    ...
    for i in range(len(target)):
        if target.iloc[i] in [1., 1.25]:
            span_list_target.append((i, i + 1))
            if target.iloc[i] == 1.:
                print(i, df['SMMA_DC_GAP'].iloc[i])
        elif target.iloc[i] == 1.5:
    ...
    # df['BuyPrice'] = np.where(df['trade_state'].shift(1) == 1., df['close'], np.nan)
    df['BuyPrice'] = np.where(df['trade_state'].shift(1) == 1.5, df['close'].shift(1), np.nan)
    ...
    while m <= length:
        ...
        # #       이전 SHORT 와 현재 매수 포인트 사이에 또다른 매수 포인트가 존재하면 진입하지 않는다.     #
        # #       이전 SHORT 찾기     #
        # short_m = m
        # while True:
        #     short_m -= 1
        #     if short_m == 0:
        #         break # 밖에서 continue
        #     if df['Fisher_short'].iloc[short_m] > short_signal_value:
        #         break
        #
        # if short_m == 0:
        #     m += 1
        #     continue
        #
        # #       또다른 매수 포인트라함은 CROSSUNDER 가 되겠다.     #
        # cross_under_cnt = 0
        # #       현재 매수 포인트와 이전 SHORT에 존재여부 확인        #
        # #       존재하면, 매수신호를 지워준다.       #
        # # print(short_m, m)
        # for index in range(short_m, m):
        #     if df['Fisher_long'].iloc[index] < long_signal_value <= df['Fisher_long'].iloc[index - 1]:
        #         cross_under_cnt += 1
        #         # print('cross_under_cnt at :', index)
        #
        # if cross_under_cnt > 1:
        #     while True:
        #         #       trade_state == 1 조건을 위와 같이 명시했기에 아래와 같이 서술한다.       #
        #         if df['Fisher_long'].iloc[m] > long_signal_value:
        #             m += 1
        #             break
        #         m += 1
        #         # print(m)
        ...
        #           지지선 손절가 찾아주기        #
        #   매수 등록 M 기준 이전 이후로 FISHER 고점 변곡점 찾기
        #   이전 고점 변곡점   #
        ip_before, ip_after = None, None
        ...
        #     if ip_m == 0:
        #         break  # & 밖에서 continue
        #     if df['Fisher_IP'].iloc[ip_m] == 1 and df['Fisher_trend'].iloc[ip_m] == 'DOWN':
        #         ip_before = ip_m
        #         break
        #
        # if ip_m == 0:
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            #       매도 시그널       #
            if df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3:
                break

            #       손절 시그널       #

            #       고점 지나고 SHORT 찍을 가망이 없어 보이는 경우       #
            if df['Fisher_long'].iloc[m] >= high_check_value and exit_signal == 0:
                exit_signal = 1

            elif df['Fisher_long'].iloc[m] >= high_check_value2:
                exit_signal = 1.5

            if exit_signal == 1 and df['Fisher_long'].iloc[m] < 0:
                exit_switch = True
                break

            elif exit_signal == 1.5 and df['Fisher_long'].iloc[m] < 1.2:
                exit_switch = True
                break
            ...
            #       진입 3M FISHER 보다 FISHER가 낮아지면 손절     #
            # if df['Fisher_long_ic'].iloc[m] < df['Fisher_long_ic'].iloc[start_m]:
            #     exit_switch = True
            #     while True:
            #         m += 1
            #         fisher_ic_cnt += 1
            ...
            #        종가가 지지선을 깨면
            # elif df['close'].iloc[m] < support['support_line'].iloc[m] * 0.9975:
            #     exit_switch = True
            #     break
            # # print('exit signal found at ', m)
            #     while True:
            #         m += 1
            #         if m > length:
            #             break
            #         condition.iloc[m] = '손절 대기'
            #         bprelay["bprelay"].iloc[m] = bprelay["bprelay"].iloc[m - 1]
            #
            #         if df['Fisher_long'].iloc[m] > df['Fisher_long'].iloc[m - 1]:
            #             while True:
            #                 m += 1
            #                 if m > length:
            #                     break
            ...
            #                 bprelay["bprelay"].iloc[m] = bprelay["bprelay"].iloc[m - 1]
            #
            #                 # print(m, df['Fisher_long'].iloc[m] < df['Fisher_long'].iloc[m - 1], df['Fisher_long'].iloc[m])
            #
            #                 if df['Fisher_short'].iloc[m] < df['Fisher_short'].iloc[m - 1]:
            #                     exit_switch = True
            #                     # print('break out at ', m)
            #                     break
            #
            #         if exit_switch or m > length:
            #             break
            #
            # if exit_switch or m > length:
            #     break
        ...
        elif df.trade_state.iloc[m] == 2 or df.trade_state.iloc[m] == 3 or exit_switch:
    ...
    if len(profits) == 0:
        ...
        plt.plot(df.DC_LOW, color='orange')
        # plt.legend(loc='upper right', fontsize=5)
        plt.title('FISHER SMMA_INTERVAL_CONCAT - %s %.3f %.3f %.3f' % (Coin,
        ...
        # plt.plot(df.Fisher_long_gaussian)
        ...
        plt.axhline(0, color='red', alpha=0.5)
        plt.axhline(long_ic_signal_value)
        plt.axhline(short_signal_value)
        for trade_num in range(len(span_list_target_ic)):
            plt.axvspan(span_list_target_ic[trade_num][0], span_list_target_ic[trade_num][1], facecolor='blue', alpha=0.7)
        for trade_num in range(len(spanlist_limit)):
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['PIVX', 'BZNT', 'DVP', 'ZEC', 'ADA', 'AMO', 'PLX', 'ZIL', 'HYC', 'ETH']
    # TopCoin = ['EGG']
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = -0.8, 2.45
