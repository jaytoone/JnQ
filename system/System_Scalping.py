...
class OrderWindow(QMainWindow, form):
    ...
    def buy_clicked(self):  # 엔터치면 매수 동작하는 걸로 가능? = returnPressed()
        ...
        try:
            ...
            # ---------------- 매도 등록 ----------------#
            self.limit_sell_price = clearance(self.limit_buy_price * 1.015)
    ...
    def sell_clicked(self):
        ...
        try:
            ...
            print(float(self.lineEdit_sell.text()))
            if self.limit_buy_price != 0:
                self.lineEdit_status.setText("%.2f%% 수익률 등록" % (self.limit_sell_price / self.limit_buy_price * 100 - 100))
            else:
                self.lineEdit_status.setText("%s KRW 매도가 등록" % self.limit_sell_price)
            ...
            while True:
                ...
                try:
                    ...
                    #       미체결 대기 / 손절 시기 파악        #
                        ...
                        try:
                            if pybithumb.get_current_price(self.thread_parent.Coin) < self.thread_parent.limit_exit_price:
                                if sell_type == 'limit':
