...
# ----------- UI SETTING -----------#
form = uic.loadUiType("SR_ui.ui")[0]
...
class OrderWindow(QMainWindow, form):
    ...
    def buy_clicked(self):  # 엔터치면 매수 동작하는 걸로 가능? = returnPressed()
        ...
        try:
            ...
            self.limit_buy_price = Funcs_CNN4.clearance(float(self.lineEdit_buy.text()))
    ...
    def sell_clicked(self):
        ...
        try:
            ...
            # ---------------- 매도 등록 ----------------#
            self.limit_sell_price = Funcs_CNN4.clearance(float(self.lineEdit_sell.text()))
            if self.limit_buy_price != 0:
                self.lineEdit_status.setText("%.2f%% 수익률 등록" % (self.limit_sell_price / self.limit_buy_price * 100 - 100))
            else:
    ...
    def exit_clicked(self):
        ...
        try:
            self.limit_exit_price = Funcs_CNN4.clearance(float(self.lineEdit_exit.text()))

            if self.limit_buy_price != 0:
                self.lineEdit_status.setText("%.2f%% 손절가율 등록" % ((self.limit_exit_price / self.limit_buy_price - 1.) * 100))
            else:
...
class TradeThread(QThread):
    ...
    def run(self):
        ...
        var_list = ['코인', '매수가', '매도가']
        for i, var in enumerate([self.thread_parent.Coin, self.thread_parent.limit_buy_price, self.thread_parent.limit_sell_price]):
            if var is None:
        ...
        if self.thread_parent.limit_buy_price != 0:
            ...
            try:
                ...
                # 매수 등록
                BuyOrder = bithumb.buy_limit_order(self.thread_parent.Coin, self.thread_parent.limit_buy_price, buyunit, "KRW")
                print("    %s %s KRW 매수 등록    " % (self.thread_parent.Coin, self.thread_parent.limit_buy_price), end=' ')
                print(BuyOrder)
                self.thread_parent.Orderinfo = BuyOrder
                self.thread_parent.lineEdit_status.setText("%s %s KRW 매수 등록" % (self.thread_parent.Coin, self.thread_parent.limit_buy_price))
            ...
            while True:
                try:
                    ...
                    self.thread_parent.lineEdit_status.setText("실시간 체결량 : %.2f %%" % ((balance[0] - start_coin) / buyunit * 100))
        ...
        #                    매수 체결                     #
            ...
            while True:
                ...
                try:
                    ...
                    SellOrder = bithumb.sell_limit_order(self.thread_parent.Coin, self.thread_parent.limit_sell_price, sellunit, 'KRW')
                    ...
                    self.thread_parent.lineEdit_status.setText("%s %s KRW 매도 등록" % (self.thread_parent.Coin, self.thread_parent.limit_sell_price))
                    break
            ...
            while True:
                ...
                try:
                    if sell_switch == 1:
                        ...
                        sellunit = int((balance[0]) * 10000) / 10000.0
                        if exit_count != -1:
                            SellOrder = bithumb.sell_limit_order(self.thread_parent.Coin, self.thread_parent.limit_sell_price, sellunit,
                                                                 "KRW")
                            print("    %s 지정가 매도     " % self.thread_parent.Coin, end=' ')
                            self.thread_parent.lineEdit_status.setText("%s %s KRW 매도 등록" % (self.thread_parent.Coin, self.thread_parent.limit_sell_price))
                        else:
                            ...
                            if ordersucceed[0] != balance[0] and ordersucceed[1] == 0.0:
                                ...
                                if self.thread_parent.limit_buy_price != 0:
                                    self.thread_parent.profits *= \
                                        self.thread_parent.limit_sell_price / self.thread_parent.limit_buy_price - self.thread_parent.fee
                                    print("Accumulated Profits : %.6f\n" % self.thread_parent.profits)
                                    self.thread_parent.lineEdit_status.setText("Accumulated Profits : %.6f\n" % self.thread_parent.profits)
                                break
                            else:
                                sell_switch = 1
                                continue
                ...
                try:
                    if bithumb.get_outstanding_order(SellOrder) is None:
                        ...
                        if type(SellOrder) in [tuple, str]:  # 서버에러는 except 로 가요..
                            try:
                                ...
                                self.thread_parent.lineEdit_status.setText('SellOrder in [tuple, str] ? 에서 에러발생 : %s' % e)
                    ...
                    #       미체결 대기 / 손절 시기 파악        #
                        ...
                        try:
                            if pybithumb.get_current_price(self.thread_parent.Coin) < self.thread_parent.limit_exit_price and exit_count == 0:
