...
while True:
    ...
    while True:
        ...
        for Coin in TopCoin:
            ...
            while True:
                ...
                #           CHECK AND GET INDICATOR VALUE         #
                try:
                    fisher_list = System_Selenium_Funcs.get_coin_data(driver, Coin, interval_key2,
                                                                      smma_period, dc_period, fisher_long_period)
                    break
                except Exception as e:
                    print('Error in get_coin_data :', e)
            ...
            #       TRADE CONDITION     #
            if fisher_list[-2] <= long_signal_value:
                if fisher_list[-1] > fisher_list[-2] < fisher_list[-3]:

                    # while True:
                    #     #           GET DF VALUE         #
                    #     try:
                    #         df = pybithumb.get_ohlcv(Coin, interval='minute3')
                    #         if df is not None:
                    #             break
                    #
                    #     except Exception as e:
                    ...
                    #
                    # df['Fisher_long'] = TA.FISH(df, period=fisher_long_period)
                    #
                    ...
                    #
                    # #       TRADE CONDITION     #
                    # # if df['SMMA_DC_GAP'].iloc[-2] >= smma_accept_range:
                    # if df['DC_TREND'].iloc[-1] == 'UP':

                            # go_to_prev_i = -1
                            # while True:
                            #     # go_to_prev_i -= 1
                            #     # if go_to_prev_i == 0 or df['Fisher_long'].iloc[go_to_prev_i] > long_signal_value:
                            #     #     break
                            #     #       매수 포인트에서 CLOSE DC GAP이 작아야한다.     #
                            #     #               BLUE LINE POINT              #
                            #     # if df['CLOSE_DC_GAP'].iloc[go_to_prev_i] <= dc_accept_range:
                            #         print('%s indicator value at long position' % Coin)
                            #         print('Fisher :', fisher_list[-1], fisher_list[-2],
                            #               fisher_list[-3])
                            #         print('SMMA_DC_GAP :', df['SMMA_DC_GAP'].iloc[-2])
                            #         print('DC_TREND :', df['DC_TREND'].iloc[-1])
                            #         print('CLOSE_DC_GAP_MIN :', df['CLOSE_DC_GAP'].iloc[go_to_prev_i])
                            #         print('CLOSE_DC_GAP :', df['CLOSE_DC_GAP'].iloc[-1])
                            #
                            #         while True:
                            #             #        바로 전 데이터를 찾을때까지 반복      #
                            #             #        이전 종가로 매수가를 지정하기 위함        #
        ...
        if buy_switch == 1:  # GET OUT OF THE WHILE LOOP
            break
    ...
    #                    매수 체결 완료                    #
        ...
        #             ATTACH SECOND INTERVAL CHART               #
        # while True:
        #     try:
        #         System_Selenium_Funcs.attach_chart(driver, Coin, interval_key2)
        #         break
        #     except Exception as e:
        #         print('Error in attach second interval chart :', e)
