...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    if not Coin.endswith('.xlsx'):
        ...
        second_df = request_client.get_candlestick_data(symbol=Coin, interval=CandlestickInterval.MIN5,
                                                        startTime=None, endTime=None, limit=1500)
        third_df = request_client.get_candlestick_data(symbol=Coin, interval=CandlestickInterval.MIN3,
                                                       startTime=None, endTime=None, limit=1500)
    else:
    ...
    # df2['MA_Long'] = df2['close'].rolling(120).mean()
    ...
    offset = 0.08
    df2['MA_Long_Offset'] = df2['MA_Long'] - offset * chart_height
    df3['MA_Long_Offset'] = df3['MA_Long'] - offset * chart_height
    # df2['MA_Long_Offset'] = df2['MA_Long'] * (1 - chart_gap * offset)
    ...
    for i in range(2, len(df)):
        ...
        if df['Fisher_Long'].iloc[i] > 0 > df['Fisher_Long'].iloc[i - 1]:
            trade_state[i] = 1.5
    ...
    span_list_fisher_bull = list()
    span_list_fisher_bear = list()
    ...
    for i in range(1, len(target)):
        ...
        if df['Fisher_Long'].iloc[i] >= 0:
            span_list_fisher_bull.append((i, i + 1))
        else:
            span_list_fisher_bear.append((i, i + 1))
    ...
    while m <= length:  # 초반 시작포인트 찾기
        ...
        while True:
            ...
            #       매도 시그널       #
            if df['Fisher_Long'].iloc[m] <= 0:
                sell_switch = True
                break

            # if df['low'].iloc[m] <= df2['MA_Long_Offset'].iloc[m]:
            #     sell_switch = True
            #     break
    ...
    if len(profits) == 0:
        ...
        # for trade_num in range(len(span_list_fisher_bull)):
        #     plt.axvspan(span_list_fisher_bull[trade_num][0], span_list_fisher_bull[trade_num][1],
        #                 facecolor='blue', alpha=0.3)
        # for trade_num in range(len(span_list_fisher_bear)):
        #     plt.axvspan(span_list_fisher_bear[trade_num][0], span_list_fisher_bear[trade_num][1],
