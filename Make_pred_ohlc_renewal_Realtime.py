...
from Make_X_total_renewal import low_high
...
if __name__ == '__main__':
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['TMTG', 'FNB', 'LUNA', 'LBA', 'ANKR', 'CTXC', 'BAT', 'FLETA', 'STRAT', 'ENJ']
    ...
    for Coin in TopCoin:
        ...
        try:
            ...
            if 'ohlcmacd' in model_num:
                X_test = X_test[:, :, [0, 1, 2, 3, 8, 9, 10, 11]]
        ...
        if len(X_test) != 0:
            ...
            for i in range(len(Y_pred_)):
                ...
                if Y_pred_[i][1] > max_value[1] * 0.95:
                    Y_pred[i] = 1
                if Y_pred_[i][2] > max_value[2] * 0.85:
                    Y_pred[i] = 2
            ...
            if get_fig == 1:
                ...
                plt.subplot(211)
                plt.plot(max_abs_scaler(chart[:, [1]]), 'gold', label='close')
                # plt.plot(Y_pred_[:, [2]], 'g', label='Y_pred')
