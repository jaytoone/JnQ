...
class ARIMA_Bot:
    ...
    def __init__(self, symbol, interval, tp, leverage_, initial_asset, stacked_df_on=False):
        ...
        self.sub_client = sub_client.subscribe_aggregate_trade_event(symbol.lower(), callback, error)
    ...
    def run(self):
        ...
        while 1:
            ...
            while 1:
                ...
                try:
                    realtime_price = get_market_price(self.symbol)
                except Exception as e:
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                if order.entry_type == OrderType.MARKET:
                    #             ep with Market            #
                    try:
                        ep = get_market_price(self.symbol)
                    except Exception as e:
                        print('Error in get_market_price :', e)
                else:
                    #             ep with Limit             #
            ...
            if exec_quantity == 0.0:
                while 1:
                    ...
                    if datetime.now().timestamp() > datetime.timestamp(stacked_df.index[-1]) + 5:  # close 데이터가 완성되기 위해 충분한 시간
                        break
