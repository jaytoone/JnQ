...
def low_high(reg_target, sigma, crop_size=0, realtime=0):
    ...
    if crop_size != 0:
        ...
        total_y_smooth = gaussian_filter1d(reg_target.values, sigma=sigma)
        for i in range(none_indexs + crop_size, len(reg_target)):
            ...
            y_smooth = y
            sep_y_smooth = total_y_smooth[i + 1 - crop_size:i + 1]
            if np.isnan(sep_y_smooth[-1]):
                continue
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['FX', 'WAVES', 'XSR', 'TMTG', 'ITC', 'CHR', 'SXP', 'FCT', 'DASH', 'INS']
    # TopCoin = ['WICC']
                ...
                for Coin in TopCoin:
                    ...
                    # try:
                        print(Coin, profitage(Coin, 'MACD_OSC',
                                              short, long, signal, sigma=10, crop_size=30, realtime=1, Date=Date, get_fig=1, excel=0))
