...
class Shield(ShieldModule):
    ...
    """
    v3_3 -> v3_3_1
        1. tf value added to dict. (this version exist for tf-signal out.
    """
    ...
    def __init__(self, paper_name, main_name, id_zip, config_type, mode="Bank", shield_close=None):
        ...
        self.open_data_key = ["open_side_msg", "res_df_open", "res_df", "fee", "open_side", "limit_leverage", "trade_log_dict", 'tf']
        self.open_remain_data_key = ["open_side_msg", "res_df_open", "res_df", "fee", "open_side", "limit_leverage", "pos_side", "trade_log_dict", 'tf',
                                     "tp", "ep", "out", "liqd_p", "leverage", "open_quantity", "post_order_res", "fake_order"]
        self.close_data_key = ["open_side_msg", "res_df", "fee", "open_side", "pos_side", "trade_log_dict", 'tf',
        ...
        self.close_remain_data_key = ["open_side_msg", "res_df", "fee", "open_side", "pos_side", "trade_log_dict", 'tf',
                                      "tp", "ep", "out", "liqd_p", "leverage", "open_exec_price_list", "open_exec_qty", "post_order_res_list", "fake_order",
                                      "real_balance", "prev_exec_tp_len", "ex_dict",
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, watch_data in self.watch_data_dict.copy().items():
                    ...
                    watch_code_dict[code] = watch_data['open_side_msg'], watch_data['tf']
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, open_remain_data in self.open_remain_data_dict.copy().items():
                    ...
                    watch_code_dict[code] = open_remain_data['open_side_msg'], open_remain_data['tf']
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, close_remain_data in self.close_remain_data_dict.copy().items():
                    ...
                    watch_code_dict[code] = close_remain_data['open_side_msg'], close_remain_data['tf']
        ...
        while 1:
            ...
            if not self.config.trader_set.backtrade:
                ...
                if self.user_text is not None:
                    ...
                    user_text_lower = self.user_text.lower()
                    ...
                    if self.config.trader_set.messenger_on:
                        ...
                        #   ii. payload phase.
                        if len(payload) == 3:
                            code, open_side_msg, tf = payload

                            with open(r"D:\Projects\System_Trading\JnQ\Bank\valid_code\ticker_in_futures.pkl", 'rb') as f:
                                valid_code_list = pickle.load(f)

                            #   1. code validation
                            if code in valid_code_list and open_side_msg in [OrderSide.BUY, OrderSide.SELL] and tf in ['T', '3T', '5T', '15T', '30T', 'H']:
                                ...
                                watch_code_dict[code_key] = open_side_msg, tf
                        ...
                        #             user_text_lower = 'watch'
                        #         elif tr_key == 'TP':
                        #             #       a. close_rmmain --> close
                        #             #       b. close_remain pop.
                        #             close_data = {}
                        ...
                        #             msg = "waiting for tp changing...".format(code)
                        #
                        #     except Exception as e:
                        ...
                        #         self.sys_log.error(msg)
                            ...
                            if 'asset' in user_text_lower:
                                self.available_balance = int(self.user_text.split(" ")[-1])
                                user_text_lower = 'watch'

                            #   ii. remove code.
                            if 'rm' in user_text_lower:
                                ...
                                if code not in self.open_remain_data_dict and code not in self.close_remain_data_dict:
                                    ...
                                    if code in self.open_remain_data_dict:
                                        ...
                                        msg = "waiting for expiration...".format(code)
                                    else:
                                        msg = "{} cannot removed, because remaining in close_remain_data_dict.".format(code)

                    #   b. watch
                    if 'watch' in user_text_lower:
                        msg = "self.available_balance : {}\n\n".format(self.available_balance)
                        msg += "------ watch ------\n"
                        for code, watch_data in self.watch_data_dict.items():
                            msg += "{} {} {}\n".format(code, watch_data['open_side_msg'], watch_data['tf'])
                        msg += "\n"
                        msg += "------ open_remain ------\n"
                        for code, open_remain_data in self.open_remain_data_dict.items():
                            msg += "{} {} {}\n".format(code, open_remain_data['open_side_msg'], open_remain_data['tf'])
                        msg += "------ close_remain ------\n"
                        for code, close_remain_data in self.close_remain_data_dict.items():
                            msg += "{} {} {}\n".format(code, close_remain_data['open_side_msg'], close_remain_data['tf'])
                ...
                for code, (open_side_msg, tf) in watch_code_dict.items():
                    #   i. add websocket.
                    ...
                    self.watch_data_dict[code] = {"open_side_msg": open_side_msg, "tf": tf, "limit_leverage": limit_leverage, "ep_loc_point2": 0, "streamer": None}
            ...
            if self.allow_watch:
                ...
                for w_i, (code, watch_data) in enumerate(self.watch_data_dict.copy().items()):
                    ...
                    open_side_msg, tf, limit_leverage, ep_loc_point2, streamer = watch_data.values()
                    ...
                    # open_side = None
                    self.utils = None
                    #   a. utils 작성으로 tf 를 일시적으로 반영해야한다.
                    self.config.loc_set.point1.tf_entry = tf
                    ...
                    # 2. paper works
                    try:
                        res_df = self.public.sync_check_v2(res_df, self.config, tf=tf)  # function usage format maintenance
                    ...
                    if open_side is not None:
                        ...
                        #       z. queue result
                        open_data_value = [open_side_msg, res_df_open, res_df, fee, open_side, limit_leverage, {}, tf]
            ...
            for code, open_data in self.open_data_dict.copy().items():
                ...
                # y. data water_fall
                open_side_msg, res_df_open, res_df, fee, open_side, limit_leverage, trade_log_dict, tf = open_data.values()
                ...
                leverage, liqd_p = self.public.lvrg_liqd_set_v2_1(res_df, self.config, open_side, ep, out, fee, limit_leverage)
                ...
                if not error_code:
                    ...
                    open_remain_data_value = [open_side_msg, res_df_open, res_df, fee, open_side, limit_leverage, pos_side, trade_log_dict, tf,
                                              tp, ep, out, liqd_p, leverage, open_quantity, post_order_res, fake_order]
            ...
            for code, open_remain_data in self.open_remain_data_dict.copy().items():
                ...
                # y. data water_fall
                open_side_msg, res_df_open, res_df, fee, open_side, limit_leverage, pos_side, trade_log_dict, tf,\
                ...
                if expired or open_exec:
                    ...
                    if open_exec or open_exec_qty != 0:
                        ...
                        close_data_value = [open_side_msg, res_df, fee, open_side, pos_side, trade_log_dict, tf,
                                            tp, ep, out, liqd_p, leverage, open_exec_price_list, open_exec_qty, post_order_res_list, fake_order,
                                            real_balance, 0, {}]
            ...
            for code, close_data in self.close_data_dict.copy().items():
                ...
                # y. data water_fall
                open_side_msg, res_df, fee, open_side, pos_side, trade_log_dict, tf,\
                tp, ep, out, liqd_p, leverage, open_exec_price_list, open_exec_qty, post_order_res_list, fake_order,\
                ...
                close_remain_data_value = [open_side_msg, res_df, fee, open_side, pos_side, trade_log_dict, tf,
                                           tp, ep, out, liqd_p, leverage, open_exec_price_list, open_exec_qty, post_order_res_list, fake_order,
                                           real_balance, prev_exec_tp_len, ex_dict,
                                           partial_tps, partial_qtys, close_side, quantity_precision]
            ...
            for code, close_remain_data in self.close_remain_data_dict.copy().items():
                ...
                if not market_close_on:  # log_out 갱신 방지
                    # res_df = self.public.sync_check_v2(res_df, self.config, tf)  # function usage format maintenance
                    # self.sys_log.info('WATCH : ~ sync_check : %.5f' % (time.time() - start_time))
