        ...
        while 1:
                ...
                if ep_loc_check:
                    ...
                    if ep_loc_pointcnt == 0:
                        try:
                            ...
                            if res_df['entry_{}'.format(config1.strat_version)][config1.trader_set.last_index] == config1.ep_set.short_entry_score or \
                                    res_df['entry_{}'.format(config2.strat_version)][config2.trader_set.last_index] == config2.ep_set.short_entry_score:

                                if res_df['entry_{}'.format(config1.strat_version)][config1.trader_set.last_index] == config1.ep_set.short_entry_score:
                                    config = config1
                                ...
                                res_df, open_side, _ = self.utils_public.short_ep_loc(res_df, config, config.trader_set.last_index)

                            # elif res_df['entry_{}'.format(strat_version)][config.trader_set.last_index] == -config.ep_set.short_entry_score:
                            elif res_df['entry_{}'.format(config1.strat_version)][config1.trader_set.last_index] == -config1.ep_set.short_entry_score or \
                                 res_df['entry_{}'.format(config2.strat_version)][config2.trader_set.last_index] == -config2.ep_set.short_entry_score:
                                ...
                                res_df, open_side, _ = self.utils_public.long_ep_loc(res_df, config, config.trader_set.last_index)
                ...
                if open_side is not None:
                    ...
                    if strat_version == "v5_2":
                        if ep_loc_pointcnt == 1:
                            ep_loc_check = 1
                            ep_loc_pointcnt += 1
                            continue    #   이 continue 를 아래 phase 에서 해야하지 않을까

                        elif ep_loc_pointcnt == 2:
                            ...
                            e_j = config.trader_set.last_index
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                if open_side == OrderSide.BUY:
                    ep = res_df['long_ep_{}'.format(strat_version)].iloc[config.trader_set.last_index]

                    if config.trader_set.long_inversion:
                        open_side = OrderSide.SELL  # side change (inversion)
                        out = res_df['long_tp_{}'.format(strat_version)].iloc[config.trader_set.last_index]
                        tp = res_df['long_out_{}'.format(strat_version)].iloc[config.trader_set.last_index]
                    else:
                        out = res_df['long_out_{}'.format(strat_version)].iloc[config.trader_set.last_index]
                        tp = res_df['long_tp_{}'.format(strat_version)].iloc[config.trader_set.last_index]
                else:
                    ep = res_df['short_ep_{}'.format(strat_version)].iloc[config.trader_set.last_index]

                    if config.trader_set.short_inversion:
                        open_side = OrderSide.BUY
                        out = res_df['short_tp_{}'.format(strat_version)].iloc[config.trader_set.last_index]
                        tp = res_df['short_out_{}'.format(strat_version)].iloc[config.trader_set.last_index]
                    else:
                        out = res_df['short_out_{}'.format(strat_version)].iloc[config.trader_set.last_index]
                        tp = res_df['short_tp_{}'.format(strat_version)].iloc[config.trader_set.last_index]
