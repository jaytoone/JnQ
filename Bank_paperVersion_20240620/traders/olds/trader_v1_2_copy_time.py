        ...
        while 1:
            ...
            while 1:
                if load_new_df:
                    ...
                    sys_log.info("load_new_df elapsed time : %.5f" % (time.time() - start_ts))

                    # ------------ paper works ------------ #
                    try:
                        start_ts = time.time()
                        res_df = self.public.sync_check(res_df, self.config)  # function usage format maintenance
                        sys_log.info('sync_check elapsed time : %.5f' % (time.time() - start_ts))

                        start_ts = time.time()
                        np_timeidx = np.array([intmin_np(date_) for date_ in res_df.index.to_numpy()])
                        res_df = self.public.public_indi(res_df, self.config, np_timeidx)
                        sys_log.info('public_indi elapsed time : %.5f' % (time.time() - start_ts))

                        # ------ use_point2 사용시, 해당 ID 로만 enlist_ 진행 ------ #
                        start_ts = time.time()
                        if ep_loc_point2:
                            res_df = self.utils.enlist_tr(res_df, self.config, np_timeidx, show_detail=True)
                        else:
                            for utils_, config_ in zip(self.utils_list, self.config_list):
                                res_df = utils_.enlist_tr(res_df, config_, np_timeidx, show_detail=True)
                        sys_log.info('enlist_tr elapsed time : %.5f' % (time.time() - start_ts))

                        if not self.config.trader_set.backtrade:    # logging 미관을 위해 \n 을 나눔.
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                if min_balance_bool:
                    break
                sys_log.info('get balance elapsed time : %.5f' % (time.time() - start_ts))
                ...
                sys_log.info('tp ep out lvrg setting elapsed time : %.5f' % (time.time() - start_ts))
