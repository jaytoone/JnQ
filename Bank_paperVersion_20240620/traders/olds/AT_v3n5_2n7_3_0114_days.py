...
class Trader:
    ...
    def __init__(self, utils_list, config_list):
        ...
        self.config1_name, self.config2_name, self.config3_name = config_list
        self.utils = None
    ...
    def run(self):
        ...
        cfg_full_path3 = os.path.join(pkg_path, "config", self.config3_name)
        initial_run = 1
        while 1:
                ...
                if load_new_df:
                        ...
                        for itv_i, interval_ in enumerate(config.trader_set.interval_list):
                            ...
                            if interval_ != "None":
                                ...
                                new_df_, _ = concat_candlestick(config.trader_set.symbol, interval_,
                                                                days=config.trader_set.days_list[itv_i],
                    ...
                    try:
                        ...
                        if ep_loc_point2:
                            ...
                            res_df = self.utils2.enlist_rtc(res_df, config3)
                            ...
                            if res_df['entry_{}'.format(config1.strat_version)][
                                ...
                                if not config.pos_set.short_ban:
                                    res_df, open_side, _ = self.utils_public.short_ep_loc(res_df, config,
                                                                                          config.trader_set.last_index, np_timeidx)
                                    ...
                                    if not config3.pos_set.short_ban:   # additional const. on trader
                                        if config.strat_version == 'v5_2' and open_side is None:
                                            ...
                                            res_df, open_side, _ = self.utils_public.short_ep_loc(res_df, config3, config.trader_set.last_index,
                                                                                                np_timeidx)
                                ...
                                if not config.pos_set.long_ban:
                                    ...
                                    if not config3.pos_set.long_ban:   # additional const. on trader
                                        if config.strat_version in ['v5_2'] and open_side is None:
                                            sys_log.warning(
                                                "ep_loc.point executed strat_ver : {}".format(config3.strat_version))
                                            res_df, open_side, zone = self.utils_public.long_ep_loc(res_df, config3, config.trader_set.last_index, np_timeidx)
                ...
                while 1:
                    ...
                    if use_new_df2:
                        ...
                        if load_new_df2:  # dynamic_out & tp phase
                            ...
                            try:
                                res_df = self.utils_public.sync_check(res_df_list, order_side="CLOSE")
                    ...
                    while 1:
                        ...
                        try:
                            ...
                            if use_new_df2:
                                ...
                                try:
                                    if load_new_df3:    # '1m' 의 dataframe 이 필요한 것
