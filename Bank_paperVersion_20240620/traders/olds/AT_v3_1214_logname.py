...
class Trader:
    ...
    def run(self):
        ...
        cfg_full_path = os.path.join(pkg_path, "config", self.config_name)
        initial_run = 1
        while 1:

            #       load config (refreshed by every trade)       #
            try:
                with open(cfg_full_path, 'r') as cfg:
            ...
            try:
                with open(dst_cfg_path, 'r') as sys_cfg:
                    ...
                    if initial_run:
                        # sys_log_cfg.handlers.file_rot.filename = os.path.join(sys_log_path, trade_log_name.replace("pkl", "log"))
                    ...
                    if config.tp_set.tp_type == OrderType.LIMIT:
                        ...
                        if config.tp_set.static_tp:
                            ...
                            if remained_tp_id is None:  # first tp_order
                                ...
                                if tp_series.iloc[config.trader_set.last_index] != tp_series.iloc[
                                    config.trader_set.last_index - 1]:
                                    limit_tp = 1
