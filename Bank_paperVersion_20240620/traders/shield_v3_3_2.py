...
class Shield(ShieldModule):
    ...
    """
    v3_3 -> v3_3_2
        1. allow tp change in open_remain.
        2. allow multiple ' ' for input MSG.
            a. replace split char '.' --> '/'
        3. allow symbol name only. ('ETH')
    """
            ...
            if not self.config.trader_set.backtrade:
                ...
                if self.user_text is not None:
                    ...
                    if self.config.trader_set.messenger_on:
                        ...
                        #   ii. payload phase.
                        if len(payload) == 5:
                            code, open_side_msg, tp, ep, out = payload
                            code += 'USDT'
                            ...
                            if code in valid_code_list and open_side_msg in [OrderSide.BUY, OrderSide.SELL]:
                                ...
                                try:
                                    ...
                                    watch_code_dict[code_key] = open_side_msg, float(tp), float(ep), float(out)
