...
class Shield(ShieldModule):
    ...
    def __init__(self, paper_name, main_name, id_zip, config_type, mode="Bank", shield_close=None):
        ...
        # 14. ML params.
        # self.config.trader_set.valid_data_path = r"D:\Projects\System_Trading\JnQ\database\binance\ML\wrr32_03\long\tr_non_fix\static_lvrg\res\200_6\random_index_99_100.pkl"
        with open(self.config.trader_set.valid_data_path, 'rb') as f:
    ...
    def run(self):
        ...
        while 1:
            ...
            #   a. after watch_loop, 최소 시간동안 watch loop 에 들어오지 못하도록 해야한다.
            #       i. 최소시간 = 20 seconds --> 임의 설정.
            #       ii. 초 단위 < 2 로 고정. (= 분당 1번 정도로 제한한다는 의미, ex : Binance)
            if not self.config.trader_set.backtrade:
                try:
                    if time.time() - end_time_watch >= 10:  # and now.second < 2:
                        self.allow_watch = 1
            ...
            if self.allow_watch:
                ...
                for w_i, (code, watch_data) in enumerate(self.watch_data_dict.copy().items()):
                    ...
                    if open_side is not None:
                        ...
                        # self.sys_log.info("x_train.shape : {}".format(x_train.shape))
                        #
                        # #       2. if close data not exist, continnue.
                        # cosine_similarity_res = cosine_similarity(self.key_data, x_train)
                        # self.sys_log.info("cosine_similarity_res : {}".format(cosine_similarity_res))
                        #
                        ...
                        #           i. just alert.
                        self.msg_bot.sendMessage(chat_id=self.chat_id, text="{} {} open signal occured.".format(code, open_side))
