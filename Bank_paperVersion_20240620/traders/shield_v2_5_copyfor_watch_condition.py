...
class Shield(ShieldModule):
    ...
    def run(self):
        ...
        while 1:
            ...
            ADD WATCH
            """
            # 1. Messenger 에 대하여, 아래의 조건문을 수정하는 것이 좋을 것. Todo
            #   a. 원하는 code 를 watch_data_dict 에 추가하고 싶은 경우에 대한 조건문 작성.
            #       i. messenger_on = True, --> if self.user_text is not None 사용가능.
            #       ii. messenger_on = False, 
            ...
            if len(self.watch_data_dict) == 0:
                if not self.config.trader_set.backtrade:
                    ...
                    if self.config.trader_set.messenger_on:
                        #   i. get code (msg form : code, pos_side)
                        if self.user_text is not None:
                            ...
                            #       1. code validation
                            if self.user_text in valid_code_list:
                                watch_code.extend([[self.user_text, self.user_text]])
                                msg = "{} extended to watch_code.".format(self.user_text)
                            else:
                                msg = "error in self.user_text : invalid code."
                                self.sys_log.error(msg)
                            self.msg_bot.sendMessage(chat_id=self.chat_id, text=msg)
            ...
            if not self.config.trader_set.backtrade:
                try:
                    if time.time() - end_time_watch >= 20:
                        self.allow_watch = 1
