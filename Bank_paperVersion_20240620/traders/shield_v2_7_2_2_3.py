...
from funcs.public.broker import intmin_np, min_max_scaler
from funcs.public.constant import *

from sklearn.metrics.pairwise import cosine_similarity  # ML module.
...
class Shield(ShieldModule):
    ...
    """
    v2_7_2_2_1 -> v2_7_2_2_3
        1. add config ML parameter.
        2. add function for DS_ML's lvrg.
            a. get_tp
    """
    ...
    def __init__(self, paper_name, main_name, id_zip, config_type, mode="Bank", shield_close=None):
        ...
        # 14. ML params.
        with open(self.config.trader_set.key_data_path, 'rb') as f:
            self.key_data = pickle.load(f)

        self.input_cols = ['open', 'high', 'low', 'close', 'ma_T50', 'ma_T200']  # , 'long_open1_1']
        self.data_size = 200
        self.cols_len = len(self.input_cols)
        self.flatten_len = self.data_size * self.cols_len
        self.cluster_threshold = 0.99
    ...
    def get_income_info(self, real_balance, leverage, ideal_profit, real_profit, mode="PROD", currency="USDT"):  # 복수 pos 를 허용하기 위한 api 미사용
        ...
        self.sys_log.info("accumulated income : {:.2f} {}".format(self.accumulated_income, currency))
    ...
    def run(self):
        ...
        while 1:
                ...
                #   3. Messenger off 시, default code = self.config.trader_set.symbol
                    ...
                    if not self.config.trader_set.messenger_on:
                        if len(self.watch_data_dict) == 0:
                            ...
                            #   a. allow multiple tickers.
                            # code = self.config.trader_set.symbol

                            with open(self.config.trader_set.ticker_list_path, 'rb') as f:
                                valid_code_list = pickle.load(f)
                            watch_code_dict = dict(zip(valid_code_list, np.full(len(valid_code_list), open_side_msg)))
            ...
            #   a. watch_loop 이후, 최소 시간동안 watch loop 에 들어오지 못하도록 해야한다.
            #       i. 최소시간 = 20 seconds --> 임의 설정.
            #       ii. 초 단위 < 2 로 고정. (= 분당 1번 정도로 제한한다는 의미, ex : Binance)
            if not self.config.trader_set.backtrade:
                try:
                    if time.time() - end_time_watch >= 3:  # 10 and now.second < 2:
                        self.allow_watch = 1
            ...
            if self.allow_watch:
                ...
                for w_i, (code, watch_data) in enumerate(self.watch_data_dict.copy().items()):
                    ...
                    if self.config.trader_set.backtrade:
                        ...
                        res_df = self.get_new_df_v2(code)
                    ...
                    if open_side is not None:
                        ...
                        #   i. ML phase
                        """
                        res_df_data = res_df[self.input_cols].to_numpy()

                        #       1. complete_index = -2, wave_point 는 complete_index 기준 --> res_df's last_row 는 x_train 에서 제외한다.
                        p1 = self.config.trader_set.complete_index
                        data_np = res_df_data[p1 + 1 - self.data_size:p1 + 1]
                        data_np_norm = min_max_scaler(data_np)
                        x_train = data_np_norm.reshape(-1, self.flatten_len)
                        self.sys_log.warning("x_train.shape : {}".format(x_train.shape))

                        #       2. if close data not exist, continnue.
                        cosine_similarity_res = cosine_similarity(self.key_data, x_train)
                        self.sys_log.warning("cosine_similarity_res : {}".format(cosine_similarity_res))

                        #       3. over-threshold count should be larger than 1 and not None.
                        if np.isnan(cosine_similarity_res).sum() != 0:
                            continue
                        if np.sum(cosine_similarity_res > self.cluster_threshold) < 1:
                            continue

                        #   ii. check open_side_msg for hand_trading (= open_side_msg is not None)
                        if open_side_msg is not None:
                            if open_side != open_side_msg:
                                continue

                        #   iii. res_df_open 정의는 이곳에서 해야함 - 아래서 할 경우 res_df_open 갱신되는 문제
            ...
            for code, open_data in self.open_data_dict.copy().items():
                ...
                tp, ep, out, leverage, open_side = self.get_tpepout_v2(open_side, res_df_open, res_df)
                ...
                multiplier = self.config.lvrg_set.multiplier
                log_base = self.config.lvrg_set.log_base
                # self.config.lvrg_set.leverage = leverage
                # leverage, liqd_p = self.public.lvrg_liqd_set_v2(res_df, self.config, open_side, ep, out, fee, limit_leverage)
                # leverage, liqd_p = self.public.lvrg_liqd_set_v4(res_df, self.config, open_side, ep, out, fee, limit_leverage)
                leverage, liqd_p = self.public.lvrg_liqd_set_v4_1(res_df, self.config, open_side, ep, out, fee, multiplier, log_base, limit_leverage)
