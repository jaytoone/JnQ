...
class Shield(ShieldModule):
    ...
    """
    v3_3_2 -> v3_3_3
        1. allow more loss range.(for tp modification.)
    """
    ...
    def __init__(self, paper_name, main_name, id_zip, config_type, mode="Bank", shield_close=None):
        ...
        self.open_remain_data_key = ["open_side_msg", "res_df_open", "res_df", "fee", "open_side", "limit_leverage", "pos_side", "trade_log_dict",
        ...
        # 10. pnl
        self.income = 0.0
        self.accumulated_income = self.config.trader_set.acc_income

        if self.config.trader_set.profit_mode == "SUM":
            self.accumulated_profit = self.config.trader_set.acc_profit_sum
        else:
            self.accumulated_profit = self.config.trader_set.acc_profit_prod
        self.ideal_accumulated_profit = self.accumulated_profit
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, watch_data in self.watch_data_dict.copy().items():
                    ...
                    watch_code_dict[code] = watch_data['open_side_msg'], watch_data['tp'], watch_data['ep'], watch_data['out']
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, open_remain_data in self.open_remain_data_dict.copy().items():
                    ...
                    watch_code_dict[code] = open_remain_data['open_side_msg'], open_remain_data['tp'], open_remain_data['ep'], open_remain_data['out']
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, close_remain_data in self.close_remain_data_dict.copy().items():
                    ...
                    watch_code_dict[code] = close_remain_data['open_side_msg'], close_remain_data['tp'], close_remain_data['ep'], close_remain_data['out']
    ...
    def get_income_info(self, real_balance, leverage, ideal_profit, real_profit, mode="PROD", currency="USDT"):  # 복수 pos 를 허용하기 위한 api 미사용
        ...
        if mode == "PROD":
            ...
            self.config_list[0].trader_set.initial_asset = self.available_balance
            with open(self.config_path_list[0], 'w') as cfg:
                json.dump(self.config_list[0], cfg, indent=2)
        ...
        if mode == "PROD":
            ...
            self.config_list[0].trader_set.acc_profit_prod = self.accumulated_profit
        else:
        ...
        # msg = "accumulated profit : {:.2%} ({:.2%})\n".format(self.accumulated_profit, self.ideal_accumulated_profit)
    ...
    def run(self):
        ...
        while 1:
            ...
            if self.shield_close is not None:
                if timestamp > datetime.timestamp(to_datetime(" ".join([date, self.shield_close]))):
            ...
            open_side_msg = None    # open_side 선택을 위해서 선언된 변수.
            if not self.config.trader_set.backtrade:
                ...
                if self.user_text is not None:
                    ...
                    if self.config.trader_set.messenger_on:
                        ...
                        if len(payload) == 3:
                            if '/' in payload[-1]:
                                payload = payload[:2] + payload[-1].split('/')

                        #   ii. payload phase.
                        if len(payload) == 5:
                            code, open_side_msg, tp, ep, out, out2 = payload
                            code += 'USDT'

                            with open(self.config.trader_set.ticker_list_path, 'rb') as f:
                                valid_code_list = pickle.load(f)

                            #   1. code validation
                            if code in valid_code_list and open_side_msg in [OrderSide.BUY, OrderSide.SELL]:
                                # #   a. check code duplication.
                                # for code_key in watch_code_dict.keys():
                                #     if code_key.contains(code):
                                #         code
                                try:
                                    code_key = "{}_{}".format(code, int(timestamp))
                                    watch_code_dict[code_key] = open_side_msg, float(tp), float(ep), float(out), float(out2)
                                    msg = "{} {} added to watch_code_dict.".format(code_key, open_side_msg)

                                except Exception as e:
                                    msg = "error in payload phase : {}".format(e)
                                    self.sys_log.error(msg)

                        elif len(payload) == 3:
                            code, tr_key, tr_value = payload

                            try:
                                #       1. out change.
                                if tr_key == 'OUT':
                                    self.close_remain_data_dict[code]['out'] = float(tr_value)
                                    user_text_upper_case = 'WATCH'

                                elif tr_key == 'OUT2':
                                    self.close_remain_data_dict[code]['out2'] = float(tr_value)
                                    user_text_upper_case = 'WATCH'

                                elif tr_key == 'TP':

                                    if code in self.open_remain_data_dict.keys():
                                        try:
                                            #       a. just modify open_remain.
                                            self.watch_data_dict[code]['open_side_msg'] = 'TP_CHANGE'
                                            self.open_remain_data_dict[code]['open_side_msg'] = 'TP_CHANGE'
                                            self.open_remain_data_dict[code]['tp'] = float(tr_value)
                                        except Exception as e:
                                            msg = "error in TP change, open_remain : {}".format(e)
                                            self.sys_log.error(msg)
                                        else:
                                            msg = "waiting for {} TP change...".format(code)

                                    elif code in self.close_remain_data_dict.keys():
                                        try:
                                            #       a. cancel close_rmmain --> re-close
                                            #           i. close_remain pop.
                                            close_data = {}
                                            close_remain_data = self.close_remain_data_dict[code]
                                            ...
                                            self.watch_data_dict[code]['open_side_msg'] = 'TP_CHANGE'
                                            self.close_data_dict[code]['open_side_msg'] = 'TP_CHANGE'
                                            self.close_data_dict[code]['tp'] = float(tr_value)
                                            self.close_remain_data_dict.pop(code, None)
                                        except Exception as e:
                                            msg = "error in TP change, close_remain : {}".format(e)
                                            self.sys_log.error(msg)
                                        else:
                                            msg = "waiting for {} TP change...".format(code)
                            ...
                            if 'ASSET' in user_text_upper_case:
                                self.available_balance = int(self.user_text.split(' ')[-1])
                                user_text_upper_case = 'WATCH'

                            #   ii. remove code.
                            if 'RM' in user_text_upper_case:
                                ...
                                if code not in self.open_remain_data_dict and code not in self.close_remain_data_dict:
                                    ...
                                    if code in self.open_remain_data_dict:
                                        self.open_remain_data_dict[code]['open_side_msg'] = 'EXPIRED'
                                        msg = "waiting for {} to be EXPIRED...".format(code)
                                    else:
                                        msg = "{} cannot removed, because remaining in close_remain_data_dict.".format(code)

                    #   b. watch
                    if 'WATCH' in user_text_upper_case:
                        msg = "self.available_balance : {}\n\n".format(self.available_balance)
                        msg += "------ watch ------\n"
                        for code in self.watch_data_dict.keys():
                            msg += "{}\n".format(code)
                        msg += "\n"
                        msg += "------ open_remain ------\n"
                        for code, open_remain_data in self.open_remain_data_dict.items():
                            msg += "{}\ntp : {}\nep : {}\nout : {}\n\n".format(code, open_remain_data['tp'], open_remain_data['ep'], open_remain_data['out'])
                        msg += "------ close_remain ------\n"
                        for code, close_remain_data in self.close_remain_data_dict.items():
                            msg += "{}\ntp : {}\nep : {}\nout : {}\n\n".format(code, close_remain_data['tp'], close_remain_data['ep'], close_remain_data['out'])
                ...
                for code, (open_side_msg, tp, ep, out) in watch_code_dict.items():
                    #   i. add websocket.
                    code_org = code.split('_')[0]
                    self.websocket_client.agg_trade(symbol=code_org, id=1, callback=self.agg_trade_message_handler)
                    #   ii. set posmode_margin_leverage.
                    limit_leverage = self.posmode_margin_leverage(code_org)
                    self.watch_data_dict[code] = {"open_side_msg": open_side_msg, "tp": tp, "ep": ep, "out": out,
                                                  "limit_leverage": limit_leverage, "ep_loc_point2": 0, "streamer": None}
            ...
            if self.allow_watch:
                ...
                for w_i, (code, watch_data) in enumerate(self.watch_data_dict.copy().items()):
                    ...
                    if self.config.trader_set.backtrade:
                        ...
                        res_df = self.get_new_df(code_org)

                        #   a. last_index validation 으로 인한 지연은 loop_duration 에서 제외한다.
                        if time.time() - start_time > self.config.trader_set.loop_duration:
                            start_time = time.time()
                    ...
                    try:
                        ...
                        if self.utils is not None and ep_loc_point2:
                            res_df = self.utils.enlist_tr(res_df, tp, ep, out, self.config, np_timeidx, show_detail=self.config.trader_set.show_detail)
                        else:
                            for utils_, config_ in zip(self.utils_list, self.config_list):
                                res_df = utils_.enlist_tr(res_df, tp, ep, out, config_, np_timeidx, show_detail=config_.trader_set.show_detail)
            ...
            for code, open_data in self.open_data_dict.copy().items():
                ...
                tp, ep, out1, out2, open_side = self.get_tpepout_v3(open_side, res_df_open, res_df)
                ...
                leverage, liqd_p = self.public.lvrg_liqd_set_v2_1(res_df, self.config, open_side, ep, out1, fee, limit_leverage)
                # leverage, liqd_p = self.public.lvrg_liqd_set_v2_2(res_df, self.config, open_side, ep, tp, fee, limit_leverage)

                #       b. get precision
                price_precision, quantity_precision = self.get_precision(code_org)

                tp, ep, out, liqd_p = [self.calc_with_precision(price_, price_precision) for price_ in [tp, ep, out2, liqd_p]]  # includes half-dynamic tp

                self.sys_log.info('tp : {}'.format(tp))
                self.sys_log.info('ep : {}'.format(ep))
                self.sys_log.info('out1 : {}'.format(out1))
                ...
                if not self.config.trader_set.backtrade:
                    while 1:
                        try:
                            ...
                            self.change_leverage(symbol=code_org, leverage=leverage, recvWindow=6000, timestamp=server_time)
                        except Exception as e:
                ...
                if not self.config.trader_set.backtrade and not fake_order:
                    ...
                    post_order_res, self.over_balance, error_code = self.limit_order_v2(code_org, self.config.ep_set.entry_type, open_side, pos_side, ep, open_quantity, order_data)
                ...
                if not error_code:
                    ...
                    open_remain_data_value = [open_side_msg, res_df_open, res_df, fee, open_side, limit_leverage, pos_side, trade_log_dict,
                                              tp, ep, out, liqd_p, leverage, open_quantity, post_order_res, fake_order]
            ...
            for code, open_remain_data in self.open_remain_data_dict.copy().items():
                ...
                # y. data water_fall
                open_side_msg, res_df_open, res_df, fee, open_side, limit_leverage, pos_side, trade_log_dict, \
                tp, ep, out, liqd_p, leverage, open_quantity, post_order_res, fake_order = open_remain_data.values()
                ...
                if open_side_msg == 'TP_CHANGE':
                    self.user_text = 'watch'
                    self.open_remain_data_dict[code]['open_side_msg'] = 'TP_CHANGED'    # reject continous 'watch' order.
                ...
                # 2. limit
                    ...
                    if not self.config.trader_set.backtrade:
                        ...
                        #   [ API ] : order_info
                        realtime_price = self.get_market_price_v3(code_org)

                        #   a. ei_k
                        #       ii. expire_tp default
                        expired = self.check_ei_k_v3(res_df_open, res_df, realtime_price, open_side, expired)
                        ...
                        #   a. ei_k
                        #       i. expire_tp default
                        expired = self.check_ei_k_onbarclose_v3(res_df_open, res_df, c_i, c_i, open_side, expired)  # e_j, tp_j
                ...
                if expired or open_exec:
                    ...
                    if expired:
                        ...
                        if self.config.trader_set.messenger_on:
                            self.watch_data_dict.pop(code)
                            # i. code duplication 을 고려해, stop_socket 사용하지 않는다.
                ...
                # 3. term for expiration API (get_order_info)
                else:
                    time.sleep(self.config.trader_set.realtime_term)
            ...
            for code, close_data in self.close_data_dict.copy().items():
                ...
                if open_side_msg == 'TP_CHANGE':
                    try:
                        ...
                        _, quantity_precision = self.get_precision(code_org)
                        self.sys_log.info('quantity_precision : {}'.format(quantity_precision))

                        open_exec_qty = self.calc_with_precision(close_remain_quantity, quantity_precision)
                        self.sys_log.info('close_remain_quantity : {}'.format(close_remain_quantity))

                        # 3. reset post_order_res_list to None.
                        for on_i, _ in enumerate(post_order_res_list):
                            if on_i >= prev_exec_tp_len:
                                post_order_res_list[on_i] = None

                        self.user_text = 'watch'

                    except Exception as e:
                        msg = "error in TP_CHANGE : {}".format(e)
                        self.sys_log.error(msg)
                        self.msg_bot.sendMessage(chat_id=self.chat_id, text=msg)
                        time.sleep(self.config.trader_set.api_term)
                        continue
                ...
                if not self.config.trader_set.backtrade and not fake_order:
                    ...
                    # [ API ]
                    post_order_res_list = self.partial_limit_order_v5(code_org, post_order_res_list, partial_tps, partial_qtys, close_side, pos_side, open_exec_qty, quantity_precision)
            ...
            for code, close_remain_data in self.close_remain_data_dict.copy().items():
                ...
                if not market_close_on:  # log_out 갱신 방지
                    ...
                    # market_close_on, log_out = self.check_signal_out_v4(res_df, np_timeidx, market_close_on, open_side)

                # 2-1. check out elevation.
                # close = res_df['close'].to_numpy()
                # if close[self.config.trader_set.complete_index] <
                ...
                if market_close_on:
                    ...
                    if not self.config.trader_set.backtrade and not fake_order:
                        ...
                        out_exec_price_list = self.market_close_order_v2(code_org, post_order_res_list, close_side, pos_side, open_exec_qty)
                    else:
                ...
                if all_executed or market_close_on:
                    ...
                    if self.config.trader_set.messenger_on:
                        self.watch_data_dict.pop(code)
                        #   ii. code duplication 을 고려해, stop_socket 사용하지 않는다.
                        # self.websocket_client.stop_socket("{}@aggTrade".format(code.lower()))
