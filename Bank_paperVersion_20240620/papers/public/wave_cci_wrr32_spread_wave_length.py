...
def lvrg_liqd_set_v2_2(res_df, config, open_side, ep_, tp_, fee, limit_leverage=50):
    ...
    if not pd.isnull(tp_):
        ...
        if loss is not None:
            leverage = config.lvrg_set.target_pct / abs(loss * (1 - (fee + config.trader_set.market_fee)) - 1)
...
def sync_check(df_, config, mode="OPEN", row_slice=True):
    ...
    try:
        ...
        htf_df_list = [to_htf(df_, itv=itv_, offset=offset_) for itv_idx, (itv_, offset_) in enumerate(zip(make_itv_list, offset_list)) if itv_idx != 0]
...
def public_indi(res_df, config, np_timeidx, mode="OPEN"):
    ...
    try:
        ...
        if itv_to_number(wave_itv1) > 1:
            ...
            res_df = res_df.join(to_lower_tf_v4(res_df, htf_df, cols, backing_i=0, ltf_itv='T'), how='inner')
