...
def lvrg_liqd_set_v4_1(res_df, config, open_side, ep_, out_, fee, multiplier=1000, log_base=30, limit_leverage=50):

    """
    v4 --> v4_1
        1. set log based dynamic_lvrg algorithm.
    ...
    if not pd.isnull(out_):
        loss_fee = config.trader_set.limit_fee + config.trader_set.market_fee
        if open_side == OrderSide.SELL:
            loss = abs((ep_ / out_) * (1 - loss_fee) - 1)
        else:
            loss = abs((out_ / ep_) * (1 - loss_fee) - 1)

        leverage = loss * multiplier
        sys_log.warning("leverage : {}".format(leverage))
        leverage = math.log(leverage, log_base) if leverage > 0 else 1
        sys_log.warning("leverage (log adj.) : {}".format(leverage))
        leverage = int(leverage)
        sys_log.warning("leverage (int) : {}".format(leverage))

    if not config.lvrg_set.allow_float:
        leverage = int(leverage)

    # 1. dynamic_lvrg 사용 경우, 감당하기 힘든 변동성의 경우 진입하지 않음.
    if leverage < 1 and config.lvrg_set.lvrg_rejection:
        return None, None

    # 2. leverage < limit_leverage
    leverage = min(limit_leverage, max(leverage, 1))

    # 3. get liquidation price.
    if open_side == OrderSide.SELL:
