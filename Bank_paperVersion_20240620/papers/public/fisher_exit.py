...
def sync_check_v2(df_, config, mode="OPEN", row_slice=True, tf=None):
    try:
        ...
        if tf == '3T':
            if 'fisher_3T30' not in df.columns:
                df_3T = fisher_v2(df_3T, 30, itv='3T')
                df = df.join(to_lower_tf_v4(df, df_3T, df_3T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        if tf == '5T':
            if 'fisher_5T30' not in df.columns:
                df_5T = fisher_v2(df_5T, 30, itv='5T')
                df = df.join(to_lower_tf_v4(df, df_5T, df_5T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능

        # ------ 15T ------ #
        if tf == '15T':
            if 'fisher_15T30' not in df.columns:
                df_15T = fisher_v2(df_15T, 30, itv='15T')
                df = df.join(to_lower_tf_v4(df, df_15T, df_15T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        # ------ 30T ------ #
        if tf == '30T':
            if 'fisher_30T30' not in df.columns:
                df_30T = fisher_v2(df_30T, 30, itv='30T')
                df = df.join(to_lower_tf_v4(df, df_30T, df_30T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        # ------ H ------ #
        if tf == 'H':
            if 'fisher_H30' not in df.columns:
                df_H = fisher_v2(df_H, 30, itv='H')
                df = df.join(to_lower_tf_v4(df, df_H, df_H.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        # ------ 4H ------ #
        if tf == '4H':
            if 'fisher_4H30' not in df.columns:
                df_4H = fisher_v2(df_4H, 30, itv='4H')
                df = df.join(to_lower_tf_v4(df, df_4H, df_4H.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
...
def sync_check(df_, config, mode="OPEN", row_slice=True):
    ...
    try:
        ...
        # ------ 3T ------ #
        # df = dc_line(df, df_3T, '3T')
        if config.loc_set.point1.tf_entry == '3T':
            if 'fisher_3T30' not in df.columns:
                df_3T = fisher_v2(df_3T, 30, itv='3T')
                df = df.join(to_lower_tf_v4(df, df_3T, df_3T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        # df = dc_line(df, df_5T, '5T')
        # df = bb_line(df, df_5T, '5T')
        if config.loc_set.point1.tf_entry == '5T':
            if 'fisher_5T30' not in df.columns:
                df_5T = fisher_v2(df_5T, 30, itv='5T')
                df = df.join(to_lower_tf_v4(df, df_5T, df_5T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능

        # ------ 15T ------ #
        if config.loc_set.point1.tf_entry == '15T':
            if 'fisher_15T30' not in df.columns:
                df_15T = fisher_v2(df_15T, 30, itv='15T')
                df = df.join(to_lower_tf_v4(df, df_15T, df_15T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        # ------ 30T ------ #
        if config.loc_set.point1.tf_entry == '30T':
            if 'fisher_30T30' not in df.columns:
                df_30T = fisher_v2(df_30T, 30, itv='30T')
                df = df.join(to_lower_tf_v4(df, df_30T, df_30T.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        # ------ H ------ #
        if config.loc_set.point1.tf_entry == 'H':
            if 'fisher_H30' not in df.columns:
                df_H = fisher_v2(df_H, 30, itv='H')
                df = df.join(to_lower_tf_v4(df, df_H, df_H.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
        ...
        # ------ 4H ------ #
        if config.loc_set.point1.tf_entry == '4H':
            if 'fisher_4H30' not in df.columns:
                df_4H = fisher_v2(df_4H, 30, itv='4H')
                df = df.join(to_lower_tf_v4(df, df_4H, df_4H.columns[-1:], backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
...
def public_indi(res_df, config, np_timeidx, mode="OPEN"):
    ...
    #                 # ------ 필요한 cols 만 join (htf's idx 정보는 ltf 와 sync. 가 맞지 않음 - join 불가함) ------ #
    #                 res_df.drop(cols, inplace=True, axis=1, errors='ignore')
    #                 res_df = res_df.join(to_lower_tf_v4(res_df, htf_df, cols, backing_i=0, ltf_itv='T'), how='inner')  # tf_entry 진입이면, backing_i = 0 가 가능
    ...
    # res_df = ma_v2(res_df, 30, itv='T')
    # res_df = ma_v2(res_df, 60, itv='T')
    # res_df = ma_v2(res_df, 120, itv='T')
...
def ep_loc_p1_v3(res_df, config, np_timeidx, show_detail=True, ep_loc_side=OrderSide.SELL):
    ...
    if config.loc_set.zone1.use_zone:
        ...
        short_ep_ = res_df['short_ep1_{}'.format(selection_id)].to_numpy()
        long_ep_ = res_df['long_ep1_{}'.format(selection_id)].to_numpy()

        #    0. ma
        ma_T30 = res_df['ma_T30'].to_numpy()
        ma_T60 = res_df['ma_T60'].to_numpy()
        ma_T120 = res_df['ma_T120'].to_numpy()

        if ep_loc_side == OrderSide.SELL:
            mr_res *= (ma_T30 < ma_T60) & (ma_T60 < ma_T120)
            # mr_res *= (ma_T30 < short_ep_)
            mr_res *= (short_ep_ < ma_T60)
            if show_detail:
                sys_log.warning("(ma_T30 < ma_T60) & (ma_T60 < ma_T120) : {:.5f} {:.5f} {:.5f} ({})".format(ma_T30[c_i], ma_T60[c_i], ma_T120[c_i], mr_res[c_i]))
                # sys_log.warning("(ma_T30 < short_ep_) : {:.5f} {:.5f} ({})".format(ma_T30[c_i], short_ep_[c_i], mr_res[c_i]))
                sys_log.warning("(short_ep_ < ma_T60) : {:.5f} {:.5f} ({})".format(ma_T30[c_i], short_ep_[c_i], mr_res[c_i]))
        else:
            mr_res *= (ma_T30 > ma_T60) & (ma_T60 > ma_T120)
            # mr_res *= (ma_T30 > long_ep_)
            mr_res *= (long_ep_ > ma_T60)
            if show_detail:
                sys_log.warning("(ma_T30 > ma_T60) & (ma_T60 > ma_T120) : {:.5f} {:.5f} {:.5f} ({})".format(ma_T30[c_i], ma_T60[c_i], ma_T120[c_i], mr_res[c_i]))
                # sys_log.warning("(ma_T30 > long_ep_) : {:.5f} {:.5f} ({})".format(ma_T30[c_i], long_ep_[c_i], mr_res[c_i]))
                sys_log.warning("(long_ep_ > ma_T60) : {:.5f} {:.5f} ({})".format(ma_T30[c_i], long_ep_[c_i], mr_res[c_i]))
