...
def sync_check(df_, config, order_side="OPEN", row_slice=True):
    try:
        ...
        if row_slice:  # recursive 가 아닌 indi. 의 latency 를 고려한 slicing
            df, df_3T, df_5T, df_15T, df_30T, df_4H = [df_s.iloc[-row_list[row_idx]:].copy() for row_idx, df_s in enumerate(htf_df_list)]
            rec_df, rec_df_3T, rec_df_5T, rec_df_15T, rec_df_30T, rec_df_4H = [df_s.iloc[-rec_row_list[row_idx]:].copy() for row_idx, df_s in
                                                                    enumerate(htf_df_list)]
        else:
            df, df_3T, df_5T, df_15T, df_30T, df_4H = htf_df_list
            rec_df, rec_df_3T, rec_df_5T, rec_df_15T, rec_df_30T, rec_df_4H = htf_df_list
...
def public_indi(res_df, config, np_timeidx, order_side="OPEN"):
    ...
    res_df = wave_range_v6(res_df, config.loc_set.point.p1_period1, config.loc_set.point.p1_period2, ltf_df=None, touch_period=50)
    if config.loc_set.point.p2_itv1 != "None":
        res_df = wave_range_v6(res_df, config.loc_set.point.p2_period1, config.loc_set.point.p2_period2, ltf_df=None, touch_period=50)
