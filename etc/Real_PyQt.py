    ...
    def buy_clicked(self, Coin, display):  # 엔터치면 매수 동작하는 걸로 가능? = returnPressed()
        ...
        try:
            ...
            if self.krw < 1000:
                ...
                print()
            buyunit = int((self.krw * 0.996 / limit_buy_price) * 10000) / 10000.0
...
while True:
    ...
    if len(candis) != 0:
        ...
        for Coin in candis:
            data = Funcs_OBV.get_obv_min(Coin)
            print(Coin, data)
            # if (data[0] > 1) and (data[1] > 0) and (pybithumb.get_current_price(Coin) > 1):
            candis2.append(Coin)
        ...
        # else:
            ...
            for Coin in candis2:
                ...
                print("%s. %s %.2f" % (i, Coin, ratio2[i - 1]))
                i += 1
            print()
            ratio2 = list(map(lambda x: int(x * 10000) / 10000.0, ratio2))

            window = OrderWindow(candis2, ratio2)
            window.show()
            app.exec_()
