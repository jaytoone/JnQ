...
class Trader:
    ...
    def __init__(self, utils_public, utils_list, config_list):
        ...
        self.utils1, self.utils2, self.utils3 = utils_list
        self.utils_list = utils_list   # Todo, 꺼낼 이유가 있나, list 로 indexing 가능한데
        ...
        while 1:
            ...
            while 1:
                ...
                if load_new_df:
                    ...
                    # ------------ get indi.  ------------ #
                    try:
                        res_df = self.utils_public.sync_check(res_df, config)  # function usage format maintenance
                        sys_log.info('~ sync_check time : %.5f' % (time.time() - temp_time))

                        np_timeidx = np.array(list(map(lambda x: intmin(x), res_df.index)))

                        res_df = self.utils_public.public_indi(res_df, np_timeidx, config)
                ...
                while 1:
                    ...
                    if use_new_df2:
                        ...
                        if load_new_df2:  # dynamic_out & tp phase
                            ...
                            try:
                                res_df = self.utils_public.sync_check(res_df, config, order_side="CLOSE")

                                np_timeidx = np.array(list(map(lambda x: intmin(x), res_df.index)))

                                res_df = self.utils_public.public_indi(res_df, np_timeidx, config, order_side="CLOSE")
