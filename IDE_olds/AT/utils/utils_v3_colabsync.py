...
def long_ep_loc(res_df, config, i):
    ...
    if config.loc_set.spread != "None":
        ...
        mr_const_cnt += 1
        if config.loc_set.spread_ep_gap != "None":
            if (res_df['long_tp'].iloc[i] - res_df['long_tr_ep'].iloc[i] - tp_fee * res_df['long_tr_ep'].iloc[i]) / (
                    res_df['long_tr_ep'].iloc[i] - res_df['long_out'].iloc[i] + out_fee * res_df['long_tr_ep'].iloc[
                i]) >= config.loc_set.spread:
                mr_score += 1
