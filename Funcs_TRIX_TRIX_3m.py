...
def profitage(Coin, long_signal_value, short_signal_value, long_period, short_period, wait_tick=3, Date='2019-09-25',
    ...
    dc_low_entry_low_accept_gap_range = 0.1
    ...
    for i in range(2, len(df)):
        ...
        if df['TRIX_LATER'].iloc[i] <= long_signal_value < df['TRIX_LATER'].iloc[i - 1]: #and df['close'].iloc[i] > clearance(df['SMMA'].iloc[i] * smma_accept_range):
            for j in range(i + 1, len(df)):
                #       FIND TRIX CROSS     #
                if df['TRIX_TREND'].iloc[j] == 'UP' and df['TRIX_TREND'].iloc[j - 1] == 'DOWN':
                    # if df['TRIX_GAUSSIAN'].iloc[j] - df['TRIX_GAUSSIAN'].iloc[j - 1] >= 1.0:
                    #     if df['Fisher_long'].iloc[j] < 0:
                    if df['DC_TREND'].iloc[j] == 'UP':
                    # and upper_line > df['TRIX_GAUSSIAN'].iloc[j] >= lower_line\
                    # and df['TRIX'].iloc[j] != 0:  # 초기에 진입하는거를 방지하고 TRIX와 비교하기 위해서
                        trade_state[j] = 1.5
                        print('ENTRY TRIX GAP :', j, (df['TRIX'].iloc[j] - df['TRIX'].iloc[j - 1]) / (df['TRIX'].max() - df['TRIX'].min()))
                    ...
                    #       DC가 DOWN일 경우 DOWN의 시작점과 진입 지점사이의 GAP을 조사한다.     #
                        ...
                        while True:
                            ...
                            if copy_j == 0:
                                break
                        print('DC GAP :', copy_j, j, (df['DC_LOW'].iloc[j] - df['DC_LOW'].iloc[copy_j]) / chart_height)
                        if (df['DC_LOW'].iloc[j] - df['DC_LOW'].iloc[copy_j]) / chart_height > dc_accept_gap_range:
                            trade_state[j] = 1.5
                            print('ENTRY TRIX GAP :', j, (df['TRIX'].iloc[j] - df['TRIX'].iloc[j - 1]) / (
                                        df['TRIX'].max() - df['TRIX'].min()))

                        # break

                if df['TRIX_LATER'].iloc[j] > long_signal_value:
                    break
                    ...
                    # while True:
                    #     copy_j -= 1
                    #     if df['TRIX_GAUSSIAN_TREND'].iloc[copy_j - 1] == 'UP' and df['TRIX_GAUSSIAN_TREND'].iloc[copy_j] == 'DOWN':
                    #         break
                    #     if copy_j == 0:
                    #         break
                    #
                    # if copy_j != 0:
                    #     if df['TRIX_GAUSSIAN'].iloc[copy_j] - df['TRIX_GAUSSIAN'].iloc[j] >= trix_vertical_gap:
                    #         print('TRIX_GAUSSIAN GAP :', j, df['TRIX_GAUSSIAN'].iloc[copy_j] - df['TRIX_GAUSSIAN'].iloc[j])
                    #         trade_state[j] = 1.5
    ...
    for i in range(1, len(target)):
        ...
        if df['TRIX'].iloc[i] >= df['TRIX_LATER'].iloc[i] and df['TRIX'].iloc[i - 1] < df['TRIX_LATER'].iloc[i - 1]:
    ...
    for long_signal_value in np.arange(-4, -1.5, 0.5):
        for short_signal_value in np.arange(1.5, 4, 0.5):
            for short_period in np.arange(60, 500, 100):
                ...
                long_signal_value, short_signal_value = 0.1, 1.9
