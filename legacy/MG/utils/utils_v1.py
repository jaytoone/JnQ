...
def enlist_rtc(res_df, config, user_rtc_1):
    ...
    res_df['h_short_rtc_1'] = user_rtc_1
...
def long_ep_loc(res_df, config, i):
    ...
    if config.loc_set.dt_k != "None":
        ...
        #     dtk_v1 & v2 platform    #
        if config.loc_set.dtk_dc_itv != "None":
            if res_df['dc_upper_%s' % config.loc_set.dtk_dc_itv].iloc[i] <= res_df['long_dtk_1'].iloc[i] + \
