...
def low_high(reg_target, sigma, crop_size=0, realtime=0):
    ...
    if crop_size != 0:
        ...
        realtime_y_smooth_0 = [np.nan] * len(reg_target)
        ...
        for i in range(none_indexs + crop_size, len(reg_target)):
            ...
            # y_smooth_0 = gaussian_filter1d(y_smooth, sigma=5)
            for iter_num in range(1):
                y_smooth = gaussian_filter1d(y_smooth, sigma=sigma)

            print(i)
            # realtime_y_smooth_0[i] = y_smooth_0[-1]
            realtime_y_smooth[i] = y_smooth[-1]

            # if realtime == 1:
            #     if realtime_y_smooth_0[i] > realtime_y_smooth[i]:
            #         trade_state[i] = 'up'
            #     else:
            #         trade_state[i] = 'down'

            if realtime == 1:
                if reg_target[i] > realtime_y_smooth[i]:
                    trade_state[i] = 'up'
                else:
                    trade_state[i] = 'down'
        ...
        # sigma0 와 sigma1의 cross 비교
        # if realtime == 0:
        #     for i in range(1, len(realtime_y_smooth)):
        #
        #         if realtime_y_smooth_0[i] > realtime_y_smooth[i]:
        #             trade_state[i] = 'up'
        #         elif realtime_y_smooth_0[i] <= realtime_y_smooth[i]:
        #             trade_state[i] = 'down'
        # return realtime_y_smooth_0, realtime_y_smooth, trade_state

        # macd 와 gaussian의 cross 비교
        if realtime == 0:
            for i in range(1, len(realtime_y_smooth)):

                if reg_target[i] > realtime_y_smooth[i]:
                    trade_state[i] = 'up'
                elif reg_target[i] <= realtime_y_smooth[i]:
                    trade_state[i] = 'down'
        return reg_target, realtime_y_smooth, trade_state
...
def profitage(Coin, reg_target, short, long, signal, sigma, crop_size, realtime, Date='2019-09-25', excel=0, get_fig=0):
    ...
    if len(profits) == 0:
        ...
        plt.subplot(312)
        plt.plot(df['osc_reg_0'], 'orange', label='gaussian_0')
        plt.plot(df['osc_reg'], 'fuchsia', label='gaussian_1')
        plt.xlim(0, len(ochl))
...
if __name__ == "__main__":
    ...
    # print(TopCoin)
    # quit()
    TopCoin = ['BTG', 'BSV', 'WOM', 'BCD', 'LINK', 'TMTG', 'PLX', 'WICC', 'QTUM', 'FLETA', 'XSR', 'EM', 'ETC', 'BCH', 'FX', 'DAD', 'KNC', 'ZRX', 'DASH', 'IOST']
    # TopCoin = ['TMTG']
                ...
                for Coin in TopCoin:
                    ...
                    # try:
                        print(Coin, profitage(Coin, 'MACD_OSC',
                                              short, long, signal, sigma=10, crop_size=30, realtime=0, Date=Date, get_fig=1, excel=0))
