            ...
            while 1:
                ...
                if entry_const_check:
                    ...
                    try:
                        ...
                        under_top = res_df['close'].shift(self.close_shift_size) <= cloud_top.shift(self.self.cloud_shift_size)
                        over_bottom = res_df['close'].shift(self.close_shift_size) >= cloud_bottom.shift(self.self.cloud_shift_size)
                        ...
                        # ---------------------- short const phase ---------------------- #
                        # ----------- cloud color lb const ----------- #
                        if np.sum(res_df["senkou_a1"].shift(self.cloud_shift_size).iloc[i + 1 - self.cloud_lookback:i + 1] <=
                                  res_df["senkou_b2"].shift(self.cloud_shift_size).iloc[i + 1 - self.cloud_lookback:i + 1]) == self.cloud_lookback and \
                                np.sum(res_df["senkou_a2"].shift(0).iloc[i + 1 - self.cloud_lookback:i + 1] <=
                                       res_df["senkou_b2"].shift(0).iloc[i + 1 - self.cloud_lookback:i + 1]) == self.cloud_lookback:  # and \
                        ...
                        # ----------- cloud color lb const ----------- #
                        if np.sum(res_df["senkou_a1"].shift(self.cloud_shift_size).iloc[i + 1 - self.cloud_lookback:i + 1] >=
                                  res_df["senkou_b1"].shift(self.cloud_shift_size).iloc[i + 1 - self.cloud_lookback:i + 1]) == self.cloud_lookback and \
                                np.sum(res_df["senkou_a2"].shift(0).iloc[i + 1 - self.cloud_lookback:i + 1] >=
                                       res_df["senkou_b2"].shift(0).iloc[i + 1 - self.cloud_lookback:i + 1]) == self.cloud_lookback:  # and \
