        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, watch_data in self.watch_data_dict.copy().items():
                    ...
                    watch_code_dict[code] = (watch_data['open_side_msg'], watch_data['h_line'])
                    self.sys_log.warning("{} {} {} added to watch_code_dict. (watch_data_dict)".format(code, watch_data["open_side_msg"], watch_data['h_line']))
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, open_remain_data in self.open_remain_data_dict.copy().items():
                    ...
                    #   b. open_remain_data 에 h_line 은 없을뿐더러, 의미가 없기에 None 으로 대체한다.
                    watch_code_dict[code] = (open_remain_data['open_side_msg'], None)
                    self.sys_log.warning("{} {} added to watch_code_dict. (open_remain_data_dict)".format(code, open_remain_data["open_side_msg"], None))
        ...
        try:
            ...
            with open(data_path, 'rb') as f:
                ...
                for code, close_remain_data in self.close_remain_data_dict.copy().items():
                    ...
                    #       a. 존재하는 미체결내역에 대해 get_new_df 를 위한 add to watch_code_dict.
                    watch_code_dict[code] = (close_remain_data['open_side_msg'], None)
                    self.sys_log.warning("{} {} added to watch_code_dict (close_remain_data_dict).".format(code, close_remain_data["open_side_msg"], None))
        ...
        while 1:
            ...
            if not self.config.trader_set.backtrade:
                    ...
                    if self.config.trader_set.messenger_on:
                        ...
                        #   iii. payload check.
                            ...
                            if pay_len == 2:
                                ...
                                code, open_side_msg, h_line = payload
                            ...
                            if code in valid_code_list and open_side_msg in [OrderSide.BUY, OrderSide.SELL]:
                                #   a. allow watch duplication.
                                # if code in self.watch_data_dict.keys():
                                code += "_{}".format(str(now.timestamp()).split('.')[0])
                                watch_code_dict[code] = (open_side_msg, h_line)
                                msg = "{} {} {} added to watch_code_dict.".format(code, open_side_msg, h_line)

                    #   b. watch
                    if "watch" in self.user_text.lower():
                        msg = "watch response."
                        for code, watch_data in self.watch_data_dict.items():
                            msg += "\nwatch : {}, open_side_msg : {}, h_line : {}".format(code, watch_data['open_side_msg'], watch_data['h_line'])
                ...
                #   3. Messenger off 시, default code = self.config.trader_set.symbol
                    ...
                    if not self.config.trader_set.messenger_on:
                        if len(self.watch_data_dict) == 0:
                            ...
                            watch_code_dict[code] = (open_side_msg, None)

                #   4. add data to watch_data_dict.
                for code, (open_side_msg, h_line) in watch_code_dict.items():
                    if '_' in code:
                        code_temp = code.split('_')[0]
                    else:
                        code_temp = code
                    #   i. add websocket.
                    self.websocket_client.agg_trade(symbol=code_temp, id=1, callback=self.agg_trade_message_handler)
                    #   ii. set posmode_margin_leverage.
                    limit_leverage = self.posmode_margin_leverage(code_temp)
                    self.watch_data_dict[code] = {"open_side_msg": open_side_msg, "h_line": h_line, "limit_leverage": limit_leverage, "ep_loc_point2": 0, "streamer": None}
                ...
                if len(self.watch_data_dict) == 0:
                    ...
                    self.watch_data_dict[code] = {"open_side_msg": None, "h_line": None, "limit_leverage": 50, "ep_loc_point2": 0, "streamer": self.get_streamer()}
            ...
            if self.allow_watch:
                ...
                for w_i, (code, watch_data) in enumerate(self.watch_data_dict.copy().items()):
                    ...
                    open_side_msg, h_line, limit_leverage, ep_loc_point2, streamer = watch_data.values()
                    self.sys_log.warning("WATCH : {} {}".format(code, open_side_msg))

                    # -1. allow watch duplication.
                    code_org = code
                    if '_' in code_org:
                        code = code_org.split('_')[0]
                    ...
                    if h_line is not None:
                        ...
                        from funcs.public.broker import to_htf

                        h_line = float(h_line)
                        res_df_15T = to_htf(res_df, itv='15T', offset='1h')
                        close_ = res_df_15T['close'].to_numpy()[self.config.trader_set.complete_index]
                        if open_side_msg == OrderSide.SELL:
                            if not close_ < h_line:
                                continue
                        else:
                            if not close_ > h_line:
                                continue
                    ...
                    if open_side is not None:
                        ...
                        #       v. add websocket for deletion by duplicated code.
                        if code not in self.market_price.keys():
                        ...
                        #           i. 이후에 code 기준으로 pop 하는 경우 keyerror 를 방지하기 위함.
                        if '_' in code_org:
                            #           1. 진행중 code 는 valid_code 로 watch_data_dict 에 입력한다.
                            self.watch_data_dict[code] = watch_data
                            self.watch_data_dict.pop(code_org)
