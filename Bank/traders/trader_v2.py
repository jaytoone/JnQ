...
class Trader:
    ...
    def get_balance_info(self, bank_module, mode="PROD"):  # bank_module 상속이 아니므로, 인자값으로 받도록 설정함.
    ...
    def run(self):
        ...
        while 1:
            ...
            # ------ open param. init by every_trades ------ # loop 별로 param check (Todo)
            get_watch = 1
            ...
            while 1:
                ...
                if get_watch:
                    try:
                        ...
                        get_watch = 0   # dynamic watch 사용할 경우 off.
                ...
                for w_i, watch_data in zip(reversed(range(len(self.watch_data_queue))), reversed(self.watch_data_queue)):
                    ...
                    if open_side is None:  # open_signal not exists, check ep_loc
                        ...
                        if open_side is not None:
                            ...
                            # a. market_check_term for market_entry - deprecated
                            # #       i. 바로 진행할 경우, data loading delay 로 인한 오류 방지
                            ...
                            # z. queue result
                            self.open_data_queue.append([code, res_df_open, res_df, fee])
                ...
                for o_i, open_data in zip(reversed(range(len(self.open_data_queue))), reversed(self.open_data_queue)):
                    ...
                    code, res_df_open, res_df, fee = open_data

                    # 0. param init.
                    fake_order = 0  # order 하는 것처럼 시간을 소비하기 위함임.

                    # 1. get balance
                    min_balance_bool = self.get_balance_info(bank_module, mode=self.config.trader_set.profit_mode)
                    if min_balance_bool:
                        continue
                        # break
                    self.sys_log.info('OPEN : ~ get balance : %.5f' % (time.time() - start_time))

                    # 2. get tr_set, leverage
                    ...
                    if not self.config.trader_set.backtrade and not fake_order:
                        ...
                        if not error_code:  # order deny exception, error_code = 0
                            # z. queue result
                            self.open_exec_data_queue.append([code, order_no, tp, ep, out, liqd_p, leverage, fee, open_side, open_quantity, res_df_open, res_df, fake_order])
                ...
                for oe_i, open_exec_data in zip(reversed(range(len(self.open_exec_data_queue))), reversed(self.open_exec_data_queue)):
                    ...
                    code, order_no, tp, ep, out, liqd_p, leverage, fee, open_side, open_quantity, res_df_open, res_df, fake_order = open_exec_data
                    ...
                    # open_exec 2 : limit
                        ...
                        if not self.config.trader_set.backtrade:
                            ...
                            expired = bank_module.check_ei_k_v2(res_df_open, res_df, open_side, realtime_price)
                    ...
                    if expired or open_exec:
                        ...
                        if open_exec:
                            ...
                            # z. queue result
                            self.close_data_queue.append([code, tp, ep, out, liqd_p, leverage, fee, open_exec_price_list, open_exec_qty, real_balance, fake_order])
                ...
                for cl_i, close_data in zip(reversed(range(len(self.close_data_queue))), reversed(self.close_data_queue)):
                    ...
                    code, tp, ep, out, liqd_p, leverage, fee, open_exec_price_list, open_exec_qty, real_balance, fake_order = close_data
                    ...
                    if not self.config.trader_set.backtrade and not fake_order:
                        ...
                        # z. queue result
                        if not error_code:
                            self.close_exec_data_queue.append([code, order_no_list, partial_tps, ep, out, liqd_p, leverage, fee, close_side, open_exec_price_list,
                                                               open_exec_qty, quantity_precision, real_balance, fake_order, 0, {}])   # add 0, {} for 누적형 parameter : prev_exec_tp_len, ex_dict
                ...
                for ce_i, close_exec_data in zip(reversed(range(len(self.close_exec_data_queue))), reversed(self.close_exec_data_queue)):
                    ...
                    code, order_no_list, partial_tps, ep, out, liqd_p, leverage, fee, close_side, open_exec_price_list,\
                    open_exec_qty, quantity_precision, real_balance, fake_order, prev_exec_tp_len, ex_dict = close_exec_data
                    ...
                    # 1. check tp execution
                    if not self.config.trader_set.backtrade and not fake_order:
                        # [ back & real-trade validation phase ]

                        #       2. CLOSE EXECUTION LOOP 를 위한 data.
                    ...
                    #   [ back & real-trade validation phase ]
                    if not self.config.trader_set.backtrade:
                        market_close_on, log_out = bank_module.check_hl_out_v2(res_df, market_close_on, out, liqd_p, open_side)
                    else:
                    ...
                    if all_executed or market_close_on:
                        ...
                        #   z. queue result
                        #     self.watch_data_queue.append([order_no_list, 0, {}])  # 거래 완료된 종목은 재거래, 무엇을 붙여주어야하나 (Todo)
