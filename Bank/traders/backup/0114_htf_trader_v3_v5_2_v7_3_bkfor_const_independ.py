...
def read_dump_cfg_list(cfg_path_list, mode='r', edited_cfg_list=None):
...
class Trader:
    ...
    def run(self):
        ...
        while 1:
            ...
            try:
                cfg_list = read_dump_cfg_list(cfg_path_list)
            ...
            if initial_run:
                ...
                read_dump_cfg_list(cfg_path_list, mode='w', edited_cfg_list=cfg_list)
                    ...
                    if self.utils is None:
                        try:
                            ...
                            if res_df['entry_{}'.format(config1.strat_version)][
                                ...
                                sys_log.warning("ep_loc.point executed strat_ver : {}".format(self.config.strat_version))

                                # initial_ep = res_df['short_ep_{}'.format(config.strat_version)].iloc[
                                #     config.trader_set.last_index]

                                if not config.pos_set.short_ban:
                                    res_df, open_side, _ = self.utils_public.short_ep_loc(res_df, self.config,
                                                                                          self.config.trader_set.last_index,
                                                                                          np_timeidx)
                                    # ------ additional ep_loc by co-point ------ #
                                    if not config3.pos_set.short_ban:  # additional const. on trader
                                        if self.config.strat_version == 'v5_2' and open_side is None:
                                            sys_log.warning(
                    ...
                    if use_new_df2:
                        ...
                        if load_new_df2:  # dynamic_out & tp phase
                            ...
                            try:
                                ...
                                res_df = self.utils_public.public_indi(res_df, config, np_timeidx, order_side="CLOSE")
