"""
trader_v1_1
1. paper_name & id_list 를 Trader 내부로 가져와 log_dir_name 에 적용시킴.
2. back & real-trade validation phase : ei_k / open / cancel / limit_tp / market_close
"""
...
class Trader:
    ...
    def run(self):
        ...
        # ------ path definition ------ #
        # self.log_dir_name = Path(__file__).stem

        log_path_list = ["sys_log", "trade_log", "df_log"]
        sys_log_path, trade_log_path, df_log_path = [os.path.join(pkg_path, "logs", path_, self.log_dir_name) for path_ in
                                                     log_path_list]

        for path_ in [sys_log_path, trade_log_path, df_log_path]:
            os.makedirs(path_, exist_ok=True)

        cfg_path_list = [os.path.join(pkg_path, "papers/config", name_) for name_ in self.config_name_list]
        ...
        while 1:
            ...
            if initial_set:
                ...
                src_cfg_path = sys_log_path.replace(self.log_dir_name, "base_cfg.json")  # dir 와 같은 수준에 위치하는 baes_cfg.json 가져옴
            ...
            while 1:  # <-- loop for 'check order type change condition'
                ...
                self.available_balance, self.over_balance, min_balance_bool = get_balance(self, first_iter, cfg_path_list)
                if min_balance_bool:
                    break
