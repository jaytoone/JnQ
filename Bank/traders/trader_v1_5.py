from funcs.binance.bank_module_v3 import BankModule
...
class Trader:
    ...
    def run(self):
        ...
        2. self.config update 이후로는 BankModule 재선언할 것. --> websocket 중복을 회피하기 위해 재선언하지 않는다.
        """
        ...
        while 1:
            ...
            while 1:
                if load_new_df:
                    ...
                    if open_side is None:  # open_signal not exists, check ep_loc
                        ...
                        1. open_signal 발생의 경우
                            ...
                            b. BankModule new config 로 재선언 --> websocket error 문제로, 재선언하지 않음, 추가로 shield version 에서는 self.config 에 종속되지 않기 때문에 재선언할 필요가 없어진다.
                        """
                ...
                while 1:
                    ...
                    if str(res_df.index[-1]).split(':')[-1] == '00':
                        last_row_timestamp = datetime.timestamp(res_df.index[-1]) + 59.999
                    else:
                        last_row_timestamp = datetime.timestamp(res_df.index[-1])

                    if self.config.trader_set.backtrade or last_row_timestamp < datetime.now().timestamp():
                        ...
                        if not self.config.trader_set.backtrade:
                            self.sys_log.info("last_row_timestamp : {}".format(last_row_timestamp))
            ...
            while 1:
                ...
                if not self.config.trader_set.backtrade:
                    while 1:
                        try:
                            bank_module.change_leverage(symbol=self.config.trader_set.symbol, leverage=leverage, recvWindow=6000)
                        except Exception as e:
                ...
                if not self.config.trader_set.backtrade and not fake_order:
                    ...
                    if res_code:  # order deny exception occurs, res_code = 0
                        break
                ...
                # ------ 2. limit : check expired (= ei_k & breakout_qty) ------ #
                    ...
                    if self.config.trader_set.backtrade:
                        ...
                        while 1:
                            ...
                            # ------ entry ------ #
                            if fake_order:
                                realtime_price = bank_module.get_market_price_v3()
                                if open_side == OrderSide.BUY:
                ...
                if not self.config.trader_set.backtrade:
                    while 1:
                        ...
                        if datetime.now().timestamp() > last_row_timestamp:
                            break
                        else:
                ...
                while 1:
                    ...
                    while 1:
                        ...
                        if not self.config.trader_set.backtrade:
                            ...
                            if datetime.now().timestamp() > last_row_timestamp:   # database timestamp 형태에 따라 다소 변경될 여지가 있음.
