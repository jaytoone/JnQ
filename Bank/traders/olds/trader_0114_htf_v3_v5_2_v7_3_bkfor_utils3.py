...
from funcs.binance.order_logger import limit_order, partial_limit_v2, market_close_order
from funcs.public.broker import intmin, calc_rows_and_days
...
class Trader:
    ...
    def __init__(self, utils_list, config_list):
        ...
        self.utils_public, self.utils1, self.utils2, self.utils3 = utils_list
    ...
    def run(self):
        ...
        cfg_full_path1, cfg_full_path2, cfg_full_path3 = cfg_path_list
        ...
        while 1:
            ...
            if initial_run:
                ...
                with open(cfg_full_path2, 'w') as cfg:
                    json.dump(config2, cfg, indent=2)
                with open(cfg_full_path3, 'w') as cfg:
                    json.dump(config3, cfg, indent=2)
            ...
            while 1:
                ...
                if load_new_df:
                    ...
                    try:
                        use_rows, days = calc_rows_and_days(config.trader_set.itv_list, config.trader_set.row_list)

                        new_df_, _ = concat_candlestick(config.trader_set.symbol, '1m',
                                                        days=days,
                                                        limit=use_rows,
                                                        timesleep=None,
                                                        show_process=0)
                    ...
                    # ------------ get indi.  ------------ #
                    try:
                        res_df = self.utils_public.sync_check(res_df, config.trader_set.row_list)  # function usage format maintenance
                        sys_log.info('~ sync_check time : %.5f' % (time.time() - temp_time))

                        np_timeidx = np.array(list(map(lambda x: intmin(x), res_df.index)))

                        res_df = self.utils_public.public_indi(res_df, np_timeidx)
                    ...
                    # ------------ enlist_rtc (form 만큼 utils 사용, config type 만큼 설정 필요함) ------------ #
                    try:
                        #        ep_loc_point2 에 따라 다르게 수행        #
                        if ep_loc_point2:
                            res_df = self.utils.enlist_rtc(res_df, config, np_timeidx)
                        else:
                            res_df = self.utils1.enlist_rtc(res_df, config1, np_timeidx)
                            res_df = self.utils2.enlist_rtc(res_df, config2, np_timeidx)
                            res_df = self.utils2.enlist_rtc(res_df, config3, np_timeidx)
                    ...
                    # ------------ enlist_tr (form 만큼 utils 사용, config type 만큼 설정 필요함) ------------ #
                    try:
                        #        1. ep_loc_point2 에 따라 다르게 수행        #
                        ...
                        if ep_loc_point2:
                            ...
                            res_df = self.utils2.enlist_tr(res_df, config3, np_timeidx)
                    ...
                    if self.utils is None:
                        try:
                            ...
                            if res_df['entry_{}'.format(config1.strat_version)][
                                ...
                                sys_log.warning("ep_loc.point executed strat_ver : {}".format(config.strat_version))
                                ...
                                if not config.pos_set.short_ban:
                                    res_df, open_side, _ = self.utils_public.short_ep_loc(res_df, config,
                                                                                          config.trader_set.last_index,
                                                                                          np_timeidx)
                                    # ------ additional ep_loc by co-point ------ #
                                    if not config3.pos_set.short_ban:  # additional const. on trader
                                        if config.strat_version == 'v5_2' and open_side is None:
                                            sys_log.warning(
                                                "ep_loc.point executed strat_ver : {}".format(config3.strat_version))

                                            res_df, open_side, _ = self.utils_public.short_ep_loc(res_df, config3,
                                                                                                  config.trader_set.last_index,
                                                                                                  np_timeidx)
                            ...
                            # ------ point matching ------ #
                                ...
                                if not config.pos_set.long_ban:
                                    res_df, open_side, _ = self.utils_public.long_ep_loc(res_df, config,
                                                                                         ...
                                                                                         np_timeidx)
                                    # ------ additional ep_loc by co-point ------ #
                                    if not config3.pos_set.long_ban:  # additional const. on trader
                                        if config.strat_version in ['v5_2'] and open_side is None:
                                            sys_log.warning(
                                                "ep_loc.point executed strat_ver : {}".format(config3.strat_version))
                                            res_df, open_side, zone = self.utils_public.long_ep_loc(res_df, config3,
                                                                                                    config.trader_set.last_index,
                                                                                                    np_timeidx)
                ...
                while 1:
                    ...
                    if datetime.timestamp(res_df.index[-1]) < datetime.now().timestamp():
                        ...
                        try:
                            ...
                            # config2.trader_set = config1.trader_set   # replaced trader_set saved, dont need anymore
                            # config3.trader_set = config1.trader_set
            ...
            #       position / 체결량 이 존재하면 close order 진행      #
                ...
                use_new_df2 = 0
                if not config.tp_set.static_tp or not config.out_set.static_out or strat_version in ['v5_2', 'v7_3']:
                ...
                while 1:
                    ...
                    if use_new_df2:
                        ...
                        if load_new_df2:  # dynamic_out & tp phase
                            ...
                            try:
                                use_rows, days = calc_rows_and_days(config.trader_set.itv_list,
                                                                    config.trader_set.row_list)
                            ...
                            try:
                                res_df = self.utils_public.sync_check(res_df, config.trader_set.row_list, order_side="CLOSE")

                                np_timeidx = np.array(list(map(lambda x: intmin(x), res_df.index)))

                                res_df = self.utils_public.public_indi(res_df, np_timeidx, order_side="CLOSE")
                            ...
                            if use_new_df2:
                                ...
                                try:
                                    ...
                                    if load_new_df3:
                                        ...
                                        new_df_, _ = concat_candlestick(config.trader_set.symbol, '1m',
                                                                        days=1,
                                                                        limit=config.trader_set.row_list[0],
                                                                        timesleep=None,
                                                                        show_process=0)
                                        if datetime.timestamp(new_df_.index[-1]) < datetime.now().timestamp():
                                            continue
