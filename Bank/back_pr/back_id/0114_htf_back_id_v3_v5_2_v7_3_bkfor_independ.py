...
def back_pr_check(res_df, png_save_path, utils_public, utils_list, config_list, key, show_plot=0, title_pos_y=0.5):
    ...
    utils1, utils2, utils3 = utils_list
    ...
    config = config1  # custom base config, if use override -> set to config1
    ...
    if override:
        res_df = enlist_rtc(res_df, config1, np_timeidx)
    else:
        res_df = utils1.enlist_rtc(res_df, config1, np_timeidx)
        if multi_mode:
            res_df = utils2.enlist_rtc(res_df, config2, np_timeidx)
            res_df = utils2.enlist_rtc(res_df, config3, np_timeidx)  # form 같은 경우, 같은 utils 사용 - config 만 변경
