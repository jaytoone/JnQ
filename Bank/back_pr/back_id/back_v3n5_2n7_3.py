...
from funcs.public.broker import sharpe_ratio
...
def back_pr_check(res_df, png_save_path, config_list, utils_lib_list, key, show_plot=0, title_pos_y=0.7):

    config1, config2, config3 = config_list
    ...
    for survey_i, just_loop in enumerate(range(1)):
        ...
        while 1:
                ...
                if override:
                    ...
                    if multi_mode:
                        if config.strat_version == 'v5_2' and open_side is None:
                            res_df, open_side, zone = utils_public.short_ep_loc(res_df, config3, i, np_timeidx,
                                                                                show_detail)
            ...
            #     res_df['entry_{}'.format(config2.strat_version)][i] == -config2.ep_set.short_entry_score:
                ...
                if override:
                    ...
                    if multi_mode:
                        if config.strat_version == 'v5_2' and open_side is None:
                            res_df, open_side, zone = utils_public.long_ep_loc(res_df, config3, i, np_timeidx,
                                                                               show_detail)
        ...
        try:
            ...
            plt.title(
                "wr : %.3f\n sr : %.3f\n acc_pr : %.3f\n sum_pr : %.3f\n acc_mdd : -%.3f\n sum_mdd : -%.3f\n min_pr : %.3f"
                % (rev_short_wr, rev_short_sr, short_rev_total_pr[-1], short_rev_sum_pr[-1], short_rev_acc_mdd,
                   short_rev_sum_mdd, np.min(rev_short_np_pr)))
        ...
        try:
            ...
            plt.title(
                "wr : %.3f\n sr : %.3f\n acc_pr : %.3f\n sum_pr : %.3f\n acc_mdd : -%.3f\n sum_mdd : -%.3f\n min_pr : %.3f"
                % (rev_long_wr, rev_long_sr, long_rev_total_pr[-1], long_rev_sum_pr[-1], long_rev_acc_mdd,
                   long_rev_sum_mdd, np.min(rev_long_np_pr)))
