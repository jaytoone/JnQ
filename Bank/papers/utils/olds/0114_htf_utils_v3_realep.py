...
def enlist_tr(res_df, config, np_timeidx, mode='OPEN'):
    ...
    res_df['short_open_{}'.format(strat_version)] = np.where((res_df['open'] >= res_df['bb_lower_%s' % config.loc_set.point.exp_itv]) &
    ...
    res_df['long_open_{}'.format(strat_version)] = np.where((res_df['open'] <= res_df['bb_upper_%s' % config.loc_set.point.exp_itv]) &
