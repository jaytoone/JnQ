...
def enlist_tr(res_df, config, np_timeidx, mode='OPEN', env='BANK', show_detail=True):
    ...
    if mode == "OPEN":
        ...
        #     c. wave_itv
        # if wave_itv1 != 'T':
        if config.loc_set.point1.tf_entry != 'T':
            # wave_itv1_num = itv_to_number(wave_itv1)
            short_open_res1 *= np_timeidx % itv_num == (itv_num - 1)
            long_open_res1 *= np_timeidx % itv_num == (itv_num - 1)
        ...
        fisher_back = res_df['fisher_{}30'.format(config.loc_set.point1.tf_entry)].shift(itv_num).to_numpy()

        short_open_res1 *= (fisher_back > 0) & (fisher_ <= 0)
        long_open_res1 *= (fisher_back < 0) & (fisher_ >= 0)

        if show_detail:
            sys_log.warning("fisher entry")
            sys_log.warning("np.sum(short_open_res1 == 1) : {}".format(np.sum(short_open_res1 == 1)))
            sys_log.warning("np.sum(long_open_res1 == 1) : {}".format(np.sum(long_open_res1 == 1)))
