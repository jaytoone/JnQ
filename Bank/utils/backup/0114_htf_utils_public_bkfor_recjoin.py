...
def sync_check(df_, config, order_side="OPEN", row_slice=False):
    ...
    df['rsi_1m'] = rsi(rec_df, 14)  # Todo - recursive, 250 period
    #      Todo     #
    #       len(df) != len(rec_df)
    # print(df.rsi_1m.tail())

    if order_side in ["OPEN"]:
        rec_df_5T['ema_5m'] = ema(rec_df_5T['close'], 155)  # Todo - recursive, 1350 period (5T)
