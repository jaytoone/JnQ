from funcs.public.olds.funcs_duration import *
...
def ep_loc(res_df, config, i, np_timeidx, show_detail=True, ep_loc_side=OrderSide.SELL):
    ...
    if config.loc_set.point.front_wick_score0 != "None":
    ...
    if np.sum(wick_score_arr == "None") != wick_score_arr.size:
        ...
        prev_hclose_idx_list = [i - ((np_timeidx[i] % config.loc_set.zone.c_itv_ticks) * (nbar_ + 1) + 1) for nbar_ in range(len(config.loc_set.point.front_wick_score))]
