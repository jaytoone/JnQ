...
def short_ep_loc(res_df, config, i, np_timeidx, show_detail=True):
    ...
    if np.sum(wick_score_arr == "None") != len(wick_score_arr):
        ...
        if config.loc_set.point.front_wick_score != "None":
            ...
            if strat_version in ['v5_2', '1_1']:
                ...
                if show_detail:
                    sys_log3.warning("front_wick_score <= -config.loc_set.point.front_wick_score : {:.5f} {:.5f} ({})".format(front_wick_score,
                                                                                                                                 -config.loc_set.point.front_wick_score,
                                                                                                                                 const_))
            ...
            if strat_version in ['v7_3', '2_2', 'v3']:
                ...
                if i < 0 or (i >= 0 and prev_hclose_idx >= 0):  # trader or colab env.
                    ...
                    if strat_version in ['v7_3']:
                        ...
                        if show_detail:
                            sys_log3.warning("total_ratio <= -config.loc_set.point.front_wick_score : {:.5f} {:.5f} ({})".format(
                                total_ratio,
                                -config.loc_set.point.front_wick_score,
                                const_))
                        ...
                        if show_detail:
                            sys_log3.warning(
                                "h_front_wick_score <= -config.loc_set.point.front_wick_score : {:.5f} {:.5f} ({})".format(
                                    h_front_wick_score,
                                    -config.loc_set.point.front_wick_score,
                                    const_))
                        mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, res_df, open_side, zone,
    ...
    if np.sum(wick_score2_arr == "None") != len(wick_score2_arr):
        ...
        if config.loc_set.point.front_wick_score2 != "None":
            ...
            if show_detail:
                sys_log3.warning("h_front_wick_score <= -config.loc_set.point.front_wick_score2 : {:.5f} {:.5f} ({})".format(h_front_wick_score,
                                                                                                                           -config.loc_set.point.front_wick_score2,
                                                                                                                           const_))
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, res_df, open_side, zone, show_detail)
        ...
        if config.loc_set.point.body_score2 != "None":
            ...
            if show_detail:
                sys_log3.warning("h_body_score >= config.loc_set.point.body_score2 : {:.5f} {:.5f} ({})".format(
                    h_body_score, config.loc_set.point.body_score2, const_))
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, res_df, open_side, zone, show_detail)

        if config.loc_set.point.back_wick_score2 != "None":
            const_ = h_back_wick_score >= config.loc_set.point.back_wick_score2
            if show_detail:
                sys_log3.warning("h_back_wick_score >= config.loc_set.point.back_wick_score2 : {:.5f} {:.5f} ({})".format(h_back_wick_score,
                                                                                                           config.loc_set.point.back_wick_score2,
                                                                                                           const_))
            mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, res_df, open_side, zone, show_detail)
    ...
    if np.sum(wick_score_arr == "None") != len(wick_score_arr):
        ...
        if config.loc_set.point.front_wick_score != "None":
            ...
            if strat_version in ['v5_2', '1_1']:
                ...
                if show_detail:
                    sys_log3.warning(
                        "front_wick_score >= config.loc_set.point.front_wick_score : {:.5f} {:.5f} ({})".format(front_wick_score,
                                                                                                                config.loc_set.point.front_wick_score,
                                                                                                                const_))
                mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, res_df, open_side, zone, show_detail)
            ...
            if strat_version in ['v7_3', '2_2', 'v3']:
                ...
                if i < 0 or (i >= 0 and prev_hclose_idx >= 0):
                    ...
                    if strat_version in ['v7_3']:
                        ...
                        const_ = h_front_wick_score >= config.loc_set.point.front_wick_score
                        if show_detail:
                            sys_log3.warning("h_front_wick_score >= config.loc_set.point.front_wick_score : {:.5f} {:.5f} ({})".format(
                                h_front_wick_score,
                                config.loc_set.point.front_wick_score,
                                const_))
                        mr_const_cnt, mr_score = mr_calc(mr_const_cnt, mr_score, const_, res_df, open_side, zone,
