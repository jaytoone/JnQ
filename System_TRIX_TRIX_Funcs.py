...
def chart_init(driver):
    ...
    while True:
        try:
            indicator_btn4 = driver.find_element_by_xpath(
                '//*[@title="피셔 트랜스폼 (Fisher Transform)"]')
            indicator_btn4.click()
            break
        except Exception as e:
    ...
    while True:
        try:
            indicator_btn4 = driver.find_elements_by_xpath('//*[@title="설정"]')[3]
            indicator_btn4.click()
            break
        except Exception as e:
...
def get_coin_data(driver, Coin, interval_key1, search_line=0, period1=80, period2=20, period3=18, period4=60):
    ...
    while True:
        try:
            ...
            value1 = float(indicator_value[1].text.split('\n')[0])
            value2 = float(indicator_value[2].text.replace("−", "-"))
            value3 = float(indicator_value[3].text.replace("−", "-"))
            value4 = float(indicator_value[4].text.split('\n')[0].replace("−", "-"))

            if value2 > search_line:
                return
...
def get_previous_data(driver):
    ...
    while True:
        try:
            ...
            #           last_offset의 value를 list에 넣어놓는다.       #
            if offset == last_offset:
                return_list.append(value3)
                return_fisher = value4
                return_close = close
            ...
            if tick_change_cnt >= 1:
                tick_change_cnt += 1
                return_list.append(value3)
            #                   UNTIL GET LAST 2 DATA                  #
            if tick_change_cnt >= 2:
                break
    ...
    return list(reversed(return_list)), return_fisher, return_close
