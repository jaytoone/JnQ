...
from Make_X_low_high_gaussian_span import low_high
...
if __name__ == '__main__':
    ...
    #           Get TopCoin List         #
    TopCoin = ['BNP', 'LRC', 'WTC', 'PLX', 'CTXC', 'MXC', 'SXP', 'TMTG',]
    ...
    for Coin in TopCoin:
        ...
        if len(X_test) != 0:
            ...
            for i in range(len(Y_pred_)):
                ...
                # if Y_pred_[i][1] > 0.9:  # 0.8
                    Y_pred[i] = 1
                if Y_pred_[i][2] > max_value[2] * 0.9:
                # if Y_pred_[i][2] > 0.9:  # 0.8
                    Y_pred[i] = 2
            ...
            if get_fig == 1:
                ...
                # plt.plot(Y_pred_[:, [1]], 'm', label='Y_pred')
                ...
                plt.subplot(212)
                plt.plot(min_max_scaler(chart[:, [1]]), 'gold', label='close')
                # plt.plot(Y_pred_[:, [2]], 'm', label='Y_pred')
                # plt.plot(chart[:, [-3]], 'r', label='close')
